...
====================================================
Copyright © 2019 Wang Xu Dong. All rights reserved.
Blog: http://www.wxdgame.com
E-mail: 153430533@qq.com
====================================================
 */
...
namespace BetaFramework
{
    public class ExcelWindow : EditorWindow
    {
        ...
        {
            m_ExcelClassPath = Application.dataPath + "/Scripts/GenExcelData/";
            ...
            m_ExcelOpenPath = Application.dataPath + "/Disign";
            m_ExcelDataPath = EditorPrefs.GetString(m_ExcelDataPathKey);
            //m_ExcelDataPath = m_ExcelDataPath == "" ? Application.dataPath + "/AssetsPackage/Config/" : m_ExcelDataPath;
            m_ExcelDataPath = Application.dataPath + "/AssetsPackage/Preload/Config/";
        ...
        private void ShowDirectoryArea()
        {
            ...
            EditorGUILayout.BeginVertical(helpBoxStyle);
            {
                ...
                {
                    // GUI.color = Color.green;
                    if (m_SelectedClearFolder != EditorGUILayout.ToggleLeft("Clear Folder", m_SelectedClearFolder, GUILayout.Height(20)))
                    {
                        m_SelectedClearFolder = !m_SelectedClearFolder;
                        EditorPrefs.SetBool(m_SelectedClearFolderKey, m_SelectedClearFolder);
                    }
                    if (GUILayout.Button(EditorGUIUtility.FindTexture("Refresh"), GUILayout.Width(30), GUILayout.Height(30)))
                    {
                        m_ExcelSelectedPaths.Clear();
                        m_ExcelPaths.Clear();
                        OpenExcelFolder();
            ...
            EditorGUILayout.BeginVertical(helpBoxStyle);
            {
                for (int i = 0; i < m_ExcelPaths.Count; i++)
                {
                    ...
                    EditorGUILayout.BeginHorizontal();
                    {
                        ...
                        {
                            EditorWindow.GetWindow<ExcelExporter>().titleContent = new GUIContent(Path.GetFileNameWithoutExtension(path));
                ...
                EditorGUILayout.BeginHorizontal();
                {
                    ...
                    if (GUILayout.Button("Create Excel Class", GUILayout.Height(30)))
                    {
                        if (m_SelectedClearFolder)
                            Utils.DeleteDirectory(m_ExcelClassPath);
                        ...
                        for (int i = 0; i < count; i++)
                        {
                            ...
                            foreach (KeyValuePair<string, List<List<ICell>>> keyPair in sheets)
                            {
                                ...
                                EditorUtility.DisplayProgressBar("Info", "Create Excel Class : " + (i + 1) + "/" + count + " " + tableClassName + ".cs", ((i + 1) / count) * 100);
                                m_ExcelClassCreator.Create(sheetIndex, m_ExcelClassPath, tableClassName, tableNoteName, sheetName, keyPair.Value);
                                sheetIndex++;
                            }
                        }
                        if (count > 0)
                        ...
                        {
                            EditorUtility.DisplayDialog("Worning", "Select some excel files(.xls or .xlsx) , please .", "OK");
                        }
                    }
                    if (GUILayout.Button("Create Excel Data", GUILayout.Height(30)))
                    {
                        if (m_SelectedClearFolder)
                            Utils.DeleteDirectory(m_ExcelDataPath);
                        ...
                        for (int i = 0; i < m_ExcelSelectedPaths.Count; i++)
                        {
                            ...
                            foreach (KeyValuePair<string, List<List<ICell>>> keyPair in sheets)
                            {
                                ...
                                AssetDatabase.CreateAsset(container, m_ExcelDataPath.Replace(Application.dataPath, "Assets") + tableClassName + "_" + sheetName + ".asset");
                                ...
                                for (int row = 3; row < keyPair.Value.Count; row++)
                                {
                                    ...
                                    for (int column = 0; column < nameRow.Count; column++)
                                    {
                                        ...
                                        object value = ExcelUtility.GetCellValue(contentRow[column]);
                                        Debug.Log(tableClassName + "_" + sheetName + "_" + row + "_" + column + "_" + typeRow[column] + "_" + typeRow[column].StringCellValue + "_" + value);
                                        fieldInfo.SetValue(data, ExcelUtility.GetConvertType(typeRow[column].StringCellValue, value));
                                    }
                                    ...
                                    AssetDatabase.AddObjectToAsset(data, m_ExcelDataPath.Replace(Application.dataPath, "Assets") + tableClassName + "_" + sheetName + ".asset");
                                }
                                EditorUtility.SetDirty(container);
                                EditorUtility.DisplayProgressBar("Info", "Create Excel Data : " + (i + 1) + "/" + m_ExcelSelectedPaths.Count + " " + tableClassName + "_" + sheetName, (i / m_ExcelSelectedPaths.Count) * 100);
                            }
                        }

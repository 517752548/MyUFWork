...
";
            m_ExcelSheetTypeAttribute += string.Format(templete, m_TableNoteName + "表", m_TableClassName, m_SheetName, sheetNum.ToString());
        }
...
@"
        {0} = {1}(data[""{0}""]);";
            ...
            {
                attribute += string.Format(templete, attributes[i], ExcelUtility.GetConvertTypeString(types[i].StringCellValue));
            }
...
->";
            ...
            Utils.CreateFile(m_SavePath + "BaseSheetData.cs", Encoding.UTF8.GetBytes(templete));
        }
...
->";
            ...
            Utils.CreateFile(m_SavePath + "ExcelSheetType.cs", Encoding.UTF8.GetBytes(templete));
        }
        private void CreateManagerClass(List<string> excelPaths, List<string> excelSelectedPaths, ExcelReader excelReader)
        {
            if (File.Exists(m_SavePath + "ExcelDataManager.cs"))
            {
                ...
                MemberInfo[] memberInfos = classType.GetMembers(BindingFlags.Instance | BindingFlags.NonPublic | BindingFlags.DeclaredOnly);
...
->";
            ...
            m_ManagerGetData = "";
            m_ManagerInitData = "";
            Utils.CreateFile(m_SavePath + "ExcelDataManager.cs", Encoding.UTF8.GetBytes(templete));
        }
...
/// {1}
/// </summary>
public class {2}_Data : BaseExcelData
<-
    __ATTRIBUTES__
...
->";
            ...
            Utils.CreateFile(m_SavePath + "ExcelDatas/" + m_TableClassName + "_Data.cs", Encoding.UTF8.GetBytes(m_ExcelDataTemplate));
        }
...
->";
            ...
            templete = string.Format(templete, DateTime.Now.ToString("yyyy-MM-dd HH:mm:ss.fff"), m_TableNoteName + "表", m_TableClassName);
            templete = templete.Replace("<-", "{");
            templete = templete.Replace("->", "}");

            Utils.CreateFile(m_SavePath + "ExcelDatas/" + m_TableClassName + ".cs", Encoding.UTF8.GetBytes(templete));
        }

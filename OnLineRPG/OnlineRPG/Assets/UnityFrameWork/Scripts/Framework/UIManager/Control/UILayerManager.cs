...
public class UILayerManager : MonoBehaviour
{
    ...
    public void Awake()
    {
        for (int i = 0; i < UICameraList.Count; i++)
        {
            ...
            if (data.m_GuideLayerParent == null)
            {
                Debug.LogError("UILayerManager :guideLayerParent is null!" + " key : " + data.m_key + " index : " + i);
            }
        }
    }
    ...
    public void SetLayer(UIWindowBase ui, string cameraKey = null)
    {
        ...
        switch (ui.m_UIType)
        {
            ...
            case UIType.TopBar:
                ui.transform.SetParent(data.m_TopbarLayerParent,false);
                ui.transform.SetAsLastSibling();
                break;

            case UIType.PopUp:
                ui.transform.SetParent(data.m_PopUpLayerParent,false);
                break;

            case UIType.Guide:
                ui.transform.SetParent(data.m_GuideLayerParent, false);
                break;
        }
    ...
    public struct UICameraData
    {
        ...
        public Transform m_GuideLayerParent;
    }
}
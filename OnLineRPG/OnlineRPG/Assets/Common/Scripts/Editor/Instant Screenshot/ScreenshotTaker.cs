...
public class Screenshot : EditorWindow
{
	...
	void OnGUI()
	{
		...
		if(GUILayout.Button("Take Screenshot",GUILayout.MinHeight(60)))
		{
			...
			{
				path = EditorUtility.SaveFolderPanel("Path to Save Images",path,Application.dataPath);
				BetaFramework.LoggerHelper.Log("Path Set");
				TakeHiResShot();
			}
			else
			{
		...
		if(GUILayout.Button("Open Last Screenshot",GUILayout.MaxWidth(160),GUILayout.MinHeight(40)))
		{
			if(lastScreenshot != "")
			{
				...
				BetaFramework.LoggerHelper.Log("Opening File " + lastScreenshot);
			}
		}
		...
		if (takeHiResShot) 
		{
            ...
            BetaFramework.LoggerHelper.Log(string.Format("Took screenshot to: {0}", filename));
            takeHiResShot = false;
        }
	...
	public void TakeHiResShot() {
		BetaFramework.LoggerHelper.Log("Taking Screenshot");
		takeHiResShot = true;
	}

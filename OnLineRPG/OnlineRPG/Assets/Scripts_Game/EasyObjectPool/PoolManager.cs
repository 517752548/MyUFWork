...
namespace SG
{
    ...
    public class PoolManager : MonoBehaviour
    {
        ...
        public static PoolManager Instance
        {
            get
            {
                if (mInstance == null)
                {
                    ...
                    mInstance = GO.GetComponent<PoolManager>();
        ...
        public void InitPool(string poolName, int size, PoolInflationType type = PoolInflationType.DOUBLE)
        {
            ...
            {
                GameObject pd = PreLoadManager.GetPreLoad<GameObject>(PreLoadConst.preload_Prefab,ViewConst.prefab_KnowledgeCardItem);
                if (pd == null)
                {
                    Debug.LogError("[ResourceManager] Invalide prefab name for pooling :" + poolName);
                    return;
                }
                poolDict[poolName] = new Pool(poolName, pd, gameObject, size, type);
            }
        }

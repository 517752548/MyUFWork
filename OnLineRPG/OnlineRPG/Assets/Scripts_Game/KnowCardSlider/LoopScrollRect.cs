...
namespace UnityEngine.UI
{
    ...
    public abstract class LoopScrollRect : UIBehaviour, IInitializePotentialDragHandler, IBeginDragHandler, IEndDragHandler, IDragHandler, IScrollHandler, ICanvasElement, ILayoutElement, ILayoutGroup
    {
        ...
        public List<object> objectsToFill
        {
            ...
            {
                if(value != null)
                    dataSource = new KnowledgeLoopScrollArraySource<object>(value);
                else
        ...
        public float headViewHeight = 430;
        public float CriticalHeight = 500;
        ...
        public void RefillCellsFromEnd(int offset = 0)
        {
            ...
            for (int i = m_Content.childCount - 1; i >= 1; i--)
            {
        ...
        protected float NewItemAtStart()
        {
            ...
            {
                itemTypeStart--;
                RectTransform newItem = InstantiateNextItem(itemTypeStart,true);
               // newItem.SetAsFirstSibling();
                newItem.SetSiblingIndex(1);
...
//            // protection
        ...
        protected float DeleteItemAtStart()
        {
            ...
            for (int i = 0; i < contentConstraintCount; i++)
            {
                RectTransform oldItem = content.GetChild(1) as RectTransform;
        ...
        private RectTransform InstantiateNextItem(int itemIdx,bool isAtartItem = false)
        {            
            KnowledgeCardViewItem nextItem = prefabSource.GetObject().GetComponent<KnowledgeCardViewItem>();
            nextItem.transform.SetParent(content, false);
            nextItem.gameObject.SetActive(true);
            nextItem.bindListener(this);
            dataSource.ProvideData(nextItem, itemIdx,isAtartItem);
            return nextItem.getRectTransform();
        }
        //==========LoopScrollRect==========
        ...
        private void UpdateBounds(bool updateItems = true,bool isUpdateBonus = false)
        {
            m_ViewBounds = new Bounds(viewRect.rect.center, viewRect.rect.size);
            m_ContentBounds = GetBounds();
            ...
            if (Application.isPlaying && updateItems && UpdateItems(m_ViewBounds, m_ContentBounds) || isUpdateBonus)
            {
                Canvas.ForceUpdateCanvases();
                m_ContentBounds = GetBounds();
            }
        ...
        public void updateBonusCallback(bool isUpdate)
        {
            if (isUpdate)
            {
                m_ViewBounds = new Bounds(viewRect.rect.center, viewRect.rect.size);
        ...
        //回调方法
        public void completeTextHeight(float moveDis,float itemSize = 0,bool isStart = true)
        {
            float size = moveDis;
            if (!reverseDirection && isStart)
            {
                Vector2 offset = GetVector(size);
                content.anchoredPosition += offset;
                m_PrevPosition += offset;
            ...
            if (itemSize > 0 && threshold < itemSize)
            {
                threshold = itemSize * 1.1f;
            }
        }

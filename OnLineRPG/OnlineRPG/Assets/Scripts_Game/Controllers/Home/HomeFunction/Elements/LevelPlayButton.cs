...
namespace Scripts_Game.Controllers.Home.Elements
{
    public class LevelPlayButton : BaseHomeUI
    {
        ...
        public HomeThemeRoot homeThemeRoot => root.GetHomeUi<HomeThemeRoot>();
        ...
        public void ClickLevelButton()
        {
            ...
            {
                EnterClassicLevel();
                GameAnalyze.LogEvent("ClickLevel",AppEngine.SSystemManager.GetSystem<ClassicGameSystem>().currentLevel.Value.ToString());
            }
        }
            ...
            UIManager.OpenUIAsync(ViewConst.prefab_GameLoadingWindow, OpenType.Replace, (ui, para) =>
            {
                AppEngine.SSystemManager.GetSystem<ClassicGameSystem>().LoadClassicLevel(
                    ...
                    (ok) =>
                    {
                        ...
                        {
                            GameAnalyze.LogEvent("LoadLevelFail",new Dictionary<string, string>(){{"level",AppEngine.SSystemManager.GetSystem<ClassicGameSystem>().currentLevel.Value.ToString()},{"net",(Application.internetReachability != NetworkReachability.NotReachable) + ""}});
                            LoadLevelFailed();
                        }
                    });
            });

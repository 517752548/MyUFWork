    ...
    public partial class VectorLine
    {
        ...
        public List<Vector2> points2
        {
            get
            {
                if (!m_is2D)
                {
                    BetaFramework.LoggerHelper.Error("Line \"" + name + "\" uses points3 rather than points2");
                    return null;
                }
                return m_points2;
            }
            set
            {
                if (value == null)
                {
                    BetaFramework.LoggerHelper.Error("List for Line \"" + name + "\" must not be null");
                    return;
                }
                m_points2 = value;
            }
        ...
        public List<Vector3> points3
        {
            get
            {
                if (m_is2D)
                {
                    BetaFramework.LoggerHelper.Error("Line \"" + name + "\" uses points2 rather than points3");
                    return null;
                }
                return m_points3;
            }
        ...
        public float capLength
        {
            ...
            set
            {
                ...
                {
                    BetaFramework.LoggerHelper.Error("LineType.Points can't use capLength");
                    return;
                }
                m_capLength = value;
            }
        ...
        public string endCap
        {
            ...
            set
            {
                if (m_lineType == LineType.Points)
                {
                    BetaFramework.LoggerHelper.Error("LineType.Points can't use end caps");
                    return;
                }
                if (m_endCap == value)
                {
                ...
                {
                    BetaFramework.LoggerHelper.Error("End cap \"" + value + "\" is not set up");
                    return;
                }
                if (m_capType != EndCap.None)
                {
        ...
        public bool useViewportCoords
        {
            ...
            set
            {
                ...
                {
                    BetaFramework.LoggerHelper.Error("Line must use Vector2 points in order to use viewport coords");
                }
            }
        }
        ...
        public int drawDepth
        {
            get
            {
                ...
                {
                    BetaFramework.LoggerHelper.Error("VectorLine.drawDepth can't be used with lines made with Draw3D");
                    return 0;
                }
        ...
        public static LineManager lineManager
        {
            get
            {
                ...
                if (!lineManagerCreated)
                {
                    ...
                    AppEngine.AddDontGameObject(lineManagerGO);
                }
        ...
        protected void SetupLine(string lineName, Texture texture, float width, LineType lineType, Joins joins, bool use2D)
        {
            ...
            {
                BetaFramework.LoggerHelper.Error("VectorLine: Must use LineType.Continuous if using Joins.Fill for \"" + lineName + "\"");
                return;
            }
            if (joins == Joins.Weld && m_lineType == LineType.Points)
            {
                BetaFramework.LoggerHelper.Error("VectorLine: LineType.Points can't use Joins.Weld for \"" + lineName + "\"");
                return;
            }
            if ((m_is2D && m_points2 == null) || (!m_is2D && m_points3 == null))
            {
                BetaFramework.LoggerHelper.Error("VectorLine: the points array is null for \"" + lineName + "\"");
                return;
            }
        ...
        private void SetupEndCap(float[] uvHeights)
        {
            ...
            {
                BetaFramework.LoggerHelper.Error("VectorLine: exceeded maximum vertex count of 65534 for \"" + m_name + "\"...use fewer points");
                return;
            }
        ...
        public void Resize(int newCount)
        {
            if (newCount < 0)
            {
                BetaFramework.LoggerHelper.Error("VectorLine.Resize: the new count must be >= 0");
                return;
            }
        ...
        private bool SetVertexCount()
        {
            ...
            {
                BetaFramework.LoggerHelper.Error("VectorLine: exceeded maximum vertex count of 65534 for \"" + name + "\"...use fewer points (maximum is 16383 points for continuous lines and points, and 32767 points for discrete lines)");
                return false;
            }
            return true;
        }
        ...
        public void SetCanvas(GameObject canvasObject, bool worldPositionStays)
        {
            ...
            if (canvas == null)
            {
                BetaFramework.LoggerHelper.Error("VectorLine.SetCanvas: canvas object must have a Canvas component");
                return;
            }
        ...
        public void SetCanvas(Canvas canvas, bool worldPositionStays)
        {
            if (m_canvasState == CanvasState.OffCanvas)
            {
                BetaFramework.LoggerHelper.Error("VectorLine.SetCanvas only works with lines made with Draw, not Draw3D.");
                return;
            }
            if (canvas == null)
            {
                BetaFramework.LoggerHelper.Error("VectorLine.SetCanvas: canvas must not be null");
                return;
            }
            if (canvas.renderMode == RenderMode.WorldSpace)
            {
                BetaFramework.LoggerHelper.Error("VectorLine.SetCanvas: canvas must be screen space overlay or screen space camera");
                return;
            }
        ...
        public void SetMask(GameObject maskObject, bool worldPositionStays)
        {
            ...
            if (mask == null)
            {
                BetaFramework.LoggerHelper.Error("VectorLine.SetMask: mask object must have a Mask component");
                return;
            }
        ...
        public void SetMask(Mask mask, bool worldPositionStays)
        {
            if (m_canvasState == CanvasState.OffCanvas)
            {
                BetaFramework.LoggerHelper.Error("VectorLine.SetMask only works with lines made with Draw, not Draw3D.");
                return;
            }
            if (mask == null)
            {
                BetaFramework.LoggerHelper.Error("VectorLine.SetMask: mask must not be null");
                return;
            }
        ...
        private bool CheckCamera3D()
        {
            if (!m_is2D && !cam3D)
            {
                ...
                if (!cam3D)
                {
                    BetaFramework.LoggerHelper.Error("No camera available...use VectorLine.SetCamera3D to assign a camera");
                    return false;
                }
            }
            return true;
        ...
        public static void SetCamera3D()
        {
            if (Camera.main == null)
            {
                BetaFramework.LoggerHelper.Error("VectorLine.SetCamera3D: no camera tagged \"Main Camera\" found. Please call SetCamera3D with a specific camera instead.");
                return;
            }
        ...
        public static void SetCamera3D(GameObject cameraObject)
        {
            ...
            if (camera == null)
            {
                BetaFramework.LoggerHelper.Error("VectorLine.SetCamera3D: camera object must have a Camera component");
                return;
            }
            SetCamera3D(camera);
        }
        ...
        public void SetEndCapColor(Color32 frontColor, Color32 backColor)
        {
            if (m_capType == EndCap.None)
            {
                BetaFramework.LoggerHelper.Error("VectorLine.SetEndCapColor: the line \"" + name + "\" does not have any end caps");
                return;
            }
            m_useCapColors = true;
        ...
        public void SetEndCapIndex(EndCap endCap, int index)
        {
            if (m_capType == EndCap.None)
            {
                BetaFramework.LoggerHelper.Error("VectorLine.SetEndCapIndex: the line \"" + name + "\" does not have any end caps");
                return;
            }
            if (endCap != EndCap.Front && endCap != EndCap.Back)
            {
                BetaFramework.LoggerHelper.Error("VectorLine.SetEndCapIndex: endCap must be EndCap.Front or EndCap.Back");
                return;
            }
            if (index < 0)
            {
        ...
        public void SetColors(List<Color32> lineColors)
        {
            if (lineColors == null)
            {
                BetaFramework.LoggerHelper.Error("VectorLine.SetColors: lineColors list must not be null");
                return;
            }
            if (pointsCount != m_pointsCount)
            {
            ...
            {
                BetaFramework.LoggerHelper.Error("VectorLine.SetColors: Length of lineColors list in \"" + name + "\" must be same length as points list");
                return;
            }
        ...
        public Color32 GetColor(int index)
        {
            ...
            {
                BetaFramework.LoggerHelper.Error("VectorLine.GetColor: index " + index + " out of range");
                return Color.clear;
            }
        ...
        private void SetWidths(List<float> lineWidthsFloat, List<int> lineWidthsInt, int arrayLength, bool doFloat)
        {
            ...
            {
                BetaFramework.LoggerHelper.Error("VectorLine.SetWidths: line widths list must not be null");
                return;
            }
            if (pointsCount != m_pointsCount)
            {
            ...
            if (m_lineType == LineType.Points)
            {
                ...
                {
                    BetaFramework.LoggerHelper.Error("VectorLine.SetWidths: line widths list must be the same length as the points list for \"" + name + "\"");
                    return;
                }
            }
        ...
        public float GetWidth(int index)
        {
            ...
            {
                BetaFramework.LoggerHelper.Error("VectorLine.GetWidth: index " + index + " out of range...must be >= 0 and < " + max);
                return 0;
            }
        ...
        public static VectorLine SetLine(Color color, float time, params Vector2[] points)
        {
            if (points.Length < 2)
            {
                BetaFramework.LoggerHelper.Error("VectorLine.SetLine needs at least two points");
                return null;
            }
        ...
        public static VectorLine SetLine3D(Color color, float time, params Vector3[] points)
        {
            if (points.Length < 2)
            {
                BetaFramework.LoggerHelper.Error("VectorLine.SetLine3D needs at least two points");
                return null;
            }
        ...
        private void SetupCanvasState(CanvasState wantedState)
        {
            ...
            if (m_material == null)
            {
                ...
                if (mat == null)
                {
                    BetaFramework.LoggerHelper.Error("No DefaultLine3D material found in Resources");
                    return;
                }
        ...
        public void Draw3D()
        {
            ...
            if (m_is2D)
            {
                BetaFramework.LoggerHelper.Error("VectorLine.Draw3D can only be used with a Vector3 array, which \"" + name + "\" doesn't have");
                return;
            }
            if (m_canvasState != CanvasState.OffCanvas)
            {
        ...
        private void SetCollider(bool convertToWorldSpace)
        {
            ...
            {
                BetaFramework.LoggerHelper.Warning("The line collider will not be correct if the camera is rotated");
            }
        ...
        public static List<Vector3> BytesToVector3List(byte[] lineBytes)
        {
            ...
            {
                BetaFramework.LoggerHelper.Error("VectorLine.BytesToVector3Array: Incorrect input byte length...must be a multiple of 12");
                return null;
            }
        ...
        public static List<Vector2> BytesToVector2List(byte[] lineBytes)
        {
            ...
            {
                BetaFramework.LoggerHelper.Error("VectorLine.BytesToVector2Array: Incorrect input byte length...must be a multiple of 8");
                return null;
            }
        ...
        public Vector2 GetPoint(float distance, out int index)
        {
            if (!m_is2D)
            {
                BetaFramework.LoggerHelper.Error("VectorLine.GetPoint only works with Vector2 points");
                index = 0;
                return Vector2.zero;
            }
        ...
        public Vector3 GetPoint3D(float distance, out int index)
        {
            if (m_is2D)
            {
                BetaFramework.LoggerHelper.Error("VectorLine.GetPoint3D only works with Vector3 points");
                index = 0;
                return Vector3.zero;
            }
        ...
        public static void SetEndCap(string name, EndCap capType, float offsetFront, float offsetBack, float scaleFront, float scaleBack, params Texture2D[] textures)
        {
            ...
            if (name == null || name == "")
            {
                BetaFramework.LoggerHelper.Error("VectorLine.SetEndCap: must supply a name");
                return;
            }
            if (capDictionary.ContainsKey(name) && capType != EndCap.None)
            {
                BetaFramework.LoggerHelper.Error("VectorLine.SetEndCap: end cap \"" + name + "\" has already been set up");
                return;
            }
            ...
            {
                BetaFramework.LoggerHelper.Error("VectorLine.SetEndCap (\"" + name + "\"): must supply two textures when using SetEndCap with EndCap.Front, EndCap.Back, or EndCap.Mirror");
                return;
            }
            if (textures[0] == null || textures[1] == null)
            {
                BetaFramework.LoggerHelper.Error("VectorLine.SetEndCap (\"" + name + "\"): end cap textures must not be null");
                return;
            }
            if (textures[0].width != textures[0].height)
            {
                BetaFramework.LoggerHelper.Error("VectorLine.SetEndCap (\"" + name + "\"): the line texture must be square");
                return;
            }
            if (textures[1].height != textures[0].height)
            {
                BetaFramework.LoggerHelper.Error("VectorLine.SetEndCap (\"" + name + "\"): all textures must be the same height");
                return;
            }
            if (capType == EndCap.Both)
            {
                ...
                {
                    BetaFramework.LoggerHelper.Error("VectorLine.SetEndCap (\"" + name + "\"): must supply three textures when using SetEndCap with EndCap.Both");
                    return;
                }
                if (textures[2] == null)
                {
        ...
        public static void RemoveEndCap(string name)
        {
            ...
            {
                BetaFramework.LoggerHelper.Error("VectorLine: RemoveEndCap: \"" + name + "\" has not been set up");
                return;
            }
        ...
        public bool Selected(Vector2 p, int extraDistance, int extraLength, out int index, Camera cam)
        {
            if (cam == null)
            {
                ...
                if (!cam3D)
                {
                    BetaFramework.LoggerHelper.Error("VectorLine.Selected: camera cannot be null. If there is no camera tagged \"MainCamera\", supply one manually");
                    index = 0;
                    return false;
                }
                cam = cam3D;

...
public partial class VectorLine {
	...
	bool WrongArrayLength (int arrayLength, FunctionName functionName) {
		if (m_lineType == LineType.Continuous) {
			if (arrayLength != pointsCount-1) {
				BetaFramework.LoggerHelper.Error (functionNames[(int)functionName] + " list for \"" + name + "\" must be length of points array minus one for a continuous line (one entry per line segment). Expected " + (pointsCount-1) + ", got " + arrayLength);
				return true;
			}
		}
		else if (arrayLength != pointsCount/2) {
			BetaFramework.LoggerHelper.Error (functionNames[(int)functionName] + " list in \"" + name + "\" must be exactly half the length of points array for a discrete line (one entry per line segment). Expected " + (pointsCount/2) + ", got " + arrayLength);
			return true;
		}
		return false;
	}
	...
	bool CheckArrayLength (FunctionName functionName, int segments, int index) {
		if (segments < 1) {
			BetaFramework.LoggerHelper.Error ("VectorLine." + functionNames[(int)functionName] + " needs at least 1 segment");
			return false;
		}
		if (index < 0) {
			BetaFramework.LoggerHelper.Error ("VectorLine." + functionNames[(int)functionName] + ": The index value for \"" + name + "\" must be >= 0");
			return false;
		}
		if (m_lineType == LineType.Points) {
			if (index + segments > pointsCount) {
				if (index == 0) {
					BetaFramework.LoggerHelper.Error ("VectorLine." + functionNames[(int)functionName] + ": The number of segments cannot exceed the number of points in the array for \"" + name + "\"");
					return false;
				}
				BetaFramework.LoggerHelper.Error ("VectorLine: Calling " + functionNames[(int)functionName] + " with an index of " + index + " would exceed the length of the Vector array for \"" + name + "\"");
				return false;
			}
			return true;
		}
		...
		if (m_lineType == LineType.Continuous) {
			if (index + (segments+1) > pointsCount) {
				if (index == 0) {
					BetaFramework.LoggerHelper.Error ("VectorLine." + functionNames[(int)functionName] + ": The length of the array for continuous lines needs to be at least the number of segments plus one for \"" + name + "\"");
					return false;
				}
				BetaFramework.LoggerHelper.Error ("VectorLine: Calling " + functionNames[(int)functionName] + " with an index of " + index + " would exceed the length of the Vector array (" + pointsCount + ") for \"" + name + "\"");
				return false;
			}
		}
		else {
			if (index + segments*2 > pointsCount) {
				if (index == 0) {
					BetaFramework.LoggerHelper.Error ("VectorLine." + functionNames[(int)functionName] + ": The length of the array for discrete lines needs to be at least twice the number of segments for \"" + name + "\"");
					return false;
				}
				BetaFramework.LoggerHelper.Error ("VectorLine: Calling " + functionNames[(int)functionName] + " with an index of " + index + " would exceed the length of the Vector array (" + pointsCount + ") for \"" + name + "\"");
				return false;
	...
	public void MakeRect (Vector3 bottomLeft, Vector3 topRight, int index) {
		if (m_lineType != LineType.Discrete) {
			if (index + 5 > pointsCount) {
				if (index == 0) {
					BetaFramework.LoggerHelper.Error ("VectorLine.MakeRect: The length of the array for continuous lines needs to be at least 5 for \"" + name + "\"");
					return;
				}
				BetaFramework.LoggerHelper.Error ("Calling VectorLine.MakeRect with an index of " + index + " would exceed the length of the Vector2 array for \"" + name + "\"");
				return;
			}
			if (m_is2D) {
			...
			if (index + 8 > pointsCount) {
				if (index == 0) {
					BetaFramework.LoggerHelper.Error ("VectorLine.MakeRect: The length of the array for discrete lines needs to be at least 8 for \"" + name + "\"");
					return;
				}
				BetaFramework.LoggerHelper.Error ("Calling VectorLine.MakeRect with an index of " + index + " would exceed the length of the Vector2 array for \"" + name + "\"");
				return;
	...
	public void MakeRoundedRect (Vector3 bottomLeft, Vector3 topRight, float cornerRadius, int cornerSegments, int index) {
		if (cornerSegments < 1) {
			BetaFramework.LoggerHelper.Error ("VectorLine.MakeRoundedRect: cornerSegments value must be >= 1");
			return;
		}
		if (index < 0) {
			BetaFramework.LoggerHelper.Error ("VectorLine.MakeRoundedRect: index value must be >= 0");
			return;
		}
		if (!m_is2D && bottomLeft.z != topRight.z) {
			BetaFramework.LoggerHelper.Error ("VectorLine.MakeRoundedRect only works on the X/Y plane");
			return;
		}
	...
	private void MakeEllipse (Vector3 origin, Vector3 upVector, float xRadius, float yRadius, float startDegrees, float endDegrees, int segments, float pointRotation, int index) {
		if (segments < 3) {
			BetaFramework.LoggerHelper.Error ("VectorLine.MakeEllipse needs at least 3 segments");
			return;
		}
	...
	public void MakeCurve (Vector2[] curvePoints, int segments, int index) {
		if (curvePoints.Length != 4) {
			BetaFramework.LoggerHelper.Error ("VectorLine.MakeCurve needs exactly 4 points in the curve points array");
			return;
		}
	...
	private void MakeSpline (Vector2[] splinePoints2, Vector3[] splinePoints3, int segments, int index, bool loop) {
		...
		if (pointsLength < 2) {
			BetaFramework.LoggerHelper.Error ("VectorLine.MakeSpline needs at least 2 spline points");
			return;
		}
		if (splinePoints2 != null && !m_is2D) {
			BetaFramework.LoggerHelper.Error ("VectorLine.MakeSpline was called with a Vector2 spline points array, but the line uses Vector3 points");
			return;
		}
		if (splinePoints3 != null && m_is2D) {
			BetaFramework.LoggerHelper.Error ("VectorLine.MakeSpline was called with a Vector3 spline points array, but the line uses Vector2 points");
			return;
		}
	...
	public void MakeText (string text, Vector3 startPos, float size, float charSpacing, float lineSpacing, bool uppercaseOnly) {
		if (m_lineType != LineType.Discrete) {
			BetaFramework.LoggerHelper.Error ("VectorLine.MakeText only works with a discrete line");
			return;
		}
		...
		for (int i = 0; i < text.Length; i++) {
			...
			if (charNum < 0 || charNum > VectorChar.numberOfCharacters) {
				BetaFramework.LoggerHelper.Error ("VectorLine.MakeText: Character '" + text[i] + "' is not valid");
				return;
			}
	...
	public void MakeWireframe (Mesh mesh) {
		if (m_lineType != LineType.Discrete) {
			BetaFramework.LoggerHelper.Error ("VectorLine.MakeWireframe only works with a discrete line");
			return;
		}
		if (m_is2D) {
			BetaFramework.LoggerHelper.Error ("VectorLine.MakeWireframe can only be used with Vector3 points, which \"" + name + "\" doesn't have");
			return;
		}
		if (mesh == null) {
			BetaFramework.LoggerHelper.Error ("VectorLine.MakeWireframe can't use a null mesh");
			return;
		}
	...
	public void MakeCube (Vector3 position, float xSize, float ySize, float zSize, int index) {
		if (m_lineType != LineType.Discrete) {
			BetaFramework.LoggerHelper.Error ("VectorLine.MakeCube only works with a discrete line");
			return;
		}
		if (m_is2D) {
			BetaFramework.LoggerHelper.Error ("VectorLine.MakeCube can only be used with Vector3 points, which \"" + name + "\" doesn't have");
			return;
		}
		if (index + 24 > pointsCount) {
			if (index == 0) {
				BetaFramework.LoggerHelper.Error ("VectorLine.MakeCube: The number of Vector3 points needs to be at least 24 for \"" + name + "\"");
				return;
			}
			BetaFramework.LoggerHelper.Error ("Calling VectorLine.MakeCube with an index of " + index + " would exceed the length of the Vector3 points for \"" + name + "\"");
			return;
		}

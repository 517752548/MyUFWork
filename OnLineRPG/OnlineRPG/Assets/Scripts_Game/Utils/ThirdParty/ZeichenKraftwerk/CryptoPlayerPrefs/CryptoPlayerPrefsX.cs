...
public class CryptoPlayerPrefsX
{
	...
	public static bool SetBoolArray (String key, bool[] boolArray)
	{
		if (boolArray.Length == 0) {
			BetaFramework.LoggerHelper.Error ("The bool array cannot have 0 entries when setting " + key);
			return false;
		}
	...
	{
		if (Record.HasKey (key)) {
			var bytes = System.Convert.FromBase64String (Record.GetString (key));
			if (bytes.Length < 6) {
				BetaFramework.LoggerHelper.Error ("Corrupt preference file for " + key);
				return new bool[0];
			}
			if ((ArrayType)bytes [0] != ArrayType.Bool) {
				BetaFramework.LoggerHelper.Error (key + " is not a boolean array");
				return new bool[0];
			}
			Initialize ();
	...
	public static bool SetStringArray (String key, String[] stringArray)
	{
		if (stringArray.Length == 0) {
			BetaFramework.LoggerHelper.Error ("The string array cannot have 0 entries when setting " + key);
			return false;
		}
		...
		for (var i = 0; i < stringArray.Length; i++) {
			if (stringArray [i] == null) {
				BetaFramework.LoggerHelper.Error ("Can't save null entries in the string array when setting " + key);
				return false;
			}
			if (stringArray [i].Length > 255) {
				BetaFramework.LoggerHelper.Error ("Strings cannot be longer than 255 characters when setting " + key);
				return false;
			}
		...
		try		{
			Record.SetString (key, System.Convert.ToBase64String (bytes) + "|" + String.Join ("", stringArray));
		} catch {
			return false;
		}
		return true;
	...
	public static string TransStringArray (String[] stringArray)
	{
		if (stringArray.Length == 0) {
			return "";
		}
		...
		return Convert.ToBase64String (bytes) + "|" + String.Join ("", stringArray);
	}
	...
	{
		if (Record.HasKey (key)) {
			var completeString = Record.GetString (key);
			...
			if ((ArrayType)bytes [0] != ArrayType.String) {
				BetaFramework.LoggerHelper.Error (key + " is not a string array");
				return new String[0];
			}
			Initialize ();
	...
	public static String[] TransStringArray (String progress)
	{
		if (string.IsNullOrEmpty(progress))
		{
			return new String[0];
		}
			var separatorIndex = progress.IndexOf ("|" [0]);
			if (separatorIndex < 4) {
				return new String[0];
			}
			var bytes = System.Convert.FromBase64String (progress.Substring (0, separatorIndex));
			if ((ArrayType)bytes [0] != ArrayType.String) {
				return new String[0];
			}
			Initialize ();
			...
			for (var i = 0; i < numberOfEntries; i++) {
				int stringLength = bytes [idx++];
				if (stringIndex + stringLength > progress.Length) {
					return new String[0];
				}
				stringArray [i] = progress.Substring (stringIndex, stringLength);
				stringIndex += stringLength;
			}
	...
	{
		if (array.Count == 0) {
			BetaFramework.LoggerHelper.Error ("The " + arrayType.ToString () + " array cannot have 0 entries when setting " + key);
			return false;
		}
	...
	private static void GetValue<T> (String key, T list, ArrayType arrayType, int vectorNumber, Action<T, byte[]> convert) where T : IList
	{
		if (Record.HasKey (key)) {
			...
			if ((ArrayType)bytes [0] != arrayType) {
				BetaFramework.LoggerHelper.Error (key + " is not a " + arrayType.ToString () + " array");
				return;
			}
			Initialize ();
	...
	public static void ShowArrayType (String key)
	{
		...
		if (bytes.Length > 0) {
			...
			BetaFramework.LoggerHelper.Log (key + " is a " + arrayType.ToString () + " array");
		}
	}
	...
	{
		try		{
			Record.SetString (key, System.Convert.ToBase64String (bytes));
		} catch {
			return false;
		}
		return true;

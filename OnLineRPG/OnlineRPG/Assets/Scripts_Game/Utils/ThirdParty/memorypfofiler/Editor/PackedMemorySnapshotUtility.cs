...
public static class PackedMemorySnapshotUtility
{
    ...
    static void SaveToFile(string filePath, PackedMemorySnapshot snapshot)
    {
        ...
        BetaFramework.LoggerHelper.LogFormat("Saving...");
        ...
        stopwatch.Stop();
        Profiler.EndSample();
        BetaFramework.LoggerHelper.LogFormat("Saving took {0}ms", stopwatch.ElapsedMilliseconds);
    }
    ...
    public static PackedMemorySnapshot LoadFromFile()
    {
        ...
        previousDirectory = Path.GetDirectoryName(filePath);
        BetaFramework.LoggerHelper.LogFormat("Loading \"{0}\"", filePath);
        var packedSnapshot = LoadFromFile(filePath);
        BetaFramework.LoggerHelper.LogFormat("Completed loading \"{0}\"", filePath);
        return packedSnapshot;
    }

    static PackedMemorySnapshot LoadFromFile(string filePath)
    {
        BetaFramework.LoggerHelper.LogFormat("Loading...");
        System.Diagnostics.Stopwatch stopwatch = new System.Diagnostics.Stopwatch();
        ...
        {
            BetaFramework.LoggerHelper.ErrorFormat("MemoryProfiler: Unrecognized memory snapshot format '{0}'.", filePath);
            result = null;
        }

        BetaFramework.LoggerHelper.LogFormat("Loading took {0}ms", stopwatch.ElapsedMilliseconds);
        return result;
    }

    private static object getSerializer(List<string> errors) {
    ...
    private static void logErrors(List<string> errors) {
        if (0 < errors.Count) {
            ...
            BetaFramework.LoggerHelper.Log(sb.ToString());
        }
    }

    private sealed class MyContractResolver
    {
        public bool IgnoreSerializableAttribute = false;
    ...
    private class FilteringTraceLogWriter  {
        ...
        public void Trace(System.Diagnostics.TraceLevel level, string message, Exception ex) {
            ...
            Debug.unityLogger.Log(logTypeFromTraceLevel[(int) level], message);
            if (null != ex) {

...
public static class Language
{
	...
	public static string settingsAssetPath = "Assets/ThirdParty/Localization/Resources/Languages/LocalizationSettings.asset";
	public static LocalizationSettings settings
	{
		get
		{
			...
			//	string settingsFile = "Languages/" + System.IO.Path.GetFileNameWithoutExtension(settingsAssetPath);
			    ...
			    _settings = Resources.Load<LocalizationSettings>("Languages/LocalizationSettings");
            }
            if (_settings == null)
                BetaFramework.LoggerHelper.Log("huhuhuhuhu");
			return _settings;
		}
	}
	...
	static Language()
	{
	    ...
	    string lastLang = _settings.defaultLangCode.ToString();
    ...
    public static void SwitchLanguage()
    {
        ...
        LoadAvailableLanguages();
        bool useSystemLanguagePerDefault = settings.useSystemLanguagePerDefault;
        LanguageCode useLang = LocalizationSettings.GetLanguageEnum(settings.defaultLangCode.ToString());    //ISO 639-1 (two characters). See: http://en.wikipedia.org/wiki/List_of_ISO_639-1_codes
    ...
    static void LoadAvailableLanguages()
	{
		...
		{
			BetaFramework.LoggerHelper.Log("None available");
			return;
			
		}
        Array arry = Enum.GetValues(typeof (LanguageCode));
	...
	public static bool SwitchLanguage(LanguageCode code)
	{
		...
		{
			BetaFramework.LoggerHelper.Error("Could not switch from language " + currentLanguage + " to " + code);
			if (currentLanguage == LanguageCode.N)
			{
				if (availableLanguages.Count > 0)
				{
					DoSwitch(LocalizationSettings.GetLanguageEnum(availableLanguages[0]));
					BetaFramework.LoggerHelper.Error("Switched to " + currentLanguage + " instead");
				}
				else
				{
					BetaFramework.LoggerHelper.Error("Please verify that you have the file: Resources/Languages/" + code + "");
					Debug.Break();
				}
			}
	...
	{
#if WordCrossyDE
        newLang = LanguageCode.DE;
#else
        newLang = LanguageCode.EN;
#endif
	...
	{
        string str = Get(key, settings.sheetTitles[0]);
        if (str is string){
            str = str.Replace("\\n", "\n");
        }
        return str;
	...
	public static string Get(string key, string sheetTitle)
	{
		...
		{
			BetaFramework.LoggerHelper.Error("The sheet with title \""+sheetTitle+"\" does not exist!");
			return "";
		}
		...
		{
			BetaFramework.LoggerHelper.Error("MISSING LANG:" + key);
			return "#!#"+ key+"#!#";
		}
	}
    ...
    {
		if (parameters != null && parameters.Length > 1) BetaFramework.LoggerHelper.Error("We cannot pass more than one argument currently!");

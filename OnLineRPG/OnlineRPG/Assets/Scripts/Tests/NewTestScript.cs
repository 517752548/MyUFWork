...
using Random = UnityEngine.Random;
 ...
 namespace Tests
 {
     ...
     { 
        ...
        [Test]
        public void TestDepartDay() {
            System.DateTime date1 = DateTime.MinValue;
            System.DateTime date2 = DateTime.Now;
            date1 = System.Convert.ToDateTime("2020-02-11 22:17");
            date2 = System.Convert.ToDateTime("2019-02-11 22:17");
            date1 = System.Convert.ToDateTime(date1.ToShortDateString());
            TimeSpan ts = date2 -date1;
            Assert.AreEqual(ts.Days, 1, "相差 {0}天,{1}", ts.Days,date1);
        }

        [Test]
        public void TestIsDateAnotherBigDay()
        {
            System.DateTime date1 = DateTime.MinValue;
            System.DateTime date2 = DateTime.Now;
            
            Assert.AreEqual(IsDateAnotherBigDay(date1, date2), true);
            Assert.AreEqual(IsDateAnotherBigDay(date2, date1), false);
            date1 = System.Convert.ToDateTime("2019-06-05 1:17");
            Assert.AreEqual(IsDateAnotherBigDay(date1, date2), true);
            date1 = System.Convert.ToDateTime("2020-02-11 1:17");
            Assert.AreEqual(IsDateAnotherBigDay(date1, date2), false);
            date1 = System.Convert.ToDateTime("2020-02-12 1:17");
            Assert.AreEqual(IsDateAnotherBigDay(date1, date2), false);
            date1 = System.Convert.ToDateTime("2020-02-10 1:17");
            Assert.AreEqual(IsDateAnotherBigDay(date1, date2), true);
            //Assert.AreEqual(IsDateAnotherBigDay(date1, date2), false,"date1 is {0}",date1);
        }
		...
		public void GetRange()
		{
			...
			Assert.AreEqual(a, b);
			Assert.AreEqual(a.GetRange(0, 1), new List<int>() { 1 });
			Assert.AreEqual(a.GetRange(1, 2), new List<int>() { 2, 3 });
			Assert.Throws<System.ArgumentException>(() => a.GetRange(1, 4));
		}

...
/**
 * 支持任意长度消息的编写，只要消息支持setBuffer和write方法, 源自天书mmo_core的同名类
 * 
 * 
 */
public class GameEncoder implements ProtocolEncoder {
	...
	private void encodeCommonMessage(Object msg, ProtocolEncoderOutput out) throws MessageParseException {
		...
		IoBuffer buf = IoBuffer.allocate(m.getInitBufferLength());
...
//			String a = CommonUtil.Bytes2HexString(bytes);
//			Loggers.msgLogger.debug("msg hex:" + a);
			buf.flip();
			if (success) {
				// 如果成功，保留缓存区，写入out
				if (buf.hasRemaining()) {
...
//				buf.release();
	...
	// IoBuffer _clone = IoBuffer.wrap(_cloneData, _offset, _length);
	// // 如果成功，保留缓存区，写入out

...
USE `tr_${server_id}`;
update t_db_version set version='1.0.0.43',updateTime=now();
...
if exists (select * from information_schema.columns where table_schema='tr_${server_id}' and table_name = 't_card_activity' and column_name = 'serverIds') then  
        alter table tr_${server_id}.t_card_activity drop column serverIds;
end if; 
alter table tr_${server_id}.t_card_activity add column serverIds text  ;

if exists (select * from information_schema.columns where table_schema='tr_${server_id}' and table_name = 't_good_activity' and column_name = 'serverIds') then  
        alter table tr_${server_id}.t_good_activity drop column serverIds;
end if; 
alter table tr_${server_id}.t_good_activity add column serverIds text  ;

if exists (select * from information_schema.columns where table_schema='tr_${server_id}' and table_name = 't_turntable_activity' and column_name = 'serverIds') then  
        alter table tr_${server_id}.t_turntable_activity drop column serverIds;
end if; 
alter table tr_${server_id}.t_turntable_activity add column serverIds text  ;

if exists (select * from information_schema.columns where table_schema='tr_${server_id}' and table_name = 't_user_snap' and column_name = 'serverId') then  
        alter table tr_${server_id}.t_user_snap drop column serverId;
end if; 
alter table tr_${server_id}.t_user_snap add column serverId int(11) NOT NULL DEFAULT 0;
...
update t_card_activity set serverIds=(select serverIds from t_db_version);
update t_good_activity set serverIds=(select serverIds from t_db_version);
update t_turntable_activity set serverIds=(select serverIds from t_db_version);
	

update t_user_snap,t_character_info set t_user_snap.serverId=t_character_info.serverId where t_user_snap.id=t_character_info.id;

...
USE `tr_${server_id}`;
update t_db_version set version='1.0.0.37',updateTime=now();
...
if exists (select * from information_schema.columns where table_schema='tr_${server_id}' and table_name = 't_character_info' and column_name = 'zuanshibi') then  
        alter table tr_${server_id}.t_character_info drop column zuanshibi;
end if; 
alter table tr_${server_id}.t_character_info add column zuanshibi bigint(20) NOT NULL DEFAULT 0;

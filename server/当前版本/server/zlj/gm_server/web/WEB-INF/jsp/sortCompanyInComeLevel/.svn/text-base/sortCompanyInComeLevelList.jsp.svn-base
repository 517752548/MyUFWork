...
function search(){
	...
	window.location.href="sortCompanyInComeLevel.do?action=init&searchType="+searchType+"&searchValue="+searchValue+"&startLevel="+startLevel+"&endLevel="+endLevel+"&startIndexSort="+startIndexSort+"&endIndexSort="+endIndexSort;
}
function goTo(i){
	var searchType=$("#searchType").val();
    var searchValue=$("#searchValue").val();
...
function kickOut(id){
	...
	if(con){
		$.post("sortCompanyInComeLevel.do?action=kickOut",{"id":"'"+id+"'"},function(info){
			var info = $.trim(info);
			if(info=="ok"){
              alert("<%=lang.readGm(GMLangConstants.CMDSUCCESS)%>");
              window.location="sortCompanyInComeLevel.do?action=init";
			}else{
...
<input id="batchSearch" type="button" class="butcom" style="margin-left: 1em;" value="<%= lang.readGm(GMLangConstants.BATCH_SEARCH_LASTLOGIN)%>" onclick="javaScript:window.location='sortCompanyInComeLevel.do?action=batchSearchInit'">
</div>
...
<table class="detail"  cellspacing="0" cellpadding="0"
  ...
  <tbody><tr>
    ...
    <th>&nbsp;<%= lang.readGm(GMLangConstants.SECROTARENALEVEL_UPORDOWN)%></th>
     <th>&nbsp;<%= lang.readGm(GMLangConstants.SECROTEVEL_COMPANYINCOME)%></th>

  </tr>
  <c:forEach items="${sortCompanyInComeLevelList}" var="sortCompanyInComeLevel">
      <tr>
       <!--  <td>&nbsp;${sortArenaLevel.arenaLevel}</td>-->  
        <td>&nbsp;${sortCompanyInComeLevel.indexSort}</td>
        <td><a 	href="user.do?action=init&searchType=userId&searchValue=${sortCompanyInComeLevel.accountId}"class="link">&nbsp;${sortCompanyInComeLevel.accountId}</a></td>
        
        <td><a href="role.do?action=init&searchType=roleId&searchValue=${sortCompanyInComeLevel.roleId}" class="link">&nbsp;${sortCompanyInComeLevel.roleId} </a></td>
         <td>&nbsp;${sortCompanyInComeLevel.name}</td>
        <td>&nbsp;${sortCompanyInComeLevel.level}</td>
        
        <td>&nbsp;${sortCompanyInComeLevel.guildPack}</td>
         <td>&nbsp;${sortCompanyInComeLevel.upOrDown}</td>
          <td>&nbsp;${sortCompanyInComeLevel.companyIncome}</td>
      </tr>
 	</c:forEach>
  <tr>

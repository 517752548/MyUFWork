...
public class GMAliasToBeanResultTransformer implements ResultTransformer {
	...
	@SuppressWarnings("unchecked")
	public GMAliasToBeanResultTransformer(Class resultClass) {
		if ( resultClass == null ) {
		...
		try {
			if ( setters == null ) {
				...
				for ( int i = 0; i < aliases.length; i++ ) {
					...
					if ( alias != null ) {
						setters[i] = propertyAccessor.getSetter( resultClass, LogTypeUtils.getPropName(resultClass,alias));
					}
				}
			}

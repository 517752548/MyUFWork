...
public class BattleCalculateHelper {
	...
	public static List<ReportItem> onAttackEnemy(Context context, Action action, IEffect effect, FightUnit owner, FightUnit target, int damageHp, ReportItem r, RealDamage realDamageHp) {
		...
		//减血时的处理
		List<ReportItem> ri = onReduceHp(context, owner, target, damageHp, r, realDamageHp, effect);
	 ...
	 * @return
	 */
	public static List<ReportItem> onReduceHp(Context context, FightUnit owner, FightUnit target, int damageHp, ReportItem r, RealDamage realDamageHp, IEffect effect) {
		List<ReportItem> ret = new ArrayList<ReportItem>();
		int realDamage = damageHp;
		List<IEffect> eList = target.getBuffEffectByCatalog(BuffCatalog.HURT_SHIELD);
		List<IEffect> eListValue = target.getBuffEffectByCatalog(BuffCatalog.HURT_SHIELD_WITH_VALUE);
		List<IEffect> eListAttack = target.getBuffEffectByCatalog(BuffCatalog.HURT_SHIELD_ATTACK);
		
		List<IEffect> finalEList = null;
		//伤害吸收盾只允许有一个,已经在buff互斥组那里做了,所以这边直接用
		if(!eList.isEmpty()){
			finalEList = eList;
		}
		if(!eListValue.isEmpty()){
			finalEList = eListValue;
		}
		if(!eListAttack.isEmpty()){
			finalEList = eListAttack;
		}
		
		//伤害吸收盾的处理
		if (damageHp < 0 ){
			if(finalEList != null && !finalEList.isEmpty()){
				int damageHpTmp = 0;
				for (IEffect e : finalEList) {
					...
					if (left <= 0) {
						...
						//反击给攻击方
						if(e instanceof HurtShieldBuffEffectAttack){
							HurtShieldBuffEffectAttack hsbea = (HurtShieldBuffEffectAttack)e;
							int attackValue = - (int) (hsbea.getInitValue() * 
									EffectHelper.int2Double(hsbe.getEffectTpl().getExtraCoef4()));
							owner.updateAttr(BattleDef.HP, attackValue);
							ReportItem ownerItem = ReportItem.valueOf(owner, effect);
							ownerItem.updateAttr(BattleReportDefine.REPORT_ITEM_HP, attackValue);
							ret.add(ownerItem);
						}
					}
				}
    ...
    public static float calcAPropByGrowth(int level, double growth) {
        //战斗实力*属性成长资质/3
        double ret = calcZDSL(level) * growth / Globals.getGameConstants().getBattleAProp1();
        return (float) ret;
    }

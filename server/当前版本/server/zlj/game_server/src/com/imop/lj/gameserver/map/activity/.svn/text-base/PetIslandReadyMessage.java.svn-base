...
public class PetIslandReadyMessage extends AbstractActivityMessage {
	
	private ActivityTemplate template;
	
	public PetIslandReadyMessage(ActivityTemplate template){
	...
	public void execute() {
		...
		if(activityService.setActivityState(activityId, ActivityState.READY)) {
			// 准备阶段的处理
			Globals.getPetIslandService().handleActivityReadyMsg(activityService.getActivity(activityId));
		}
	}

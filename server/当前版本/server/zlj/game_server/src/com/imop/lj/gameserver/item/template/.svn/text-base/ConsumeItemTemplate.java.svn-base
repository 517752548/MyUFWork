...
public class ConsumeItemTemplate extends ConsumeItemTemplateVO {
	...
	public void check() throws TemplateConfigException {
		// 给货币的不能给元宝或绑定元宝
		if (ConsumableFunc.valueOf(this.functionId) == ConsumableFunc.GIVE_MONEY) {
...
//		
		...
		//灵犀仙葫道具，使用函数必须是礼包
		if (this.getItemType() == ItemType.XIANHU_LINGXI){
			if (ItemDef.ConsumableFunc.valueOf(this.functionId) != ConsumableFunc.GIFE_PECK) {
				throw new TemplateConfigException(this.getSheetName(), this.getFunctionId(), 0, "使用后触发函数错误！");
			}
		}

		//使用对象是否非法
		if(this.useTargetId > 0){
			if(ItemDef.UseTarget.valueOf(this.useTargetId) == null){
				throw new TemplateConfigException(this.getSheetName(), this.useTargetId, 0, "使用对象是否非法！");
			}
		}
	}

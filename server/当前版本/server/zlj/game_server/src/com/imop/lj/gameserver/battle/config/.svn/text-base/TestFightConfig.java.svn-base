 ...
 * 
 */
public class TestFightConfig implements IFightConfig {
	/** 最大回合数 */
	...
	public int getMaxRound() {
		return TestFightConfig.MAX_ROUND;
	}
	...
	public IBattle buildBattle(List<FightUnit> attackers, List<IEffect> attackerEffects, List<FightUnit> defenders,
		...
		Battle result = new Battle(this);
		result.initialize(attackers, attackerEffects, defenders, defenderEffects, this);
		return result;
	}
	...
	public void adjustResult(IBattle battle) {
		...
		if (brr.getBattleResult().equals(BattleResult.TIE)) {

...
public class CorpsWarReadyMessage extends AbstractActivityMessage {
	
	private ActivityTemplate template;
	
	public CorpsWarReadyMessage(ActivityTemplate template){
	...
	public void execute() {
		...
		if(activityService.setActivityState(activityId, ActivityState.READY)) {
			// 准备阶段的处理
			Globals.getCorpsWarService().handleActivityReadyMsg(activityService.getActivity(activityId));
		}
	}

package com.imop.lj.core.msg.property;
 ...
 * value支持九种类型：byte, short, int, long, float, double, byte[], string, PropertyBag
 * changed by yanyue at 2009/10/14 value增加一种类型PropertyList
...
public class PropertyBag implements Serializable, IPropertyBagable {
	...
	public static class Value<T>{
		...
		public String toString(){
			return String.format("{%s[%d]:%s}", type.name(),
					size, valueStr());
		}
	 ...
	 * @return
	 */
	public static PropertyBag readBuffer(IoBuffer buf){
	 ...
	 * @param buf	采用mina的ByteBuffer，因为这个支持自动增长
	 */
	public void writeBuffer(IoBuffer buf){
	...
	public static PropertyBag valueOfByties(byte[] byties)
	{
		...
		return readBuffer(IoBuffer.wrap(byties));
...
//
//		// 测试与ByteBuffer的转化
//		IoBuffer buffer = IoBuffer.allocate(10).order(SharedConstants.BYTE_ORDER);

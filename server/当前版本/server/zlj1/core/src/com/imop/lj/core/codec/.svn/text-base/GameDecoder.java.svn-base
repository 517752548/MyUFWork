...
/**
 * 支持任意尺寸消息，不是通过ByteBuffer的自增长，而是程序根据不足再分配,源自天书mmo_core同名类
 * 
 * 
 */
public class GameDecoder extends ProtocolDecoderAdapter {
	...
	/** 数据处理缓存* */
	private IoBuffer readBuf = IoBuffer.allocate(IMessage.DECODE_MESSAGE_LENGTH);
...
//		String a = CommonUtil.Bytes2HexString(in.array());
//		Loggers.msgLogger.debug(a);
	...
	private void expand(int newCapacity) {
		IoBuffer newBuf = IoBuffer.allocate(newCapacity);

...
public class PlayerCheckNeedGiveRewardMsg extends SysInternalMessage {
	...
	private void checkDoublePointRelated() {
		Globals.getOfflineDataService().checkDoublePointRelated(human);
		//上面给双倍点后,才能发送双倍状态
		Globals.getTowerService().openTowerPanel(human);
	}

 ...
 *
 */
public class MoveTemp2PrimBag extends AbstractMoveItemOperation {
	...
	protected boolean moveImpl(Human user, Item from, Item to) {
		...
		// 不能全放下，一个也不放
		if (primBag.getMaxCanAdd(fromItemTemplate) < from.getOverlap()) {
			//放不下
			user.sendSystemMessage(LangConstants.TEMP_BAG_2_PRIM_BAG_FAIL);
			return false;
		}
		...
		if(tmpl == null){
			Loggers.itemLogger.warn("MoveTemp2PrimBag.moveImpl the template is null");
			return false;
		}
		Inventory inventory = user.getInventory();
		boolean flag = inventory.dropItem(from.getBagType(), fromBagIndex,ItemLogReason.TEMP2PRIM,reasonDetail);
		if(!flag){
			//临时背包如果更新为空肯定有问题
			Loggers.itemLogger.warn("MoveTemp2PrimBag.moveImpl drop item is failed");
			return false;
		}
		
		Collection<Item> updatedPrimBagItems = primBag.add(tmpl, dropItemOoverlap, ItemGenLogReason.TEMP2PRIM, reasonDetail);
		if(updatedPrimBagItems.isEmpty()){
			//主背包如果更新为空肯定有问题
			Loggers.itemLogger.warn("MoveTemp2PrimBag.moveImpl add item is failed");
			return false;
		}
...
//			user.sendMessage(message);
		...
		user.sendSystemMessage(LangConstants.TEMP_BAG_2_PRIM_BAG_SUCCESS,  dropItemOoverlap, tmpl.getName());
		return true;
	}

	private boolean handleCanNotOverlap(Human user, Item from){
		CommonBag primBag = user.getInventory().getPrimBag();
		CommonBag tempBag = user.getInventory().getTempBag();
...
//		user.sendMessage(emptySlot.getUpdateMsgAndResetModify());
	...
	protected boolean isSuiltable(BagType fromBag, BagType toBag) {
		// 从主背包到身上
		if (fromBag == BagType.TEMP && toBag == BagType.PRIM) {
			return true;
		} else {
			return false;
		}

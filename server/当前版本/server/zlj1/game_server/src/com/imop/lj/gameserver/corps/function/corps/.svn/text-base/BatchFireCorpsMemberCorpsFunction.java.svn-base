 ...
 * 
 */
public class BatchFireCorpsMemberCorpsFunction extends AbstractCorpsFunction {
	...
	public String isAvailable(Human human, Long target) {
		if(Globals.getCorpsService().canUseBatchFireMember(human, target)){
			return null;
		}
	...
	public int getTitleLangId() {
		return LangConstants.BATCH_FIRE_MEMBER;
	}

	...
	public void execute(ISession playerSession, String[] commands) {
		...
		try {
			...
			if (petTpl.getPetType() == PetType.PET) {
				Globals.getPetService().onCatchPet(human, petTpl.getId(), PetLogReason.GM_PET_CATCH_PET);
				player.sendErrorMessage("add pet ok!tplId=" + petTplId);
			} else if (petTpl.getPetType() == PetType.HORSE) {
				Globals.getPetService().onGetPetHorse(human, petTpl.getId(), PetLogReason.GM_PET_HORSE_HIRE);
			}
		} catch (Exception e) {
			e.printStackTrace();

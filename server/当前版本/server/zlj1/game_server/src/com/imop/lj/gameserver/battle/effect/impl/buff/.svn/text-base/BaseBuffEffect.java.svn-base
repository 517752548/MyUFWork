...
public class BaseBuffEffect extends AbstractEffect {
	 ...
	 * @return
	 */
	public List<ReportItem> addTo(FightUnit target, int value) {
		List<ReportItem> result = new ArrayList<ReportItem>();
		...
		if (needRemoveEffect != this && needRemoveEffect instanceof BaseBuffEffect) {
			...
			//加当前这个新的buff
			result.addAll(doAddTo(target, value));
			if (Loggers.battleLogger.isDebugEnabled()) {
				Loggers.battleLogger.debug(getOwner().getIdentifier() + "给" + target.getIdentifier() + "加buffId=" + getEffectTpl().getBuffTypeId());
			}
		} else {
			//自己就是那个需要删掉的buff,就不加了
			if (Loggers.battleLogger.isDebugEnabled()) {
				Loggers.battleLogger.debug(getOwner().getIdentifier() +";自己就是那个需要删掉的buffId= "+ getEffectTpl().getBuffTypeId());
			}
		}

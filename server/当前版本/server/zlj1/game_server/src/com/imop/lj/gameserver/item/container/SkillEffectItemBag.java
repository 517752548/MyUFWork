 ...
 * 
 */
public class SkillEffectItemBag extends CommonBag {

	public SkillEffectItemBag(Human owner, BagType bagType,int capacity) {
	 ...
	 * @return
	 */
	public Collection<Item> add(ItemTemplate template, int count, ItemGenLogReason reason, String detailReason) {
		if (count == 0) {
			return Collections.emptyList();
		}
		// 不能全放下，一个也不放
		if (getMaxCanAdd(template) < count) {
			return Collections.emptyList();
		}
		...
		if (template.canOverlap()) {
			left = addToOverlapSlot(template, left, ItemLogReason.COUNT_ADD, detailReason, genKey, updateList, needSendLog);
		}
		// 还没全放下,就添加到空格中
		if (left > 0) {
			left = addToEmptySlot(template, left, ItemLogReason.COUNT_ADD, detailReason, genKey, updateList, needSendLog);
		}
		// 还没全放下，肯定出错了
		if (left != 0) {
	...
	private int addToEmptySlot(ItemTemplate template, int count, ItemLogReason reason, String reasonDetail, String genKey, List<Item> updateList,
		...
		for (Item item : items) {
			...
			if (item.isEmpty()) { // 只检查空的格子
				// 产生一个新的激活的物品实例
				Item newItem = Globals.getItemService().newActivatedInstance(owner, template, getBagType(), item.getIndex());
				newItem.setModified();
	...
	public Collection<Item> removeItem(int templateId, int count, ItemLogReason reason, String detailReason) {
		...
		int hasCount = getCountByTmpId(templateId);
		// 看看够不够删，要么全删，要么不删
		if (count > hasCount) {
			return Collections.emptyList();
		}
		// 筛选出所有符合要求的item
		List<Item> matchItems = getAllItemByTmpId(templateId);
		Collection<Item> itemColl = directRemove(matchItems, count, reason, detailReason);
		
//		Globals.getEventService().fireEvent(new MainBagRemoveItemEvent(owner, 
//				Globals.getTemplateCacheService().get(templateId, ItemTemplate.class), count));
		// 财务汇报
		Globals.getMoneyReportService().onRemoveItem(this.owner, reason, templateId, count);
		
		//汇报热云
		Globals.getReyunService().reportRemoveItem(this.getOwner().getPlayer(), templateId, count, reason.getReasonText());
		//汇报dataEye
		Globals.getDataEyeService().removeItemLog(getOwner().getPlayer(), templateId, count, reason.getReasonText());
	 ...
	 * @return 删除的所有item
	 */
	private Collection<Item> directRemove(List<Item> items, int count, ItemLogReason reason, String detailReason) {
		...
		for (Item item : items) {
			...
			if (item.getOverlap() >= toDelete) {
				item.changeOverlap(item.getOverlap() - toDelete, reason, detailReason, "", true);
				toDelete = 0;
				updateList.add(item);
			} else {
...
//		this.owner.setPrimBagNum(newCapacity);
	}
}

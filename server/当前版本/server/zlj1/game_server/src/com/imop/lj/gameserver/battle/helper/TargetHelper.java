...
public abstract class TargetHelper {
	...
	private static List<FightUnit> findTargets(Action action, boolean isTargetSelf, boolean isTargetSelect,
		...
		//我方已死亡的
		case OUR_DEAD:
			result.addAll(getTargetOfCollection(false, action, targetPos, rangType, effectType, targetNum, false, false));
			break;
		//我方全部，包含已死亡的
		case OUR_ALL:
			result.addAll(getTargetOfCollection(false, action, targetPos, rangType, effectType, targetNum, false, true));
			break;
		//所有属于我的单位
		case MY_OWN_ALL:
	...
	private static List<FightUnit> getTargetOfCollection(boolean isEnemy, Action action, 
			int targetPos, RangeType rangType, EffectType effectType, int targetNum, boolean isLive, boolean isAll) {
		List<FightUnit> result = new ArrayList<FightUnit>();

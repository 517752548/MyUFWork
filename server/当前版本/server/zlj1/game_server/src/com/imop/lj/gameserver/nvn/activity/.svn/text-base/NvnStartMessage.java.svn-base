...
public class NvnStartMessage extends AbstractActivityMessage {
	
	private ActivityTemplate template;
	
	public NvnStartMessage(ActivityTemplate template){
	...
	public void execute() {
		...
		if(activityService.setActivityState(activityId, ActivityState.OPENING)) {
			// 开始阶段的处理
			Globals.getNvnService().handleActivityStartMsg(activityService.getActivity(activityId));
		}
	}

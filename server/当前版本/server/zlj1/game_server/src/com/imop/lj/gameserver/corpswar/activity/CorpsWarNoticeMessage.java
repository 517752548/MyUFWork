package com.imop.lj.gameserver.corpswar.activity;
...
public class CorpsWarNoticeMessage extends AbstractActivityMessage {
	
	private ActivityTemplate template;
	
	public CorpsWarNoticeMessage(ActivityTemplate template){
	...
	public void execute() {
		...
		if(activityService.setActivityState(activityId, ActivityState.NOT_OPEN)) {
			// 提醒阶段的处理
			Globals.getCorpsWarService().handleActivityNoticeMsg(activityService.getActivity(activityId));
		}
	}

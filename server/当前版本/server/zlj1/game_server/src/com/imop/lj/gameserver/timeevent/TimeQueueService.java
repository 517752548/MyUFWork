...
import com.imop.lj.gameserver.player.msg.SysGiveDoublePointMessage;
...
public class TimeQueueService implements InitializeRequired {
	...
	public void init() {
		...
		// 补充双倍经验点
		SysPowerGiveTimeTemplate sysDoublePointGiveTimeTemplate = tmplService.get(SharedConstants.CONFIG_TEMPLATE_DOUBLE_POINT_ID, SysPowerGiveTimeTemplate.class);
		for (int timeEventId : sysDoublePointGiveTimeTemplate.getTimeEventIds()) {
			addTask(timeEventId, this.new SysGivePDoublePointTask());
		}
...
//		}
	 ...
	 *
	 */
	public class SysGivePDoublePointTask implements Runnable{
		...
		public void run() {
			...
			long start=Globals.getTimeService().now();
			for (Player player : onlinePlayerService.getOnlinePlayers()) {
				player.putMessage(new SysGiveDoublePointMessage(player.getRoleUUID()));
			}
			long end=Globals.getTimeService().now();
			// 记录日志
			if (Loggers.timeEventTaskLogger.isInfoEnabled()) {
				String content = "任务：{0}:执行所需时间:{1}ms";
				Loggers.timeEventTaskLogger.warn(MessageFormat.format(content,"SysGivePDoublePointTask",end-start+""));
			}
		}

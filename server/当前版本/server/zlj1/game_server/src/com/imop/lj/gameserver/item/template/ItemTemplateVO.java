...
public abstract class ItemTemplateVO extends TemplateObject {
	...
	/** 获取途径列表 */
	@ExcelCollectionMapping(clazz = com.imop.lj.gameserver.item.template.ItemGetWayTemplate.class, collectionNumber = "24,25,26;27,28,29;30,31,32")
	...
	public void setIdentityTypeId(int identityTypeId) {
		if (identityTypeId == 0) {
			throw new TemplateConfigException(this.getSheetName(), this.getId(),
					7, "[道具身份类型,对应ItemDef.IdentityType]identityTypeId不可以为0");
		}
	...
	public void setItemTypeId(int itemTypeId) {
		if (itemTypeId == 0) {
			throw new TemplateConfigException(this.getSheetName(), this.getId(),
					8, "[道具类型,对应ItemDef.ItemType]itemTypeId不可以为0");
		}
	...
	public void setBagId(int bagId) {
		if (bagId == 0) {
			throw new TemplateConfigException(this.getSheetName(), this.getId(),
					9, "[包裹标示,增加道具时将被放在哪种包裹中]bagId不可以为0");
		}
		this.bagId = bagId;
	}
	...
	public void setSellCurrencyId(int sellCurrencyId) {
		if (sellCurrencyId < 0) {
			throw new TemplateConfigException(this.getSheetName(), this.getId(),
					17, "[出售给商店的货币类型]sellCurrencyId的值不得小于0");
		}
	...
	public void setExpiredHour(int expiredHour) {
		if (expiredHour < 0) {
			throw new TemplateConfigException(this.getSheetName(), this.getId(),
					34, "[时效时长,以小时为单位]expiredHour的值不得小于0");
		}
	...
	public String toString() {
		return "ItemTemplateVO[nameLangId=" + nameLangId + ",name=" + name + ",descLangId=" + descLangId + ",desc=" + desc + ",icon=" + icon + ",identityTypeId=" + identityTypeId + ",itemTypeId=" + itemTypeId + ",bagId=" + bagId + ",rarityId=" + rarityId + ",level=" + level + ",maxOverlap=" + maxOverlap + ",pageId=" + pageId + ",orderId=" + orderId + ",canUsed=" + canUsed + ",canSelled=" + canSelled + ",sellCurrencyId=" + sellCurrencyId + ",sellPrice=" + sellPrice + ",modelId=" + modelId + ",canCompose=" + canCompose + ",listingFeeType=" + listingFeeType + ",listingFee=" + listingFee + ",tradeBasePriceType=" + tradeBasePriceType + ",tradeBasePrice=" + tradeBasePrice + ",wayList=" + wayList + ",expiredHour=" + expiredHour + ",]";

...
/**
 * 关卡低于世界等级的经验奖励【个人类型】，对应奖励类型为【关卡奖励】和【关卡宝箱】
 * 
 */
public class MissionWorldLevelRewardAmend extends AbstractRewardAmend {

	public MissionWorldLevelRewardAmend() {
		super(RewardAddedFromType.FROM_WORLD_LEVEL);
	}
...
//		}
		...
		int level = player.getHuman().getLevel();
		int worldLevel = 20;//FIXME Globals.getArenaService().getWorldLevel();
		// 低于世界等级时，才有加成
		if (level < worldLevel) {
			Map<RewardAddedType, Integer> map = new HashMap<RewardAddedType, Integer>();
			map.put(RewardAddedType.EXP, Globals.getGameConstants().getMissionWorldLevelExpAdd());
			return map;
		}

...
public class ArenaService implements InitializeRequired {
	...
	public void onBattleEnd(long humanId, ArenaOpponent targetOp, BattleProcess bp) {
		...
		if (Globals.getTeamService().isPlayerOnline(humanId)) {
			human = Globals.getOnlinePlayerService().getPlayer(humanId).getHuman();
			//给玩家发战报，需要先发战报，否则后边给奖励的冒泡就先冒了
			Globals.getBattleReportService().sendBattleReportMsg(human, bp.getFinalReport(), reportId, false, false, bp.getBattleType().getIndex(), "");
...
//			}
	...
	public void buyChallengeTimes(Human human) {
		...
		if (!human.costMoney(needBond, Currency.BOND, false, 0, MoneyLogReason.ARENA_BUY_TIMES, detailReason, 0)) {
	...
	public void killCdTime(Human human) {
		...
		//扣钱
		boolean flag = human.costMoney(cost, Currency.GOLD, false, 0, 

...
//			role.updateFightPower();
	...
	public void updateProperty(int effectMask) {
		...
		//是否满值
		boolean hpFull = hp >= (int)getBProperty(PetBProperty.HP);
		boolean mpFull = mp >= (int)getBProperty(PetBProperty.MP);
		boolean lifeFull = life >= (int)getBProperty(PetBProperty.LIFE);

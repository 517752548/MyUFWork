...
public class TowerMsgBuilder {
	...
	private static TowerInfo createTowerInfo(int curTowerLevel, long curDoublePoint, int isOpen){
		TowerInfo info = new TowerInfo();
		info.setCurTowerLevel(curTowerLevel);
		info.setCurDoublePoint(curDoublePoint);
		info.setOpenDouble(isOpen);
		return info;
	}
	...
	public static GCTowerInfo createGCTowerInfo(Human human){
		...
		msg.setTowerInfo(createTowerInfo(towerManager.getCurTowerLevel(),
				offlineData.getCurDoublePoint(), offlineData.getIsOpenDouble()));
		return msg;
	} 

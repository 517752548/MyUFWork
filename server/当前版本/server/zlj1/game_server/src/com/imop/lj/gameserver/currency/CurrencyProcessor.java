...
//				return false;
	...
	public boolean costMoney(Human human, final long amount, final Currency mainCurrency, final Currency altCurrency, final Currency thirdCurrency, boolean needNotify,
		...
		if (needNotify) {
			...
			// gcSystemMessage.setColor(GameColor.SYSMSG_COMMON.getRgb());
			// gcSystemMessage.setShowType(SysMsgShowTypes.generic);
			// String content =
			// Globals.getLangService().readSysLang(LangConstants.COST_REASON,
			// amount,
			// Globals.getLangService().readSysLang(mainCurrency.getNameKey()),
			// Globals.getLangService().readSysLang(usageLangId));
			// gcSystemMessage.setContent(content);
			// human.sendMessage(gcSystemMessage);
		}

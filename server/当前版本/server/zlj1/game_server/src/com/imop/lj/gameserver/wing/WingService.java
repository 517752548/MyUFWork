...
public class WingService {
	...
	public void openWingPanel(Human human) {
		...
		if(wList == null || wList.isEmpty()){
			Loggers.wingLogger.warn("#WingService#openWingPanel player's wing is null!humanId=" + human.getCharId());
		}
	...
	public List<Wing> getPlayerWingList(Human human) {
		...
		if(human != null && human.getWingManager() != null){
			w = human.getWingManager().getAllWingList();
		}
		return w;
	}
	...
	public void upgradeWing(Human human, Wing w,Integer templateId,Integer upgradeType) {
		...
		//3.扣除道具和金钱
		Collection<Item> symbolList =  human.getInventory().removeItem(upgradeTpl.getItemId(), upgradeTpl.getItemNum(),LogReasons.ItemLogReason.WING_UPGRADE_COST, LogUtils.genReasonText(LogReasons.ItemLogReason.WING_UPGRADE_COST));

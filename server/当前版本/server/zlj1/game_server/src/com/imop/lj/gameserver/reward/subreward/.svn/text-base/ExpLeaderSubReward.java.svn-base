 ...
 * 
 */
public class ExpLeaderSubReward extends AbstractExpSubReward {
	
	public ExpLeaderSubReward(Reward reward) {
		super(reward, RewardType.REWARD_LEADER_EXP);
	}
	...
	public static RewardParam createRewardParam(int exp){
		...
		param.setRewardType(RewardType.REWARD_LEADER_EXP);
		param.setParam2(exp);
		return param;
	}
...
//		Globals.getPetService().addExpForAllPet(human, exp, reward.getReasonType().getPetExpLogReason(), reward.getParams(), needNotify);
//		Globals.getCorpsService().addExpByMission(human, exp, reward.getReasonType().getPetExpLogReason(), reward.getParams(), needNotify);
		Globals.getPetService().addExpForLeader(human, exp, reward.getReasonType().getPetExpLogReason(), reward.getParams(), needNotify);
		return true;
	}

	@Override
	public ISubReward createEmptySubReward(Reward reward) {
		return new ExpLeaderSubReward(reward);
	}

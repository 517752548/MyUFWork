...
public class BattleTemplateCache implements InitializeRequired {
	 ...
	 * @return
	 */
	public List<IEffect> getSkillEffectsNoMind(int skillId, int skillLevel) {
		return getSkillEffects(skillId, skillLevel, 0, 0);
	}
	 ...
	 * @return
	 */
	public List<IEffect> getSkillEffects(int skillId, int skillLevel, int mindId, int mindLevel) {
		...
		if (eList != null && !eList.isEmpty()) {
			effects = EffectFactory.createSkillEffect(eList, skillId, skillLevel);
		}
		return effects;
	}

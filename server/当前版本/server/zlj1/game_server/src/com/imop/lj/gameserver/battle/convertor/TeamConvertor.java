	...
	public List<FightUnit> convert(Object param, boolean isAttacker, BattleType type) {
		...
		for (TeamMember tm : team.getMemberList()) {
			...
			if (Globals.getTeamService().isOnlineNow(tm)) {
				...
				FightUnit unit = Globals.getFightUnitService().createFightUnit(leader, leaderPos, id, battleTypeTpl, isAttacker);
				list.add(unit);
				...
				if (pet != null) {
					...
					FightUnit petUnit = Globals.getFightUnitService().createFightUnit(pet, petPos, petId, battleTypeTpl, isAttacker);
					list.add(petUnit);
				}
			} else {
				//离线，按照userSnap来创建
				...
				String id = genFighterId(leaderPos, isAttacker);
				FightUnit unit = Globals.getFightUnitService().createFightUnitByPetSnap(userSnap, leader, leaderPos, id, battleTypeTpl, isAttacker);
				list.add(unit);
				...
				if (pet != null) {
					String petId = genFighterId(petPos, isAttacker);
					FightUnit petUnit = Globals.getFightUnitService().createFightUnitByPetSnap(userSnap, pet, petPos, petId, battleTypeTpl, isAttacker);
					list.add(petUnit);
				}
			}
			//主将位置+1

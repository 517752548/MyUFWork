...
//		prop.set(PetBProperty.SP, petTemp.getSp());
		prop.set(PetBProperty.LIFE, petTemp.getLife());
...
//		}
	...
	public static void addProperty(PetBProperty targetProp, Item item, Pet pet) {
		ItemFeature feature = item.getFeature();
		if(!(feature instanceof AbstractAttrFeature)){
			return;
		}
...
//	};
	...
	private static void addPropertyForGem(PetBProperty targetProp, Item item, Pet pet, Position p) {
		...
		KeyValuePair<Integer, Integer> pair = Globals.getTemplateCacheService().getGemTemplateCache().getPropKeyAndValue(p,GemType.valueOf(git.getGemType()), git.getGemLevel());
		if (pair == null) {
			return;
		}

...
public class HibernateMergeDBServcieImpl implements DBService {
	 ...
	 * @param resourceNames
	 *            额外的Hibernate Mapping配置文件(如定义查询的配置,以及单独的mapping文件等),这些资源需要在classpath中可以找到
	 */
	public HibernateMergeDBServcieImpl(URL hibernateCfgXmlUrl, String... resourceNames) {
	...
	@Override
	public List findByNamedQueryAndNamedParam(final String queryName, final String[] paramNames, final Object[] values, final int maxResult, final int start)
				...
				try {
					...
					if (_e != null) {
						if (logger.isErrorEnabled()) {
							logger.error("Trigger event fail", _e);
						}
					}
				}
			}
	...
	@Override
	public void saveAll(final List<BaseEntity> entityList) throws DataAccessException {
		this.transTemplate.doCall(new HibernateCallback<Void>() {
			@Override
			public Void doCall(Session session) {
				for (BaseEntity entity : entityList) {
					try {
						...
						Loggers.mergeDbLogger.error("insert entity is " + entity.getClass().getSimpleName() + " id:" + entity.getId());
						throw new MergeException(entity.getId(), "insert entity is " + entity.getClass().getSimpleName() + " id:" + entity.getId());
					}
				}
				return null;
			}

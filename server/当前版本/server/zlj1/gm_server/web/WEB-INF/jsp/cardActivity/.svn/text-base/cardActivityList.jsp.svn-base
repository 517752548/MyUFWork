...
<%=lang.readGm(GMLangConstants.CARD_ACTIVITY_DESC)%>
	...
	<tbody>
		<tr>
		 ...
		 <th width="10%"><%=lang.readGm(GMLangConstants.MOBILE_ACTIVITY_START_TIME)%></th>
 		 ...
 		 <th width="10%"><%=lang.readGm(GMLangConstants.CLOSE_TIME)%></th>
     ...
     <c:forEach items="${mobileActivityShowData}" var="mobileActivityShow">
        ...
        <c:otherwise>
		 ...
		 <td>&nbsp;${mobileActivityShow.closeDate}&nbsp;${mobileActivityShow.closeTime}</td>
		 <td>&nbsp;
		 ...
		 <td <c:if test="${mobileActivityShow.forceEndOrNot eq 1}">bgcolor="ffff00"</c:if>>&nbsp;
		 	...
		 	<c:if test="${mobileActivityShow.forceEndOrNot eq 0}">
			 	...
			 	<c:when test="${mobileActivityShow.isClosed eq 1}">
			 		${mobileActivityShow.closeDate}&nbsp;${mobileActivityShow.closeTime}
				</c:when>
				<c:otherwise>
		 			<input class="butcom" type="button" value="<%= lang.readGm(GMLangConstants.MOBILE_ACTIVITY_FORCE)%>" 
		 				onClick="javascript:if(window.confirm('确认强制关闭该活动吗？id=${mobileActivityShow.id}'))window.location.href='cardActivity.do?action=updateUseOrNotNotUse&Id=${mobileActivityShow.id}';"/>
		 			&nbsp;&nbsp;&nbsp;&nbsp;
	 				<input class="butcom" type="button" value="<%= lang.readGm(GMLangConstants.COMMON_NOR)+lang.readGm(GMLangConstants.CLOSE)%>" 
		 				onClick="javascript:if(window.confirm('确认正常关闭该活动吗？id=${mobileActivityShow.id}'))window.location.href='cardActivity.do?action=closeCurActivity&Id=${mobileActivityShow.id}';"/>
				</c:otherwise>
				</c:choose>
			</c:if>
...
function insertMobileActivity(){
	window.location.href="cardActivity.do?action=insertMobileActivity";
}
function forceClose(id) {
	
}
function closeCur(){
	  window.location.href="cardActivity.do?action=closeCurActivity";
}
</script>
</body>
</html>
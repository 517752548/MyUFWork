...
function search(){
	...
	window.location.href="battle.do?action=init&searchType="+searchType+"&searchValue="+searchValue+"&startLevel="+startLevel+"&endLevel="+endLevel;
}
function goTo(i){
	var searchType=$("#searchType").val();
    var searchValue=$("#searchValue").val();
...
function kickOut(id){
	...
	if(con){
		$.post("battle.do?action=kickOut",{"id":"'"+id+"'"},function(info){
			var info = $.trim(info);
			if(info=="ok"){
              alert("<%=lang.readGm(GMLangConstants.CMDSUCCESS)%>");
              window.location="battle.do?action=init";
			}else{
...
<ul>
	...
	<li id="man_nav_2"
		onclick="list_sub_nav(id,'<%=lang.readGm(GMLangConstants.BATTLE_INFO_ARRAYID)%>')"
		class="bg_image"><%=lang.readGm(GMLangConstants.BATTLE_INFO_ARRAYID)%></li>   
</ul>
</div>
<div id="sub_info">
<span id="show_text">
...
<input id="batchSearch" type="button" class="butcom" style="margin-left: 1em;" value="<%= lang.readGm(GMLangConstants.BATCH_SEARCH_LASTLOGIN)%>" onclick="javaScript:window.location='battle.do?action=batchSearchInit'">
</div>
...
<table class="detail"  cellspacing="0" cellpadding="0"
  ...
  <tbody><tr>
    ...
    <th>&nbsp;<%= lang.readGm(GMLangConstants.BATTLE_INFO_ID)%></th>
    <!--  <th>&nbsp;<%= lang.readGm(GMLangConstants.BATTLE_INFO_ARMS)%></th>-->
 	<th>&nbsp;<%= lang.readGm(GMLangConstants.BATTLE_INFO_ARRAYID)%></th>
 	<th>&nbsp;<%= lang.readGm(GMLangConstants.BATTLE_INFO_LEVEL)%></th>
 	<th>&nbsp;<%= lang.readGm(GMLangConstants.BATTLE_INFO_NAME)%></th>
 	
  </tr>
  <c:forEach items="${battleSnapList}" var="battle">
      <tr>
		<!-- <td><input type="checkbox" id="${battle.id}" name="users"/></td> -->
        <td><a href="role.do?action=init&searchType=roleId&searchValue=${battle.id}" class="link">&nbsp;${battle.id} </a></td>
          <!-- <td>&nbsp;${battle.armies}</td> -->
         <td>&nbsp;${battle.arrayId}</td>
        <td>&nbsp;${battle.level}</td>
        <td>&nbsp;${battle.name}</td>
      </tr>
 	</c:forEach>

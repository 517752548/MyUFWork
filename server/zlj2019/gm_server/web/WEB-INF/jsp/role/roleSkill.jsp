...
function goTo(i){
	...
	window.location="role.do?action=roleSkill&id=${id}&currentPage="+i+"&tabId="+tabId;
}
</script>
</head>
<body>
...
<ul>
   ...
   <li id="man_nav_6" 
		...
		class="bg_image_onclick"><%=lang.readGm(GMLangConstants.SKILL)%></li>
   <li id="man_nav_7" 
...
<a href="#"><%=lang.readGm(GMLangConstants.FIGHT)%><%=lang.readGm(GMLangConstants.SKILL)%></a></h2>
<h2 id='tab_2' onclick="switchTab(id)" class="tab" >
<a  href="#"><%=lang.readGm(GMLangConstants.LIFE)%><%=lang.readGm(GMLangConstants.SKILL)%></a></h2>
</div>
<div class="nofloat" />
...
<table name='tab_1' class="detail no_bottom"  cellspacing="0" cellpadding="0"
	...
	<tbody>
		<tr>
		 ...
		 <th>&nbsp;<%=lang.readGm(GMLangConstants.SKILL)%><%=lang.readGm(GMLangConstants.COMMON_NAME)%></th>
 		 <th>&nbsp;<%=lang.readGm(GMLangConstants.LEVEL)%></th>
		</tr>
     <c:forEach items="${fightSkills}" var="fightSkill">
        <tr>
		 <td style="width: 350px;">&nbsp;${fightSkill.skillId}</td>
         <td>&nbsp;
		  ${fightSkill.skillName}
         </td>
		 <td>&nbsp;${fightSkill.skillLevel}</td>
		</tr>
     </c:forEach>
   </tbody>
</table>
<table name='tab_2' class="detail no_bottom"  cellspacing="0" cellpadding="0"
	...
	<tbody>
     ...
     <c:forEach items="${lifeSkills}" var="lifeSkill">
        <tr>
		 <td style="width: 350px;">&nbsp;${lifeSkills.skillId}</td>
         <td>&nbsp;
		  ${lifeSkills.skillName}
         </td>
		 <td>&nbsp;${lifeSkills.skillLevel}</td>
		</tr>
     </c:forEach>
   </tbody>
</table>

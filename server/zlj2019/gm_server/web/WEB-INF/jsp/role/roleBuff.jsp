...
function goTo(i){
    window.location="role.do?action=roleBuff&id=${id}&currentPage="+i;
   }
</script>
</head>
<body>
...
<ul>
   ...
   <li id="man_nav_9"
		...
		class="bg_image_onclick"><%=lang.readGm(GMLangConstants.BUFINFO)%></li>
   <li id="man_nav_10"
...
<table name='tab_1' class="detail" width="1100px" cellspacing="0" cellpadding="0"
	...
	<tbody>
		<tr>
		 <th style="width: 350px;">&nbsp;<%=lang.readGm(GMLangConstants.BUFF_ID)%></th>
		 <th style="width: 300px;">&nbsp;<%=lang.readGm(GMLangConstants.TEMPLATENAME)%></th>
 		 <th>&nbsp;<%=lang.readGm(GMLangConstants.OVERDUETIME)%></th>
		</tr>
     <c:forEach items="${buffs}" var="buff">
          <td>&nbsp;<a class="link pointer" href="role.do?action=buffBasicInfo&role_id=${id}&item_id=${buff.id}">${buff.id}</a></td>
          <td style="width: 300px;">&nbsp;
	          <c:forEach items="${xlsData['nbbuffs']}" var="m" >
	             <c:if test="${m.key eq buff.templateId}"><c:out value="${m.value}" /></c:if>
	          </c:forEach>
         </td>
		 <td >&nbsp;${buff.overdueTime}</td>
        </tr>
     </c:forEach>
		<tr>

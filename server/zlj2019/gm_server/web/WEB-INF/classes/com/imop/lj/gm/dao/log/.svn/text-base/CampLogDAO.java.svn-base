...
public class CampLogDAO extends GenericDAO {
	@SuppressWarnings("unchecked")
	public List<CampLog> getCampLogList(final String roleID,final String date,
			final String reason, final String sortType, final String order,
			final long startTimel, final long endTimel) {
		return (List<CampLog>) getTemplate().doCall(
				new HibernateCallback<List>() {
					@Override
					public List doCall(Session session) {
						StringBuffer sql = new StringBuffer();
						sql
								.append("select * from camp_log_" + date
										+ " where 1=1");
						...
						LogTypeUtils.addLogQueryScalar(query, CampLog.class);
						query.setResultTransformer(GMTransformers
								.aliasToBean(CampLog.class));

...
public class NameLookupTable {
	...
	public NameInfo get(NameRefMark type, int id) {
		...
		// 找地图Id最小的
		int minMapId = Integer.MAX_VALUE;
		for (NameInfo info : infoList) {
	 ...
	 * @return
	 */
	public NameInfo get(NameRefMark type, int id, int mapId) {
		List<NameInfo> infoList = getInfoList(type, id);
		for (NameInfo info : infoList) {
			if (mapId == info.getMapId()) {
				return info;
			}
		}
		return null;

...
public abstract class ItemTemplate extends ItemTemplateVO{
	...
	public void setBindTypeId(int bindTypeId) {
		super.setBindTypeId(bindTypeId);
		
		this.bindType = BindType.valueOf(this.bindTypeId);
		if (bindType == null) {
			throw new TemplateConfigException(this.getSheetName(), getId(), String.format("绑定类型%d非法", this.bindTypeId));
		}
	}
...
//	public abstract boolean isHorseEquipment();
	...
	/**
	 * 是否绑定物品，绑定状态为 绑定或拾取绑定，则认为是绑定的
	 * @return
	 */
	public boolean isBind() {
		return isBind(this.bindType);
	}

	public static boolean isBind(BindType bindType) {
		return bindType == BindType.BIND || bindType == BindType.GOT_BIND;
	}

	/**
	 * 是否可合成，这里是指道具通用的合成，与宝石合成没关系
	 * @return
	 */
	public abstract boolean canCompose();

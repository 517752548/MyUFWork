...
//	private boolean isLocked;
	 ...
	 *            堆叠数量
	 */
	private Item(Human owner, long wearerId, ItemTemplate template, BagType bagType, int index, int overlap, boolean isBind) {
	...
	private Item(Human owner, long wearerId, ItemTemplate template, BagType bagType, int index, int overlap, ItemFeature itemFeature, boolean isBind) {
		lifeCycle = new LifeCycleImpl(this);
		this.owner = owner;
		this.wearerId = wearerId;
		this.template = template;
	 ...
	 * @return
	 */
	public static Item newActivatedInstance(Human owner, ItemTemplate template, BagType bagType, int index, boolean isBind) {
		Assert.notNull(owner);
		Assert.notNull(template);
		Item item = new Item(owner, 0, template, bagType, index, 0, isBind);
	 ...
	 * @return
	 */
	public static Item newActivatedInstance(Human owner, ItemTemplate template, BagType bagType, int index, int overlap, boolean isBind) {
		Assert.notNull(owner);
		Assert.notNull(template);
		Item item = new Item(owner, 0, template, bagType, index, overlap, isBind);
		item.getFeature().onCreate();
		item.setUUID(KeyUtil.UUIDKey());
		item.setCreateTime(new Timestamp(Globals.getTimeService().now()));
		item.setDeadline(new Timestamp(TimeUtils.getDeadLine(item.getCreateTime(),
	 ...
	 * @return
	 */
	public static Item newActivatedInstanceForGM(Human owner, ItemTemplate template, BagType bagType, int index, boolean isBind, int[] attrA, int[] attrB, Object...params) {
		Assert.notNull(owner);
		Assert.notNull(template);
		Item item = new Item(owner, 0, template, bagType, index, 0, isBind);
	...
	public static Item newActivatedInstanceWithParams(Human owner, ItemTemplate template, BagType bagType, int index, boolean isBind, int[] attrA, int[] attrB, Object...params) {
		Assert.notNull(owner);
		Assert.notNull(template);
		Item item = new Item(owner, 0, template, bagType, index, 0, isBind);
	...
	public static Item newActivatedInstanceWithFeature(Human owner, ItemTemplate template, BagType bagType, int index, ItemFeature itemFeature, boolean isBind) {
		Assert.notNull(owner);
		Assert.notNull(template);
		Item item = new Item(owner, 0, template, bagType, index, 0, itemFeature, isBind);
		item.setUUID(KeyUtil.UUIDKey());
		item.setCreateTime(new Timestamp(Globals.getTimeService().now()));
		item.setDeadline(new Timestamp(TimeUtils.getDeadLine(item.getCreateTime(),
				template.getExpiredHour(),TimeUtils.HOUR)));
	 ...
	 *             如果template为null
	 */
	public static Item newDeactivedInstance(Human owner, ItemTemplate template, BagType bagType, int index, boolean isBind) {
		Assert.notNull(template);
	...
	public static Item newEmptyOwneredInstance(Human owner, BagType bagType, int index) {
		Assert.notNull(owner);
		return new Item(owner, 0, null, bagType, index, 0, false);
	}
	...
	public static Item buildFromItemEntity(ItemEntity entity, Human owner) {
		...
		Item item = new Item(owner, entity.getWearerId(), template, bagType, entity.getBagIndex(), entity.getOverlap(), 
				entity.getBindFlag() == BindType.BIND.getIndex());

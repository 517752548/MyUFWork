...
//		snap.getHorseSnap().rebuild(human);
	...
	public Map<Long, UserOfflineData> getAllUserOfflineData(){
		return this.allUserOfflineData;
	}
	...
	protected UserPetHorseData buildInitUserPetHorseData(Pet pet) {
		...
		petData.setLastUpdateCloTime(now);
		petData.setLife((long)pet.getPropertyManager().getBProperty(PetBProperty.LIFE));
		//只有初始化的时候赋值,之后不会更改
		petData.setExperience(petData.getTemplate().getLeasehold() > 0);
		petData.setCreateExperTime(now);
		if(petData.isExperience()){
			petData.setDeadline(now + petData.getTemplate().getLeasehold() * 60 * 60 * 1000);
		}else{
			petData.setDeadline(0L);
		}
		return petData;
	}

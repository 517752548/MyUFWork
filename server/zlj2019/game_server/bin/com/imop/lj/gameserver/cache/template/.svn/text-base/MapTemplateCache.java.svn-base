...
public class MapTemplateCache implements InitializeRequired {
	...
	/** 安全区,不可以挂机的地图*/
	private Set<Integer> safeMapSet = new HashSet<Integer>();
	...
	private void initSafeMapSet() {
		for (MapTemplate tpl : templateService.getAll(MapTemplate.class).values()) {
			if(tpl.getMeetMonsterPlanId() == 0){
				safeMapSet.add(tpl.getId());
			}
		}
	}
	...
	public boolean isInSafeMap(int mapId){
		return this.safeMapSet.contains(mapId);
	}
}

...
public class WizardRaidService {
	...
	public void enterRaidSingle(Human human, int raidId) {
		...
		if (costItemFlag) {
			...
			Collection<Item> itemRet = human.getInventory().removeItem(Globals.getGameConstants().getWizardRaidEnterItemId(), 
					...
					ItemLogReason.WIZARD_RAID_SINGLE_ENTER_COST.getReasonText(), true);
	...
	/**
	 * 变南瓜怪，删除之前的怪物，该位置新增加南瓜怪，如果战斗进行中，则不变南瓜了
	 * @param data
	 * @param monster
	 */
...
//		}
	...
	public boolean canEnterRaidTeam(Human human, int raidId) {
		...
		for (TeamMember member : team.getMemberMap().values()) {
			...
			//功能是否开启
			if (!Globals.getFuncService().hasOpenedFunc(memberHuman, FuncTypeEnum.WIZARD_RAID)) {
				team.noticeTeamMemberErrorMsg(LangConstants.WIZARD_RAID_FUNC_NOT_OPEN, member.getName());
				return false;
			}
			//是否有进入次数

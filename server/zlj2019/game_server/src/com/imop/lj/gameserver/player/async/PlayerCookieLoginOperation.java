 ...
 *
 */
public class PlayerCookieLoginOperation implements LocalBindUUIDIoOperation {
	 ...
	 *            玩家角色
	 */
	public PlayerCookieLoginOperation(Player player, String cookieValue,UserAuth userAuth,SysLangService langService) {
		this.player = player;
	...
	public int doIo() {
		...
		authResult = authUserWithCookie(player, cookieValue);
		String userId = userInfo == null ? "" : userInfo.getId();
		Loggers.loginLogger.info("PlayerCookieLoginOperation.doIo() passportId="+userId+" cookieValue="+cookieValue+" timeStart="+timeStart+" timeBlank="+(System.currentTimeMillis()-timeStart));
...
//					+ "isQuickLogin:"+ userInfo.isQuickLogin() + ";"
		...
		String userId = userInfo == null ? "" : userInfo.getId();
		Loggers.loginLogger.info("PlayerCookieLoginOperation.doStop() passportId="+userId+" cookieValue="+cookieValue+" timeStart="+timeStart+" timeBlank="+(System.currentTimeMillis()-timeStart));
	...
	private boolean authUserWithCookie(Player player, String cookieValue) {
		...
		AuthResult result = this.userAuth.auth(player, cookieValue, ip);
	...
	private boolean handleAuthResult(Player player, final AuthResult result) {
		...
		if (!result.success) {
			...
			logger.info("PlayerCookieLoginOperation.handleAuthResult " + "NotifyException" + DisconnectReason.COOKIE_VALID_FAIL.code + " " + result.message) ;
			return false;
		}

...
//		}
	...
	public void handleBuyMsItem(Human human, int msItemId, boolean flag) {
		...
		int buyItemId = tmpl.getItem().getId();
		int buyItemNum = tmpl.getNum();
		
		boolean isBind = false;
		//货币如果指定的绑定类型，则以此为准
		if (currency != null && currency.getItemBindType() != null) {
			isBind = ItemTemplate.isBind(currency.getItemBindType());
		} else {
			//货币没有指定的以道具本身为准
			isBind = Globals.getTemplateCacheService().get(buyItemId, ItemTemplate.class).isBind();
		}
		
		//背包是否有空间
		List<ItemParam> list = new ArrayList<ItemParam>();
		list.add(new ItemParam(buyItemId, buyItemNum, isBind));
		...
		if (null != costDetail) {
			human.getInventory().addItem(buyItemId, buyItemNum, ItemGenLogReason.MS_BUY_ITEM, ItemGenLogReason.MS_BUY_ITEM.getReasonText(), true, true, 
				currency, costDetail.getTotalCost(), costDetail.getActualCost(), isBind);
		} else {
			human.getInventory().addItem(buyItemId, buyItemNum, ItemGenLogReason.MS_BUY_ITEM, ItemGenLogReason.MS_BUY_ITEM.getReasonText(), true, true, 
					currency, cost, cost, isBind);
		}

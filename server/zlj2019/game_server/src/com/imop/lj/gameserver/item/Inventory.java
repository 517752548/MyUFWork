...
//		}
	 ...
	 */
	public Collection<Item> addItem(int templateId, int count, ItemGenLogReason reason, String detailReason, boolean isBind, boolean needNotify) {
		Collection<Item> updateList = this.addItem(templateId, count, reason, detailReason, needNotify, true, Currency.NULL, 0, 0, isBind);
		return updateList;
	}
...
//	}
	 ...
	 *            是否向前端发送消息
	 * 
	 * @param costedCurrency XXX 花钱购买的道具必须传入这个参数，绑定与否是根据这个字段判断的
	 * 
	 * @return 操作后需要跟新的Item列表
	 */
	public Collection<Item> addItem(int templateId, int count, ItemGenLogReason reason, String detailReason, boolean needNotify, boolean sendMsg,
			Currency costedCurrency, long totalCost, long actualCost, boolean isBindOuter) {
		if (count <= 0) {
		...
		//道具绑定状态，外层只有花钱买的才能覆盖内层，否则取外层和内层的交集，即优先还是绑定的
		boolean isBind = tmpl.isBind() || isBindOuter;
		if (costedCurrency != null 
				&& costedCurrency.getItemBindType() != null) {
			//指定货币购买的道具，仅由货币本身决定绑定状态
			isBind = ItemTemplate.isBind(costedCurrency.getItemBindType());
		}
		...
		if (bgType == BagType.PRIM || bgType == BagType.SKILL_EFFECT_BAG) {
			...
			int primCanAdd = bag.getMaxCanAdd(tmpl, isBind);
			int primLeft = count - primCanAdd;
			reportDelNum = primLeft;
			if(primLeft <= 0){
				//如果主背包能放下物品则放进此物品
				updatedItems = bag.add(tmpl, count, reason, detailReason, isBind);
				Loggers.itemLogger.info("玩家放入 " + bgType + " 背包中=" + owner.getUUID() + ":" + owner.getName() + ":" + tmpl.getId() + ":" + count);
			}else{
				updatedItems = bag.add(tmpl, primCanAdd, reason, detailReason, isBind);
			...
			// 检查是否有足够的空间存放，不做提示，提示需要请求者在调用此方法前作
			if (bag.getMaxCanAdd(tmpl, isBind) < count) {
				return Collections.emptyList();
			}
			// 执行添加
			updatedItems = bag.add(tmpl, count, reason, detailReason, isBind);
...
//		if (checkSpace(mergedParams, false)) {
		for (ItemParam param : mergedParams) {
			updateList.addAll(addItem(param.getTemplateId(), param.getCount(), reason, detailReason, param.isBind(), needNotify));
		}
//		}
		this.sendTempBagNotice(needNotify);
		return updateList;
	...
	public int getItemCountByTmplId(int templateId, boolean isBind) {
		CommonBag bag = this.getBagByTemplateId(templateId);
		if (bag != null) {
			return bag.getCountByTmpId(templateId, isBind);
		} else {
			// 不在那三个包中视为没有
			return 0;
		}
	...
	/**
	 * 查询是否有template指定的那些道具，不区别绑定状态<br/>
	 ...
	 * @return
	 */
	public boolean hasItemByTmplId(int templateId, int count, boolean isBind) {
		int gotCount = getItemCountByTmplId(templateId, isBind);
		return gotCount >= count;
	}
...
//	}
	 ...
	 * @param reason
	 * @param detailReason
	 * @param bindFirst true 优先扣除绑定的（一般都为true），false 优先扣除非绑定的
	 * @return
	 */
	public Collection<Item> removeItem(int templateId, int count, ItemLogReason reason, String detailReason, boolean bindFirst) {
		return this.removeItem(templateId, count, reason, detailReason, bindFirst, true);
	}
	 ...
	 * @return
	 */
	public Collection<Item> removeItem(int templateId, int count, ItemLogReason reason, String detailReason, boolean bindFirst, boolean notifyClient) {
		CommonBag bag = this.getBagByTemplateId(templateId);
		if (bag == null) {
			return Collections.emptyList();
		}
		Collection<Item> updateList = bag.removeItem(templateId, count, reason, detailReason, bindFirst);
		if(notifyClient){
			// 通知客户端
	...
	public Collection<Item> removeItem(Collection<ItemParam> params, ItemLogReason reason, String detailReason) {
		...
		for (ItemParam param : merged) {
			deletedList.addAll(removeItem(param.getTemplateId(), param.getCount(), reason, detailReason, param.isBind()));
		}
		return deletedList;
	}
...
//	}
	...
	private EnumMap<BagType, Integer> countNeedSlot(Collection<ItemParam> params) {
		...
		for (ItemParam param : params) {
			...
			// 此包需要的空位
			int needSlot = bag.getNeedSlot(template, count, param.isBind());

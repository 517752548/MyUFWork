 ...
 * 
 */
public class AddExpForOtherPet extends AbstractConsumeOperation {

	@Override
	public ConsumableFunc getConsumableFunc() {
		return ConsumableFunc.OTHER_PET_EXP_CARD;
	}
	...
	protected <T extends Role> boolean useImpl(Human user, Item item, int count, T role) {
		...
		if(Item.isEmpty(item) || role == null){
			return false;
		}
		...
		long diffExp = 0;//Globals.getTemplateCacheService().getPetTemplateCache().getDiffExp(pet, 0);

...
//		boolean isBind = from.isBind();
		...
		// 不能全放下，一个也不放
		if (primBag.getMaxCanAdd(from.getTemplate(), from.isBind()) < itemNum) {
			//放不下
		...
		//像主背包里面添加一个宝石
		Collection<Item> newItems = user.getInventory().addItem(tempId, itemNum, LogReasons.ItemGenLogReason.GEM_TAKE_OFF, 
				LogUtils.genReasonText(LogReasons.ItemGenLogReason.GEM_TAKE_OFF, tempId), from.isBind(), false);

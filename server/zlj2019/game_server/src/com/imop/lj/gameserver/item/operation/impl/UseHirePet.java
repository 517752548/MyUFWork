...
public class UseHirePet extends AbstractConsumeOperation {
	...
	protected <T extends Role> boolean useImpl(Human user, Item item, int count, T role) {
		...
		// 扣除武将卡成功
		Collection<Item> coll = user.getInventory().removeItem(it.getId(), 1, ItemLogReason.PET_HIRE_CARD_COST, 
				ItemLogReason.PET_HIRE_CARD_COST.getReasonText(), isBind);
		if(coll.isEmpty()){
			return false;
		}
		
		// 添加武将
		Globals.getPetService().onCatchPet(user, petTemp.getId(), PetLogReason.PET_HIRE_CARD, isBind);
		return true;
	}

...
namespace DCETRuntime
{
	public class Lua : IDisposable
	{
		...
		private const string luaDir = "Bundles/LuaCode/";
		...
		private static byte[] AssetBundleLoader(ref string filepath)
		{
			if (!string.IsNullOrWhiteSpace(filepath))
			{
				...
				if (splits != null)
				{
					...
					{
						var textAsset = new TextAsset();//AssetBundles.Default.LoadAsset(BundleNameToLower($"{splits[0]}_lua.unity3d"), $"{splits[l - 2]}.{splits[l - 1]}");
					...
					{
						var textAsset = new TextAsset();//AssetBundles.Default.LoadAsset(BundleNameToLower($"{splits[0]}_lua.unity3d"), $"{splits[l - 1]}{luaExtensionName}");

...
import cn.uc.g.sdk.cp.config.Version;
import cn.uc.g.sdk.cp.model.SDKException;
import cn.uc.g.sdk.cp.protocol.CpRequest.Game;
...
public class CpRequestHelper {
    ...
    private static Game cpGame;
    
    static {
        init();
    }
    
    private static String cpId = String.valueOf(Configuration.getCpId());

    private static String gameApiKey = Configuration.getGameApiKey();
    
    //初始化游戏信息
    private static void init() {
        cpGame = new CpRequest().new Game();
        cpGame.setCpId(Configuration.getCpId());
        cpGame.setChannelId(Configuration.getChannelId());
        cpGame.setGameId(Configuration.getGameId());
        cpGame.setServerId(Configuration.getServerId());
        cpGame.setServerName(Configuration.getServerName());
    }
    ...
    public static String createMD5Sign(Map<String, Object> params){
        return createMD5Sign(params, "", gameApiKey);
    }
    ...
    public static String createSignData(Map<String, Object> params, String[] notIn) {
        ...
        if (DEBUG) {
            logger.debug("business data sign:"+result);
        }

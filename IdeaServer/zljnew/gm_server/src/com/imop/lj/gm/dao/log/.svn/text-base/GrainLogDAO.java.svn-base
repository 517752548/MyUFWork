...
public class GrainLogDAO extends GenericDAO {
	@SuppressWarnings("unchecked")
	public List<GrainLog> getGrainLogList(final String roleID,final String date,
			final String reason, final String sortType, final String order,
			final long startTimel, final long endTimel) {
		return (List<GrainLog>) getTemplate().doCall(
				new HibernateCallback<List>() {
					@Override
					public List doCall(Session session) {
						StringBuffer sql = new StringBuffer();
						sql
								.append("select * from grain_log_" + date
										+ " where 1=1");
						...
						LogTypeUtils.addLogQueryScalar(query, GrainLog.class);
						query.setResultTransformer(GMTransformers
								.aliasToBean(GrainLog.class));

...
function goTo(i){
	...
	window.location.href="boss.do?action=init&currentPage="+i+"&searchType="+searchType+"&searchValue="+searchValue+"&userStatus="+userStatus+"&accountType="+accountType;
}
...
function search(){
	...
	var userStatus=$("#userStatus").val();
	var accountType=$("#accountType").val();
	window.location.href="boss.do?action=init&searchType="+searchType+"&searchValue="+searchValue+"&userStatus="+userStatus+"&accountType="+accountType;
}
...
function batchLockUser(){
	...
	window.location='boss.do?action=batchLockUserInit&userIds='+userIds;
}
...
function unBatchLockUser(){
	...
	userIds = userIds.substring(0,userIds.length-1);
	$.post("boss.do?action=unBatchLockUser",{userIds:userIds},function(info){
		var data=$.trim(info);  
		if("true"==data){
			alert("<%=lang.readGm(GMLangConstants.CMDSUCCESS)%>");
			window.location = "boss.do?action=init";
		}else{
...
<input id="batchSearch" type="button" class="butcom" style="margin-left: 1em;" value="<%= lang.readGm(GMLangConstants.BATCH_SEARCH_LASTLOGIN)%>" onclick="javaScript:window.location='boss.do?action=batchSearchInit'">
</div>
...
<table class="detail"  cellspacing="0" cellpadding="0" border="0">
  <tbody><tr>
    ...
    <th>&nbsp;<%= lang.readGm(GMLangConstants.BOSS_ACTIVITYID)%></th>
 	<th>&nbsp;<%= lang.readGm(GMLangConstants.BOSS_BOSSID)%></th>
 	<th>&nbsp;<%= lang.readGm(GMLangConstants.BOSS_LASTUPDATETIME)%></th>	
  </tr>
  <c:forEach items="${bossList}" var="boss">
      <tr>
		<td><input type="checkbox" id="${boss.id}" name="users"/></td>
        <td>&nbsp;${boss.id}</td>
         <td>&nbsp;${boss.activityId}</td>
        <td>&nbsp;${boss.bossId}</td>
        <td>&nbsp;${boss.lastUpdateTime}</td>
      </tr>
 	</c:forEach>
  <tr>

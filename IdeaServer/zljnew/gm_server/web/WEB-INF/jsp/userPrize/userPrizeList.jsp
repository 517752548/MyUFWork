...
<script type="text/javascript">
  ...
  function goTo(i){
	  ...
	  window.location.href="userPrize.do?action=init&currentPage="+i+"&passportId="+passportId+"&reason="+reason+"&startTime="+startTime+"&endTime="+endTime+"&date="+date;	
  }
  function search(){
	  var passportId = $("#passportId").val();
	  var reason=$("#reason").val();
	  ...
	  var startTime=$("#startTime").val();
	  var endTime=$("#endTime").val();
	  window.location.href="userPrize.do?action=init&passportId="+passportId+"&reason="+reason+"&startTime="+startTime+"&endTime="+endTime+"&date="+date;	
  }
  function del(id){
	...
	function delChosen()
	{
		 var con= confirm("<%=lang.readGm(GMLangConstants.CONFRIM_DEL)%>?");
			if(con){
				...
				$.post("userPrize.do?action=delUserPrize",{ids:ids},function(){
			        alert("<%=lang.readGm(GMLangConstants.DEL_SUCCESS)%>");
			        window.location="userPrize.do?action=init";	
				});	
			}
...
<table name='tab_1' class="detail"  cellspacing="0" cellpadding="0"
	...
	<tbody>
		...
		<input type="checkbox" id="allPrize" />
		 ...
		 <th>&nbsp;<%=lang.readGm(GMLangConstants.USER_PRIZE_PARAMS)%></th>
		 <c:if test="${DBType eq 1}">
         	<th>&nbsp;<%=lang.readGm(GMLangConstants.COMMON_OPERATION)%>
			<img  class="pointer" title="<%=lang.readGm(GMLangConstants.NEW_ADD)%>" src="images/add.gif" onclick="javaScript:window.location='userPrize.do?action=addUserPrizeInit'"/>	
		 	&nbsp;<a id="importCSV"  href="userPrize.do?action=csvBatchPrizeInit"><%= lang.readGm(GMLangConstants.IMPORT_CSV)%>"</a>
		 	&nbsp;<%=lang.readGm(GMLangConstants.ADD_PRIZE_ITEM_PARAMS)%>
			<img class="pointer" title="<%=lang.readGm(GMLangConstants.NEW_ADD)%>" src="images/add.gif"
				onclick="javaScript:window.location='userPrize.do?action=addUserPrizeItemWithParamsInit'"/>
			</th>
		</c:if>
		</tr>
     <c:forEach items="${userPrizelist}" var="userPrizeVo">
        ...
        <td>&nbsp;${userPrizeVo.userPrize.params}</td>
		<c:if test="${DBType eq 1}">
			 <td> 

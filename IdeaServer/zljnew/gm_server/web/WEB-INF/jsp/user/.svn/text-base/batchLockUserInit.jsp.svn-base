...
<%= lang.readGm(GMLangConstants.BATCH_LOCK_USERS)%>
</div>

<div id="nav">
    <ul>  
        <li id="man_nav_1"  onclick="list_sub_nav(id)"  class="bg_image_onclick"><%= lang.readGm(GMLangConstants.BATCH_LOCK_USERS)%></li>
     </ul>
</div>
...
<form name="form1" method="post" action='user.do?action=batchLockUsers&userIds=${userIds}'  onsubmit="return is_ok();">
<input name="userId" type="hidden" value="${userInfo.id}">
<table class="detail" style="width:50%;" cellspacing="0" cellpadding="0" border="0" >
  <tbody><tr>
    <th colspan="2"><%= lang.readGm(GMLangConstants.BATCH_LOCK_USERS)%></th>
   </tr>
   <tr>

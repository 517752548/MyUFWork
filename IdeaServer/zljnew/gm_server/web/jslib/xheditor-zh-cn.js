...
var htmlLink='<div>链接地址: <input type="text" id="xhEdtLinkHref" value="event:http://" class="text" /></div><div>打开方式: <select id="xhEdtLinkTarget"><option selected="selected" value="">默认</option><option value="_blank">新窗口</option><option value="_self">当前窗口</option><option value="_parent">父窗口</option></select></div><div style="text-align:right;"><input type="button" id="xhEdtSave" value="确定"/></div>';
...
$.xheditor=function(textarea,options)
{
	...
	this.showLink=function()
	{
		...
		jSave.click(function(){
			_this.focus();
			 if(isIE){
			  _this._exec('Underline');
			 }
			var url=jHref.val();
			var patrn = /^(event:http:\/\/)([\w-]+\.)+[\w-]+([\w-./?%&=]*)?$/;
			if(!patrn.exec(url)){
				alert("超链接地址不对,请重新输入!");
				return false;
			}

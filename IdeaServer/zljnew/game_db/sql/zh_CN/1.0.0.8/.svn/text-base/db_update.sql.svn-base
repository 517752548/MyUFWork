...
USE `tr`;
update t_db_version set version='1.0.0.8',updateTime=now();

drop procedure if exists schema_change;
delimiter ';;';
create procedure schema_change() begin  
if exists (select * from information_schema.columns where table_name = 't_user_prize' and column_name = 'itemParams') then  
	alter table t_user_prize drop column itemParams;
end if; 
alter table t_user_prize ADD COLUMN itemParams text NULL DEFAULT NULL AFTER userPrizeName;
end;;
delimiter ';';
call schema_change();
drop procedure if exists schema_change;

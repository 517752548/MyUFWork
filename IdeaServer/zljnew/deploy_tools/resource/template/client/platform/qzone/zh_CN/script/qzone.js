...
//显示活动中心
function showCouponActivity(context)
{
	//if(pf == "pengyou" || pf == "qzone" || pf == "weibo"){
		var param = {
		  context : context,
		  onSuccess :qq_activitySuccess,
		  onClose :qq_activityClose
		};
		fusion2.dialog.showCouponActivity(param);
	//}
}

function qq_activitySuccess(opt){
	var fl = getfl();
	if(fl&&fl.qq_activitySuccess)
	{
		fl.qq_activitySuccess(opt);
	}
}

function qq_activityClose(opt){
	var fl = getfl();
	if(fl&&fl.qq_activityClose)
	{
		fl.qq_activityClose(opt);
	}
}
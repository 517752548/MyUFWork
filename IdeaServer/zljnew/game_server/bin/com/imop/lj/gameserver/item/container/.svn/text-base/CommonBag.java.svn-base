...
public abstract class CommonBag extends AbstractItemBag {
	...
	abstract public int getMaxCanAdd(ItemTemplate itemTemplate, boolean isBind);
	
	abstract public void setResetBagNum(int newCapacity);

	abstract public Collection<Item> add(ItemTemplate template, int count, ItemGenLogReason reason, String detailReason, boolean isBind);
...
//	};
	...
	/** 绑定优先，叠加数比较器，按叠加从小到大排列 */
	protected final Comparator<Item> overlapCompBind = new Comparator<Item>() {

		@Override
		public int compare(Item itemA, Item itemB) {
			//两个绑定状态不同，则绑定的排在前面
			if (itemA.isBind() != itemB.isBind()) {
				if (itemA.isBind()) {
					return -1;
				} else if (itemB.isBind()) {
					return -1;
				}
			}
			
			//下面是两个道具绑定状态相同，按照默认的叠加数来
			int ovlpA = itemA.getOverlap();
			int ovlpB = itemB.getOverlap();
			if (ovlpA > ovlpB) {
				return 1;
	...
	/** 非绑定优先，叠加数比较器，按叠加从小到大排列 */
	protected final Comparator<Item> overlapCompNotBind = new Comparator<Item>() {
	 ...
	 * @return 移除了的Item
	 */
	public abstract Collection<Item> removeItem(int templateId, int count, ItemLogReason reason, String detailReason, boolean bindFirst);
	 ...
	 * @return
	 */
	public abstract int getNeedSlot(ItemTemplate template, int count, boolean isBind);

...
@ExcelRowBinding
public class ScribedLogTimeEventTemplate extends ScribedLogTimeEventTemplateVO {

	private int[] timeEventIds;

	@Override
	public void setRefreshActivityTimeIds(String refreshActivityTimeIds) {
		super.setRefreshActivityTimeIds(refreshActivityTimeIds);
	...
	public void check() throws TemplateConfigException {
		...
		for (int timeEventId : timeEventIds) {
			TimeEventTemplate template = templateService.get(timeEventId, TimeEventTemplate.class);
			if (template == null) {
				throw new TemplateConfigException(this.getSheetName(), this.getId(), "timeEventId=" + timeEventId + "不存在于timeEvent模板中");
			}
		}
	}

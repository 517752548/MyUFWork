...
import com.imop.lj.gameserver.lifeskill.LifeSkillManager;
...
import com.imop.lj.gameserver.ringtask.RingTaskManager;
...
public class Human extends Role implements PersistanceObject<Long, HumanEntity>, RoleDataHolder, ITaskOwner {
    ...
    /** 非法移动累计时间 */
    private long invalidMoveDeltaTime;
     ...
     * 可见玩家的列表，一定策略更新
     */
    private Set<Long> canSeeSet = new HashSet<Long>();
    /**
     * 最近一次刷新可见玩家列表的实际
     */
    private long lastRefreshCanSeeTime;
     ...
     * 跑环任务管理器
     */
    private RingTaskManager ringTaskManager;
    ...
    public Human() {
        ...
        this.guaJiManager = new GuaJiManager(this);
        this.lifeSkillManager = new LifeSkillManager(this);
        ...
        this.ringTaskManager = new RingTaskManager(this);
    ...
    public HumanEntity toEntity() {
        ...
        entity.setGuaJiPoint(this.getGuaJiPoint());
        entity.setGuaJiPoint2(this.getGuaJiPoint2());
...
//        entity.setMainSkillType((Integer) (this.getRunningMainSkillType() == null ? this.getJobType().getDefaultMainSkillType() : this.getRunningMainSkillType().index));
        entity.setMainSkillPack(mainSKillMapToJson());
        entity.setMineLevel(this.getMineLevel());
        
        //月卡
        entity.setBuyMonthCardTime(this.getBuyMonthCardTime());
        //非法请求战斗次数
        entity.setInvalidBattleNum(this.getInvalidBattleNum());
        ...
        json.put(HumanPropAttr.GUA_JI.toString(), this.guaJiManager.toJsonProp());
        json.put(HumanPropAttr.LIFE_SKILL.toString(), this.lifeSkillManager.toJsonProp());
        json.put(HumanPropAttr.RING_TASK.toString(), this.ringTaskManager.toJsonProp());
    ...
    public Map<Integer, Integer> getMainSkillMap() {
		return mainSkillMap;
	}
    ...
    public void checkAfterRoleLoad() {
		...
		this.ringTaskManager.checkAfterRoleLoad();
		...
		this.lifeSkillManager.checkAfterRoleLoad();
	}
...
//			break;
            ...
            case GOLD:
                ...
                break;
            //免费挂机点是否足够，不够用充值挂机点顶替
            case GUA_JI_POINT:
            	flag = CurrencyProcessor.instance.hasEnoughMoney(this, amount, Currency.GUA_JI_POINT, Currency.GUA_JI_POINT2, needNotify);
            	break;
            case ENERGY:
    ...
    public CurrencyCostDetail getCurrencyCostDetail(long amount, Currency mainCurrency) {
        ...
        switch (mainCurrency) {
            ...
            case GUA_JI_POINT:
            	return CurrencyProcessor.instance.getCurrencyCostDetail(this, amount, Currency.GUA_JI_POINT, Currency.GUA_JI_POINT2, null);
            default:
                return null;
        }
    }
...
//			return CurrencyProcessor.instance.costMoney(this, amount, mainCurrency, null, null, needNotify, usageLangId, reason, detailReason, reportItemId);
            ...
            //免费挂机点可用充值挂机点顶替
            case GUA_JI_POINT:
            	return CurrencyProcessor.instance.costMoney(this, amount, Currency.GUA_JI_POINT, Currency.GUA_JI_POINT2, null, needNotify, usageLangId, reason,
            			detailReason, reportItemId);
            default:
                return CurrencyProcessor.instance.costMoney(this, amount, mainCurrency, null, null, needNotify, usageLangId, reason, detailReason, reportItemId);
        }
...
//	}
    ...
    public long getCanGiveMoneyNum(Currency currency) {
        ...
        switch (currency) {
            ...
            //挂机点，不能超过long最大值
            case GUA_JI_POINT:
                canGive = Globals.getGameConstants().getGuaJiPointMaxNum() - this.getGuaJiPoint();
                break;
            case GUA_JI_POINT2:
                canGive = Long.MAX_VALUE - this.getGuaJiPoint2();
                break;
    ...
    public boolean giveMoney(long amount, Currency currency, boolean needNotify, MoneyLogReason reason, String detailReason) {
        ...
        switch (currency) {
            ...
            case RED_ENVELOPE:
            	...
            	break;
            case GUA_JI_POINT:
            	flag = CurrencyProcessor.instance.giveMoney(this, amount, Currency.GUA_JI_POINT, needNotify, reason, detailReason, true);
            	break;
...
//        this.setModified();
    ...
    public long getGuaJiPoint() {
    	return baseStrProperties.getLong(RoleBaseStrProperties.GUA_JI_POINT);
    }
    
    public void setGuaJiPoint(long guaJiPoint) {
    	this.baseStrProperties.setLong(RoleBaseStrProperties.GUA_JI_POINT, guaJiPoint);
    	this.setModified();
    }
    
    public long getGuaJiPoint2() {
    	return baseStrProperties.getLong(RoleBaseStrProperties.GUA_JI_POINT2);
    }
    
    public void setGuaJiPoint2(long guaJiPoint2) {
    	this.baseStrProperties.setLong(RoleBaseStrProperties.GUA_JI_POINT2, guaJiPoint2);
    	this.setModified();
    }
...
//	
    ...
    public GuaJiManager getGuaJiManager() {
		return guaJiManager;
	}
...
//		}
    ...
    public RingTaskManager getRingTaskManager() {
		return ringTaskManager;
	}
...
//	}
     ...
     * @return
     */
    public long getBuyMonthCardTime() {
    	return this.finalProps.getLong(RoleFinalProps.BUY_MONTH_CARD_TIME);
    }
     ...
     * @param buyTime
     */
    public void setBuyMonthCardTime(long buyTime) {
    	this.finalProps.setLong(RoleFinalProps.BUY_MONTH_CARD_TIME, buyTime);
    	this.setModified();
    }
    
    /**
     * 获取非法请求战斗次数（指前台动画未播完，就请求下一轮战斗）
     * @return
     */
    public int getInvalidBattleNum() {
    	return this.finalProps.getInt(RoleFinalProps.INVALID_BATTLE_NUM);
    }
     ...
     * @param num
     */
    public void setInvalidBattleNum(int num) {
    	this.finalProps.setInt(RoleFinalProps.INVALID_BATTLE_NUM, num);
    	this.setModified();
    }
     ...
     * 非法请求战斗计数+1
     */
    public void addInvalidBattleNum() {
    	setInvalidBattleNum(getInvalidBattleNum() + 1);
    }
...
//        this.setModified();
    ...
    public void onPositionChanging(int x, int y) {
    	...
    	int move = (int) (distance / 72 + 0.5);
    	
    	//移动速度影响遇怪概率的限制如果打开，则需要验证非法情况
    	if (SharedConstants.MOVE_SPEED_LIMIT_MEET_MONSTER_OPEN) {
	    	long lastMoveTime = getLastMoveTime();
	    	long now = Globals.getTimeService().now();
	    	long deltaTime = now - lastMoveTime;
	    	
	    	//移动时间间隔过段，则可能玩家卡了，或者频繁发包造成
	    	if (deltaTime < SharedConstants.MOVE_TIME_MIN) {
	    		invalidMoveDeltaTime += deltaTime;
	    		//非法时间累计超过1秒，按走一步算
	    		if (invalidMoveDeltaTime > TimeUtils.SECOND) {
	    			//按移动一步算
	    			move = 1;
	    		} else {
	    			//累计非法时间太短，不更新步数
	    			Loggers.humanLogger.warn("两次移动时间间隔过短，不更新步数，last=" + lastMoveTime + ";now=" + now + 
	    					";deltaTime=" + deltaTime + ";invalidMoveDeltaTime=" + invalidMoveDeltaTime +";roleId=" + getRoleUUID());
	    			return;
	    		}
	    		
	    		Loggers.humanLogger.warn("两次移动时间间隔过短last=" + lastMoveTime + ";now=" + now + ";deltaTime=" + deltaTime + ";roleId=" + getRoleUUID());
			}
	        
	        double speed = distance * 1000 / deltaTime;
	        //移动速度
	        if (speed > SharedConstants.MOVE_SPEED_MAX) {
	        	//移动速度过快则按照最少的移动步数计算
				move = 1;
				
				Loggers.humanLogger.warn("移动速度过快=speed=" + speed + ";distance=" + distance + ";deltaTime=" + deltaTime + ";roleId=" + getRoleUUID());
			}
...
//			sendLocationInfo();
    ...
    public void refreshCanSee(Set<Long> ccSet) {
    	this.canSeeSet.clear();
    	this.canSeeSet.addAll(ccSet);
    	this.lastRefreshCanSeeTime = Globals.getTimeService().now();
    }
    
    public void addCanSee(long roleId) {
    	this.canSeeSet.add(roleId);
    }
    
    public int getCanSeeNum() {
    	return this.canSeeSet.size();
    }
    
    public boolean isCanSee(long roleId) {
    	return this.canSeeSet.contains(roleId);
    }
    
    public Set<Long> getCanSeeSet() {
    	return this.canSeeSet;
    }
    ...
    public long getLastRefreshCanSeeTime() {
		return lastRefreshCanSeeTime;
	}
    
    public void checkRefreshCanSeeSet() {
    	Globals.getHumanPosService().refreshHumanCanSee(this);
    }
...
//    }
     ...
     * @return
     */
    public boolean isInGuaJi(){
    	if(getOwner().getGuaJiManager().isGuaJi()){
    		return true;
    	}
    ...
    public boolean isFullEnemy(){
    	if(getOwner().getGuaJiManager().isFullEnemy()){
    		return true;
    	}
    ...
    public boolean isSwitchScene(){
    	if(getOwner().getGuaJiManager().isSwitchScene()){
    		return true;
    	}
     ...
     * @return
     */
    public boolean isInGather(){
    	int lifeSkillFlag = getOwner().getLifeSkillManager().getLifeSkillFlag();
    	if(lifeSkillFlag > 0
    			&& lifeSkillFlag <= LifeSkillType.values().length){
    		return true;
    	}
    ...
    public int getLifeSkillFlag(){
    	return getOwner().getLifeSkillManager().getLifeSkillFlag();
    }

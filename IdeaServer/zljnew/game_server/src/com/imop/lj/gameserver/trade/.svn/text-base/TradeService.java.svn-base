...
//		}
	...
	protected boolean hasEnoughSpace(Human human,Trade trade) {
		...
		case ITEM : {
			...
			//背包是否可以放下
			int maxCanAdd = human.getInventory().getCommonBagByType(ti.getTemplate().getBagType()).getMaxCanAdd(ti.getTemplate(), false);
	...
	protected boolean sendCommodityToHuman(Human human,Trade trade,boolean isBuy){
		...
		case ITEM : {
			...
			if (ti.getTemplate().isEquipment() || ti.getTemplate().isSkillEffectItem()) {
				Item equipItem = Globals.getItemService().addItemWithFeature(reason, genDetailReason, reasonItem, human, itemTplId, ti.getFeature(), false);
				if (equipItem == null) {
					return false;
				}
			} else {
				//拍卖行都是用银子买卖，非绑定
				long cost = isBuy ? trade.getCurrencyNum()*trade.getCommodityNum() : 0;
				Collection<Item> items = human.getInventory().addItem(itemTplId, trade.getCommodityNum(), reason, genDetailReason, true, true, 
						trade.getCurrencyType(), cost, cost, false);
//				Collection<Item> items = human.getInventory().addItem(itemTplId, trade.getCommodityNum(), reason, genDetailReason, false, true);

...
public class ExamReadyMessage extends AbstractActivityMessage {
	
	private ActivityTemplate template;
	
	public ExamReadyMessage(ActivityTemplate template){
	...
	public void execute() {
		...
		if(activityService.setActivityState(activityId, ActivityState.READY)) {
			// 准备阶段的处理
//			Globals.getExamService().handleActivityReadyMsg(activityService.getActivity(activityId));
		}
	}

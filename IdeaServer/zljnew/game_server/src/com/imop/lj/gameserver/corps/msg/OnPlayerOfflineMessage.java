...
public class OnPlayerOfflineMessage extends SysInternalMessage {
	...
	@Override
	public void execute() {
		Human humanOnlineOrCache = Globals.getHumanCacheService().getHumanOnlineOrCache(humanId);
...
//		Globals.getSeeOtherService().removeUserInfoByUUID(humanId);
		...
		//挂机暂停
		Globals.getGuaJiService().pauseGuaJi(humanOnlineOrCache);
		//停止开采
		Globals.getLifeSkillService().cancelLifeSkill(humanOnlineOrCache, false);
	}

...
//		}
	...
	protected BackupPubTaskInfo buildBackupPubTaskInfo(Human human, BackupPubTask bpt) {
		...
		if (questTpl != null) {
			RewardInfo rewardInfo = getPubCalculateReward(human, bpt.getStar(),  questTpl.getRewardId()).toRewardInfo(); 
	...
	public boolean onFinishTask(Human human) {
		...
		//单独给奖励
		Reward baseRewad = getPubCalculateReward(human, human.getPubTaskManager().getCurTask().getStar(), 
				Globals.getGameConstants().getCalculateExpPubRewardId());
		Globals.getRewardService().giveReward(human, baseRewad, true);
	...
	private Reward getPubCalculateReward(Human human, int star, int rewardId) {
		...
		baseParams.put(RewardDef.CALC_KEY_STAR, star);
	...
	public void refreshTaskManual(Human human, int refreshType) {
		...
		if (itemFlag) {
			Collection<Item> ci = human.getInventory().removeItem(itemId, count, 
					ItemLogReason.PUBTASK_REFRESH_COST, ItemLogReason.PUBTASK_REFRESH_COST.getReasonText(), true);
			if (ci == null || ci.isEmpty()) {
				return;
			}
		} else {
			boolean mf = human.costMoney(costNum, costCurrency, true, 0, 

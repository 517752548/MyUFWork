...
public class GiveMoneyListener implements IEventListener<GiveMoneyEvent> {
	...
	public void fireEvent(GiveMoneyEvent event) {
		...
		case SKILL_POINT:
			...
			//提升
			if (Globals.getHumanSkillService().canMindLevelUpgrade(human, new MainSkillTipsInfo()) || Globals.getHumanSkillService().canMindSkillUpgrade(human, new MainSkillTipsInfo())){

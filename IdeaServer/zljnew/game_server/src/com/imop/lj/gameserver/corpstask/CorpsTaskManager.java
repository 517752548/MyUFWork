 ...
 *
 */
public class CorpsTaskManager extends AbstractTaskManager<Human> implements RoleDataHolder, JsonPropDataHolder {
	/** 当前进行的任务，进行中或已放弃 */
	private CorpsTask curTask;
	
	public CorpsTaskManager(Human owner) {
		super(owner);
	}
	...
	public void load() {
		...
		List<CorpsTaskEntity> entityList = Globals.getDaoService().getCorpsTaskDao().loadEntityByCharId(charId);
		for (CorpsTaskEntity entity : entityList) {
			QuestTemplate template = Globals.getTemplateCacheService().get(entity.getQuestId(), QuestTemplate.class);
			if (null == template) {
				// 记录错误日志，对应的任务已被删除
				Loggers.questLogger.error("#CorpsTaskManager#load#ERROR!questTpl is null!questId=" + entity.getQuestId());
				continue;
			}
			CorpsTask task = new CorpsTask(owner, template);
	...
	private void updateCurTask(CorpsTask task) {
		curTask = task;
	}
	...
	public boolean canAcceptSpecialCheck(int questId) {
		boolean flag = Globals.getCorpsTaskService().canAcceptTask(getOwner(), questId);
		return flag;
	}
	...
	public CorpsTask getCurTask() {
		return curTask;
	}

	public void setCurTask(CorpsTask curTask) {

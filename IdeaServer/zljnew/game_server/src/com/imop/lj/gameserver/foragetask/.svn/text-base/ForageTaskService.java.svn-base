...
public class ForageTaskService {
	...
	public void acceptTask(Human human, int questId) {
		...
		if (ypFlag) {
			//优先银票
			boolean isYPCost = human.costMoney(ypCount, ypCurrency, true, 0, 
	...
	public void refreshTaskManual(Human human) {
		...
		Collection<Item> baseList =  human.getInventory().removeItem(itemId, count,
				LogReasons.ItemLogReason.FORAGETASK_REFRESH_COST, 
				LogUtils.genReasonText(LogReasons.ItemLogReason.FORAGETASK_REFRESH_COST), true);

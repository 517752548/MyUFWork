...
public class CorpsWarStartMessage extends AbstractActivityMessage {
	
	private ActivityTemplate template;
	
	public CorpsWarStartMessage(ActivityTemplate template){
	...
	public void execute() {
		...
		if(activityService.setActivityState(activityId, ActivityState.OPENING)) {
			// 开始阶段的处理
			Globals.getCorpsWarService().handleActivityStartMsg(activityService.getActivity(activityId));
		}
	}

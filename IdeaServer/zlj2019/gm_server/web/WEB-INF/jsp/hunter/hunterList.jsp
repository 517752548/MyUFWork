...
function search(){
	...
	window.location.href="hunter.do?action=init&searchType="+searchType+"&searchValue="+searchValue+"&startLevel="+startLevel+"&endLevel="+endLevel;
}
function goTo(i){
	var searchType=$("#searchType").val();
    var searchValue=$("#searchValue").val();
...
function kickOut(id){
	...
	if(con){
		$.post("hunter.do?action=kickOut",{"id":"'"+id+"'"},function(info){
			var info = $.trim(info);
			if(info=="ok"){
              alert("<%=lang.readGm(GMLangConstants.CMDSUCCESS)%>");
              window.location="hunter.do?action=init";
			}else{
...
<span id="show_text">
    ...
    <span><%=lang.readGm(GMLangConstants.HUNTER_HUNTERINDEX)%>:<input id='startLevel' type='text' value="${startLevel}" style="width: 50px;"/>&mdash;&mdash;<input id='endLevel' type='text' value="${endLevel}" style="width: 50px;margin:0px;"/></span>
</span>&nbsp;&nbsp;
...
<input id="batchSearch" type="button" class="butcom" style="margin-left: 1em;" value="<%= lang.readGm(GMLangConstants.BATCH_SEARCH_LASTLOGIN)%>" onclick="javaScript:window.location='hunter.do?action=batchSearchInit'">
</div>
...
<table class="detail"  cellspacing="0" cellpadding="0" border="0">
  <tbody><tr>
    ...
    <th>&nbsp;<%= lang.readGm(GMLangConstants.HUNTER_HUNTERINDEX)%></th>
 	<!-- <th>&nbsp;<%= lang.readGm(GMLangConstants.HUNTER_HUNTERID)%></th> -->
 	<th>&nbsp;<%= lang.readGm(GMLangConstants.HUNTER_OPEN)%></th>	
 	<th>&nbsp;<%= lang.readGm(GMLangConstants.HUNTER_ISVIP)%></th>	
  </tr>
  <c:forEach items="${hunterList}" var="hunter">
      <tr>
		<!-- <td><input type="checkbox" id="${hunter.id}" name="users"/></td> -->
        <td><a href="role.do?action=init&searchType=roleId&searchValue=${hunter.charId}" class="link">&nbsp;${hunter.charId} </a></td>
         <td>&nbsp;${hunter.hunterIndex}</td>

...
function search(){
	...
	window.location.href="sortHonerLevel.do?action=init&searchType="+searchType+"&searchValue="+searchValue+"&startLevel="+startLevel+"&endLevel="+endLevel+"&startIndexSort="+startIndexSort+"&endIndexSort="+endIndexSort;
}
function goTo(i){
	var searchType=$("#searchType").val();
    var searchValue=$("#searchValue").val();
...
function kickOut(id){
	...
	if(con){
		$.post("sortHonerLevel.do?action=kickOut",{"id":"'"+id+"'"},function(info){
			var info = $.trim(info);
			if(info=="ok"){
              alert("<%=lang.readGm(GMLangConstants.CMDSUCCESS)%>");
              window.location="sortHonerLevel.do?action=init";
			}else{
...
<input id="batchSearch" type="button" class="butcom" style="margin-left: 1em;" value="<%= lang.readGm(GMLangConstants.BATCH_SEARCH_LASTLOGIN)%>" onclick="javaScript:window.location='sortHonerLevel.do?action=batchSearchInit'">
</div>
...
<table class="detail"  cellspacing="0" cellpadding="0"
  ...
  <tbody><tr>
    ...
    <th>&nbsp;<%= lang.readGm(GMLangConstants.SECROTARENALEVEL_UPORDOWN)%></th>
     <th>&nbsp;<%= lang.readGm(GMLangConstants.SECROTEVEL_HONOR)%></th>

  </tr>
  <c:forEach items="${sortHonorLevelList}" var="sortHonorLevel">
      <tr>
       <!--  <td>&nbsp;${sortArenaLevel.arenaLevel}</td>-->  
        <td>&nbsp;${sortHonorLevel.indexSort}</td>
        <td><a 	href="user.do?action=init&searchType=userId&searchValue=${sortHonorLevel.accountId}"class="link">&nbsp;${sortHonorLevel.accountId}</a></td>
        
        <td><a href="role.do?action=init&searchType=roleId&searchValue=${sortHonorLevel.roleId}" class="link">&nbsp;${sortHonorLevel.roleId} </a></td>
         <td>&nbsp;${sortHonorLevel.name}</td>
        <td>&nbsp;${sortHonorLevel.level}</td>
        
        <td>&nbsp;${sortHonorLevel.guildPack}</td>
         <td>&nbsp;${sortHonorLevel.upOrDown}</td>
          <td>&nbsp;${sortHonorLevel.honor}</td>
      </tr>
 	</c:forEach>
  <tr>

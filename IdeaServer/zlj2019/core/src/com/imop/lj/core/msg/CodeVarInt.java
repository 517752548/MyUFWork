...
public class CodeVarInt {
	...
	public static void writeTag(final int fieldNumber, final int wireType, final IoBuffer buffer) {
		writeRawVarint32(WireFormat.makeTag(fieldNumber, wireType), buffer);
	}

	public static int readTag(IoBuffer buffer) {
		return readRawVarint32(buffer);
	}

	public static void writeRawVarint32(int value, IoBuffer buffer) {
		while (true) {
			if ((value & ~0x7F) == 0) {
	 ...
	 * upper bits.
	 */
	public static int readRawVarint32(IoBuffer buffer) {
		byte tmp = buffer.get();
		if (tmp >= 0) {
			return tmp;
		}
	...
	public static long readRawVarint64(final IoBuffer buffer) {
		int shift = 0;
		long result = 0;
		while (shift < 64) {
	...
	public static void writeRawVarint64(long value, IoBuffer buffer) {
		while (true) {
			if ((value & ~0x7FL) == 0) {
				buffer.put((byte) value);
				return;

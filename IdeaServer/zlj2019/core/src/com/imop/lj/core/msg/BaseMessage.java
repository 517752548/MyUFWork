...
public abstract class BaseMessage implements IMessage {
	...
	//消息体是否加密的标识位
	protected static boolean ENC_FLAG = false;
	protected static byte[] ENC_ARR = {1, 0, 1, 1, 0, 0, 1, 1};
	protected static int ENC_ARR_LEN = ENC_ARR.length;
	...
	public boolean write() throws MessageParseException {
		try {
			...
			boolean b = false;
			if(isCompress()){
				b = MessageCompressor.compressMessageBody(this);
			}else{
				b = writeImpl();
			}
			if (!b) {
	...
	/**
	 * 消息体加密，最多加密{@link BaseMessage#ENC_ARR_LEN}个字节
	 */
	private void encryptMsgBody() {
		if (!ENC_FLAG) {
			return;
		}
		byte[] msgArr = buf.array();
		int head = 4;
		//空消息，不处理
		...
		for (int i = 0; i < encLen; i++) {
			msgArr[i + head] = (byte) (msgArr[i + head] ^ ENC_ARR[i]);
		}
	}
	...
	public boolean read() throws MessageParseException {
		try {
			...
			boolean b = false;
			if(isCompress()){
				b = MessageCompressor.uncompressMessageBody(this);
			}else{
				b = readImpl();
			}
			return b;
...
//		_buf.release();
	...
	public String toString() {
		ReflectionToStringBuilder _builder = new BaseToStringBuilder(this,
				ToStringStyle.SIMPLE_STYLE, toStringExcludeFields);

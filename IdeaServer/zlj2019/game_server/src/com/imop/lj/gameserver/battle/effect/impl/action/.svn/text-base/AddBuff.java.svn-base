...
public class AddBuff extends AbstractAction {
	...
	protected boolean preCost(FightUnit owner, FightUnit target, List<ReportItem> content, RealDamage realDamageHp) {
		return true;
	}
	...
	protected List<ReportItem> doActionExecute(Phase paramPhase,
		...
		boolean canExexute = canExucute(owner, action);
		if(!canExexute){
			if (this.logger.isDebugEnabled()) {
				this.logger.debug("owner " + owner.getIdentifier() + " canExucute ret value is invalid,can not add buff.buffEffectId=" + effectId);
			}
			return content;
		}
		for (FightUnit target : action.getTargets(this)) {
			...
			boolean canCost = preCost(owner, target, content, realDamageHp);
			if(!canCost){
				if (this.logger.isDebugEnabled()) {
			...
			List<ReportItem> report = buffEffect.addTo(target, Math.abs(buffEffect.getValue().intValue()), realDamageHp);

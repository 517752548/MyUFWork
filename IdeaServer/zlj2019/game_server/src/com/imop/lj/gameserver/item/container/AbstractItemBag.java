...
public abstract class AbstractItemBag implements Bag<Item, Human> {
	...
	public final List<Item> getAllItemByTmpId(int templateId, boolean isBind) {
		List<Item> resultList = new ArrayList<Item>();
		findByTmplId(templateId, resultList);
		return resultList;
	}
	 ...
	 * @return
	 */
	public final int getCountByTmpId(int templateId, boolean isBind) {
		int count = 0;
		for (Item item : items) {
			if (Item.isSameTemplateId(templateId, item) 
					&& isBind == item.isBind()) {
	...
	private void findByTmplId(int templateId, boolean isBind, Collection<Item> resultList) {
		for (Item item : items) {
			if (Item.isSameTemplateId(templateId, item) 
					&& isBind == item.isBind()) {
				resultList.add(item);

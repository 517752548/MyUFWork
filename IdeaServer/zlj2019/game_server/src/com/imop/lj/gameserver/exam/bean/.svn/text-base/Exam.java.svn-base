...
public class Exam extends AbstractExam{
	...
	public void examChoseAnswer(Human human, Integer examType, Integer choseAnswer) {
		...
		if(exam.getExamInfo().getTotalNum()>=0 && exam.getExamInfo().getTotalNum()<Globals.getGameConstants().getQuestionNumOfProvincialExamination()){
			...
			baseParams.put(RewardDef.CALC_KEY_EXAM_ANSWER, flag);
	...
	public void examUseItem(Human human, Integer examType, Integer itemId) {
		...
		//8.扣除特殊道具
		Collection<Item> itemList = human.getInventory().removeItem(ExamDef.ExamAssistItem.valueOf(itemId).getItemId(), useNum, LogReasons.ItemLogReason.COST_ITEM_FOR_EXAM, 
				LogUtils.genReasonText(LogReasons.ItemLogReason.COST_ITEM_FOR_EXAM, ExamDef.ExamAssistItem.valueOf(itemId).getItemId()), true);

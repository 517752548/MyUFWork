...
//		x+1,y-1
	...
	public static boolean inArea(int x, int y, int dx, int dy, int offset) {
		//TODO XXX 这里先不判断位置，因为前台是定时发送位置的，服务器的位置不及时，所以会出现前台在npc旁边，但后台不在，会出错
//		if(offset <= 0){
//			return false;
//		}
...
//		}
	...
	protected boolean userEnterMapPoint(Human human, boolean isLogin, int x, int y) {
		...
		//重置可见集合
		Globals.getHumanPosService().refreshHumanCanSee(human);
...
//		int neighborListCount = 0;
		...
		for (Human h : humans.values()) {
			//排除自己
			if (h.getRoleUUID() == human.getRoleUUID()) {
				continue;
			}
			
//			//自己能看到的其他人
//			if (human.isCanSee(h.getRoleUUID())) {
...
//			}
			...
			//其他人能看到自己
			if (h.isCanSee(human.getRoleUUID())) {
...
//		}
		...
		if (needDelayPos) {
			...
			for (Human h : humans.values()) {
				...
				if (h.getCharId() != human.getCharId()) {
					...
					if (h.isCanSee(human.getRoleUUID()) &&
							DiffType.NEAR == getDiffType(human, human.getX(), human.getY(),	h.getX(), h.getY()) ||
	...
	public boolean userMove(Human human, int dx, int dy, int fx, int fy) {
		...
		//组队信息，需要特殊处理
		boolean isInTeamNormal = Globals.getTeamService().isInTeamNormal(human.getRoleUUID());
		int humanTeamId = 0;
		if (isInTeamNormal) {
			humanTeamId = Globals.getTeamService().getHumanTeam(human.getRoleUUID()).getId();
		}
		...
		for (Human h : humans.values()) {
			...
			//如果两个玩家在一个队伍，不能删对方
			if (isInTeamNormal && Globals.getTeamService().isInTeamNormal(h.getRoleUUID())) {
				if (Globals.getTeamService().getHumanTeam(h.getRoleUUID()).getId() == humanTeamId) {
					isTeamMate = true;
				}
			}
			...
			if (human.isCanSee(h.getRoleUUID())) {
				...
				case TREND_NEAR_2_FAR:
					//队友之间不发delete的消息
					if (!isTeamMate && changedListCount < MapDef.MAP_PLAYER_CHANGED_LIST_MAX) {
...
//		}
	...
	public Human getMapHuman(long roleId) {
		return humans.get(roleId);
	}

...
public class MovePrimBag2Store extends AbstractMoveItemOperation {
	...
	protected boolean moveImpl(Human user, Item from, Item to) {
		...
		// 不能全放下，一个也不放
		if (storeBag.getMaxCanAdd(fromItemTemplate, isBind) < from.getOverlap()) {
			//放不下
		...
		Collection<Item> updatedPrimBagItems = storeBag.add(tmpl, dropItemOoverlap, ItemGenLogReason.PRIM2STORE, reasonDetail, isBind);

...
//        Globals.getLogService().sendTitleLog(); TODO 获取离线的human
    ...
    public String getCurrentTitleName(Human human) {
        String re = "";
        if (!allTitle.containsKey(human.getCharId())) {
            return re;
        }
        TitleInfo tinfo = getCurrentTitleInfo(human.getCharId());
        if(tinfo == null){
            return re;
        }
        
        re = tinfo.getTitleName();
        if (human != null) {
        	//隐藏title了，不显示
        	if (human.getTitleDis() == 0) {
        		re = "";
        	}
        }

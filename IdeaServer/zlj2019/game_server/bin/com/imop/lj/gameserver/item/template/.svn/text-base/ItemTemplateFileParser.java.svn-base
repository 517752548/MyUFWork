...
public class ItemTemplateFileParser extends TemplateFileParser {
	...
	/** 身份类型所在列 */
	private static final int IDENDITY_TYPE_COLUMN = 7;
	...
	public ItemTemplateFileParser() {
		...
		clazzes[9] = EquipCraftItemTemplate.class;
		clazzes[10] = LifeSkillBookTemplate.class;
		clazzes[11] = PetSkillBookItemTemplate.class;
	}
	...
	private Map<Integer, TemplateObject> parseXlsSheet(HSSFSheet sheet, int sheetNum, String excelFileName, TemplateService templateService)
		...
		for (int i = 1; i < Short.MAX_VALUE; i++) {
			...
			case LEADER_SKILL_BOOK:
				...
				break;
				//装备打造材料
			case CRAFT_EQUIP_ITEM:
				obj = (ItemTemplate) clazzes[9].newInstance();
				break;
				//生活技能书
			case LIFE_SKILL_BOOK:
				obj = (ItemTemplate) clazzes[10].newInstance();
				break;
				//骑宠技能书
			case PET_HORSE_SKILL_BOOK:
				obj = (ItemTemplate) clazzes[11].newInstance();
				break;
			default:
				throw new TemplateConfigException(ItemTemplate.SHEET_NAME, templId, "错误的道具身份类型 typeId=" + equipType);
			}

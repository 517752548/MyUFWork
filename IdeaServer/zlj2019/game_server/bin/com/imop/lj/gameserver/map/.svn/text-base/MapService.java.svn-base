...
//			human.sendErrorMessage(LangConstants.MAP_ENTER_FAIL);
	...
	public void move(Human human, int mapId, int dx, int dy, int fx, int fy) {
		...
		boolean canStartBattle = canBattleByState(human) ? false : canStartBattle(human);
		if (canStartBattle) {
			fx = dx;
			fy = dy;
		}
	...
	//采集或挂机中不遇怪
	private boolean canBattleByState(Human human) {
		return human.isInGather() || human.isInGuaJi();
	}
	...
	/**
	 * 判断玩家能否触发战斗，在地图中挂机时调用
	 * @param human
	 * @return
	 */
	public boolean canStartBattleForGuaJi(Human human) {
		//战斗中，不能再战斗
		if (human.isInAnyBattle()) {
			return false;
		}
		...
		//必须通过通天塔NPC
		if (this.isTower(human.getMapId())) {
		...
		// 战斗结束一定时间内不会遇怪,读取用户挂机配置的时间
		if (Globals.getTimeService().now() - human.getLastBattleEndTime()
				< human.getGuaJiManager().getEncounterSecond() * Globals.getGameConstants().getScale()) {
			return false;
		}
	...
	public int mapFightNpc(Human human, NpcInfo npcInfo, boolean needBroadcast) {
		...
		if (Globals.getTeamService().canTriggerSingleBattle(roleId)) {
			...
			//固定npc，数量按最大算
			EnemyParamContent epc = new EnemyParamContent(enemyArmyId, GameConstants.MAX_HUMAN_NUM, human.getLevel(), human.getMapId(), false, false);
			...
			EnemyParamContent epc = new EnemyParamContent(enemyArmyId, GameConstants.MAX_HUMAN_NUM, team.getAvgLevel(), team.getMapId(), false, false);
			Fighter<?> defender = Fighter.valueOf(FighterType.ENEMY, epc, false);
	 ...
	 * @return
	 */
	public boolean isInSafeMap(int mapId){
		return Globals.getTemplateCacheService().getMapTemplateCache().isInSafeMap(mapId);
	}
...
//		}
	 ...
	 * @return
	 */
	public boolean isInLifeSkillArea(Human human, int mapId, int x, int y) {
		if (mapId <= 0 || x <= 0 || y <= 0) {
			return false;
		}
		//玩家当前是否在指定的地图
		...
		//玩家是否在指定的区域范围内
		if (!AbstractGameMap.inArea(human.getTileX(), human.getTileY(), x, y, Globals.getGameConstants().getLifePointInAreaOffset())) {
			return false;
		}
		return true;
	}

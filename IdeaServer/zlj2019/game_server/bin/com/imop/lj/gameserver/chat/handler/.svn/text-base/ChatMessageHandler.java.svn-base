...
import com.imop.lj.gameserver.show.ShowDef;
...
//		}
		...
		if (msg.getChatType() == ChatConstants.VOICE_CHAT_TYPE) {
			...
			hasDirtyWords = chatInfoFilter.containKeywords(wordStr);
			if (hasDirtyWords) {
		...
		} else if (msg.getChatType() == ChatConstants.SHOW_CHAT_TYPE) {
			if (_msgContent.length() <= 2) {
				return;
			}
			//展示宠物、道具等，需要以指定字符结尾
			if (!_msgContent.endsWith(ShowDef.SHOW_CHAR)) {
				return;
			}
			
			//倒数第二个指定字符的位置
			int s1 = _msgContent.substring(0, _msgContent.length() - 1).lastIndexOf(ShowDef.SHOW_CHAR);
			//最后一个指定字符的位置
			int s2 = _msgContent.length() - 1;
			if (s1 < 0 || s1 == s2) {
				return;
			}
			...
			//如果展示前面有说其他内容，需要过滤
			if (s1 > 0) {
				String tmpStr = _msgContent.substring(0, s1);
				tmpStr = chatInfoFilter.filterHtmlTag(tmpStr);
				hasDirtyWords = chatInfoFilter.containKeywords(tmpStr);
				if (hasDirtyWords) {
					tmpStr = chatInfoFilter.filter(tmpStr);
					_msgContent = tmpStr + _msgContent.substring(s1);
				}
			}
		} else {
			//普通聊天

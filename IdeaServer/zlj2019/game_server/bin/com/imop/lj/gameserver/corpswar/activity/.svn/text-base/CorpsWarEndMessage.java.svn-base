...
public class CorpsWarEndMessage extends AbstractActivityMessage {
	
	private ActivityTemplate template;
	
	public CorpsWarEndMessage(ActivityTemplate template){
	...
	public void execute() {
		...
		if(activityService.setActivityState(activityId, ActivityState.FINISHED)) {
			// 结束阶段的处理
			Globals.getCorpsWarService().handleActivityEndMsg();
		}
	}

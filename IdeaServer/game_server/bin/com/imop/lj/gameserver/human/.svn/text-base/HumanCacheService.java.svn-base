...
public class HumanCacheService {
	 ...
	 * @param roleId
	 */
	public void gmDelHumanCache(final long roleId) {
		IMessage msg = new SysInternalMessage() {
			@Override
			public void execute() {
				Human delHuman = delHumanOnTime(roleId, "gmDelHumanCache");
				Loggers.humanLogger.info("gmDelHumanCache delHuman=" + delHuman + ";roleId=" + roleId);
			}
		};
	...
	public void checkDel() {
		...
		for (Long roleId : delSet) {
			delHumanOnTime(roleId, "checkDel");
		}
	}
	
	protected Human delHumanOnTime(long roleId, String source) {
		try {
		...
		//从humanMap中删除该玩家
		Human ret = delHuman(roleId, source);
		return ret;
	}

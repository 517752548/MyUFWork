...
//		entity.setMainSkillLevel(RoleConstants.HUMAN_INIT_LEVEL_NUM);
		...
		//设置初始心法Id和等心法级
		entity.setMainSkillPack(this.genDefaultMainSkill());
		//设置采矿等级
	 ...
	 * @return
	 */
	public String genDefaultMainSkill() {
		Map<Integer, Integer> mainSkillMap = Maps.newHashMap();
		for (MainSkillType type : MainSkillType.values()) {
			if(type == MainSkillType.NULL){
				continue;
			}
			mainSkillMap.put(type.getIndex(), BattleDef.DEFAULT_MAIN_SKILL_LEVEL);
		}

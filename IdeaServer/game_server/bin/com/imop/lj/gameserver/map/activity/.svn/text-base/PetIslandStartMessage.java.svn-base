...
public class PetIslandStartMessage extends AbstractActivityMessage {
	
	private ActivityTemplate template;
	
	public PetIslandStartMessage(ActivityTemplate template){
	...
	public void execute() {
		...
		if(activityService.setActivityState(activityId, ActivityState.OPENING)) {
			// 开始阶段的处理
			Globals.getPetIslandService().handleActivityStartMsg(activityService.getActivity(activityId));
		}
	}

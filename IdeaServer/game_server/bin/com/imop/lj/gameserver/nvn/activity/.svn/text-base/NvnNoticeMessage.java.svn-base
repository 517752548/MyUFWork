...
public class NvnNoticeMessage extends AbstractActivityMessage {
	
	private ActivityTemplate template;
	
	public NvnNoticeMessage(ActivityTemplate template){
	...
	public void execute() {
		...
		if(activityService.setActivityState(activityId, ActivityState.NOT_OPEN)) {
			// 提醒阶段的处理
			Globals.getNvnService().handleActivityNoticeMsg(activityService.getActivity(activityId));
		}
	}

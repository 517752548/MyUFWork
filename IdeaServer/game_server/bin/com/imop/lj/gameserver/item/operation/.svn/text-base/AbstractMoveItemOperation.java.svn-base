...
public abstract class AbstractMoveItemOperation implements MoveItemOperation {
	...
	@Override
	public <T extends Role> boolean move(Human user, T wearer, Item from, Item to) {
		Assert.notNull(user);
	 ...
	 * @return
	 */
	protected abstract <T extends Role> boolean moveImpl(Human user, T wearer, Item fromItem, Item toItem);

	/**
	 * 交换两个Item，用来辅助各种移动操作 Human所属背包之间的道具交换的基本实现
	 * 
	 * @param from
	 * @param to
	 * @return
	...
	protected boolean swapItem(Item from, Item to) {
		...
		AbstractItemBag fromBag = fromOwner.getInventory().getBagByType(from.getBagType(), from.getWearerId());
		// 应该强制wearerId都是0l

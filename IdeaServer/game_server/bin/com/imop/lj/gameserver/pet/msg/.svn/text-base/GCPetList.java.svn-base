...
public class GCPetList extends GCMessage{
	...
	// 层数
	int _petInfoList_skillList_layer = readInteger();
	//end
	_petInfoList_skillList[petInfoList_skillListIndex].setLayer (_petInfoList_skillList_layer);

	// 熟练度
	long _petInfoList_skillList_proficiency = readLong();
	//end
	_petInfoList_skillList[petInfoList_skillListIndex].setProficiency (_petInfoList_skillList_proficiency);
	}
	//end
	...
	// 宠物技能栏数量
	int _petInfoList_petSkillBarNum = readInteger();
	//end
	_petInfoList[petInfoListIndex].setPetSkillBarNum (_petInfoList_petSkillBarNum);

	// 宠物资质丹索引
	int petInfoList_propItemIndexSize = readUnsignedShort();
	...
	for(petInfoList_propItemIndexIndex=0; petInfoList_propItemIndexIndex<petInfoList_propItemIndexSize; petInfoList_propItemIndexIndex++){
		_petInfoList_propItemIndex[petInfoList_propItemIndexIndex] = readInteger();
	}//end
	_petInfoList[petInfoListIndex].setPropItemIndex (_petInfoList_propItemIndex);

	// 技能快捷栏信息
	int petInfoList_shortcutListSize = readUnsignedShort();
	com.imop.lj.common.model.pet.ShortcutInfo[] _petInfoList_shortcutList = new com.imop.lj.common.model.pet.ShortcutInfo[petInfoList_shortcutListSize];
	...
	for(petInfoList_shortcutListIndex=0; petInfoList_shortcutListIndex<petInfoList_shortcutListSize; petInfoList_shortcutListIndex++){
		_petInfoList_shortcutList[petInfoList_shortcutListIndex] = new com.imop.lj.common.model.pet.ShortcutInfo();
	// 快捷栏索引, 默认为-1
	int _petInfoList_shortcutList_shortcutIndex = readInteger();
	//end
	_petInfoList_shortcutList[petInfoList_shortcutListIndex].setShortcutIndex (_petInfoList_shortcutList_shortcutIndex);

	// 技能Id
	int _petInfoList_shortcutList_skillId = readInteger();
	//end
	_petInfoList_shortcutList[petInfoList_shortcutListIndex].setSkillId (_petInfoList_shortcutList_skillId);
	}
	//end
	_petInfoList[petInfoListIndex].setShortcutList (_petInfoList_shortcutList);

	// 灵魂链接骑宠ID, 0-无灵魂链接
	long _petInfoList_soulLinkPetHorseId = readLong();
	//end
	_petInfoList[petInfoListIndex].setSoulLinkPetHorseId (_petInfoList_soulLinkPetHorseId);
	}
	//end
	...
	int petInfoList_skillList_layer = petInfoList_skillList[petInfoList_skillListIndex].getLayer();

	// 层数
	writeInteger(petInfoList_skillList_layer);

	long petInfoList_skillList_proficiency = petInfoList_skillList[petInfoList_skillListIndex].getProficiency();

	// 熟练度
	writeLong(petInfoList_skillList_proficiency);
	}
	//end
	...
	int petInfoList_petSkillBarNum = petInfoList[petInfoListIndex].getPetSkillBarNum();

	// 宠物技能栏数量
	writeInteger(petInfoList_petSkillBarNum);

	int[] petInfoList_propItemIndex = petInfoList[petInfoListIndex].getPropItemIndex();

	// 宠物资质丹索引
	writeShort(petInfoList_propItemIndex.length);
	int petInfoList_propItemIndexSize = petInfoList_propItemIndex.length;
	int petInfoList_propItemIndexIndex = 0;
	for(petInfoList_propItemIndexIndex=0; petInfoList_propItemIndexIndex<petInfoList_propItemIndexSize; petInfoList_propItemIndexIndex++){
		writeInteger(petInfoList_propItemIndex [ petInfoList_propItemIndexIndex ]);
	}//end

	com.imop.lj.common.model.pet.ShortcutInfo[] petInfoList_shortcutList = petInfoList[petInfoListIndex].getShortcutList();

	// 技能快捷栏信息
	writeShort(petInfoList_shortcutList.length);
	int petInfoList_shortcutListIndex = 0;
	int petInfoList_shortcutListSize = petInfoList_shortcutList.length;
	for(petInfoList_shortcutListIndex=0; petInfoList_shortcutListIndex<petInfoList_shortcutListSize; petInfoList_shortcutListIndex++){

	int petInfoList_shortcutList_shortcutIndex = petInfoList_shortcutList[petInfoList_shortcutListIndex].getShortcutIndex();

	// 快捷栏索引, 默认为-1
	writeInteger(petInfoList_shortcutList_shortcutIndex);

	int petInfoList_shortcutList_skillId = petInfoList_shortcutList[petInfoList_shortcutListIndex].getSkillId();

	// 技能Id
	writeInteger(petInfoList_shortcutList_skillId);
	}
	//end

	long petInfoList_soulLinkPetHorseId = petInfoList[petInfoListIndex].getSoulLinkPetHorseId();

	// 灵魂链接骑宠ID, 0-无灵魂链接
	writeLong(petInfoList_soulLinkPetHorseId);
	}
	//end

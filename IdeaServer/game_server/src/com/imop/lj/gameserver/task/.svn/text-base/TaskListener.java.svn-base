...
import com.imop.lj.gameserver.task.dest.IMeetFixedMapMonsterDest;
...
public class TaskListener<T extends ITaskOwner> {
	...
	private void initAllManager() {
		...
		if (owner instanceof Human) {
			...
			addTaskManager(((Human)owner).getRingTaskManager());
	 ...
	 * @return
	 */
	public int getQuestMeetMonsterPlanId(int mapId) {
		//由于managerList第一个就是主线任务，所以这里是主线优先
		for (AbstractTaskManager atm : getManagerList()) {
			...
			for (AbstractTask task : taskList) {
				...
				for (IQuestDestination dest : dests) {
					//必须是指定遇怪方案的目标类型
					if (!(dest instanceof IMeetFixedMapMonsterDest)) {
						continue;
					}
					
					IMeetFixedMapMonsterDest mmDest = (IMeetFixedMapMonsterDest) dest;
					//如果参数都大于0，则需要按照指定的方案遇怪
					if (mmDest.getMapId() > 0 && mmDest.getMeetMonsterPlanId() > 0
							&& mapId == mmDest.getMapId()) {
						return mmDest.getMeetMonsterPlanId();
					}
				}
			}
		}
	...
	private void onUpdateMindSkillLevel(AbstractTask task, int mindSkillLevel) {
		...
		for (IQuestDestination dest : dests) {
			...
			if (dest.getDestType() == DestType.LEADER_MIND_SKILL_LEVEL_X) {
				LeaderMindSkillLevelXDest tdesc = (LeaderMindSkillLevelXDest) dest;
				if (mindSkillLevel >= tdesc.getNeedLevel()) {
					task.increaseOne(DestType.LEADER_MIND_SKILL_LEVEL_X, dest.getInstKey());
				}
			}
		}
	}

...
public abstract class SkillEffectTemplateVO extends TemplateObject {
	...
	protected int effectWeight;

	/** 同一个互斥组内的效果分组 */
	@ExcelCellBinding(offset = 5)
	protected int effectGroupId;
	...
	protected List<Integer> skillLayerEffectList;
	...
	public void setEffectWeight(int effectWeight) {
		if (effectWeight < 0) {
			throw new TemplateConfigException(this.getSheetName(), this.getId(),
					5, "[同一个互斥组内的效果权重]effectWeight的值不得小于0");
		}
	...
	public void setEffectGroupId(int effectGroupId) {
		if (effectGroupId < 0) {
			throw new TemplateConfigException(this.getSheetName(), this.getId(),
					6, "[同一个互斥组内的效果分组]effectGroupId的值不得小于0");
		}
	...
	public void setBuffOverlapNum(int buffOverlapNum) {
		if (buffOverlapNum < 0) {
			throw new TemplateConfigException(this.getSheetName(), this.getId(),
					7, "[buff叠加层数]buffOverlapNum的值不得小于0");
		}
	...
	public void setBuffRoundNum(int buffRoundNum) {
		if (buffRoundNum < 0) {
			throw new TemplateConfigException(this.getSheetName(), this.getId(),
					9, "[buff持续回合数]buffRoundNum的值不得小于0");
		}
	...
	public void setBuffContinued(int buffContinued) {
		if (buffContinued > 1 || buffContinued < 0) {
			throw new TemplateConfigException(this.getSheetName(), this.getId(),
					10, "[0非持续型，1持续型]buffContinued的值不合法，应为0至1之间");
		}
	...
	public void setBuffGoodBad(int buffGoodBad) {
		if (buffGoodBad > 2 || buffGoodBad < 0) {
			throw new TemplateConfigException(this.getSheetName(), this.getId(),
					11, "[buff好坏（0中性，1好，2坏）]buffGoodBad的值不合法，应为0至2之间");
		}
	...
	public void setBuffTargetLiveFlag(int buffTargetLiveFlag) {
		if (buffTargetLiveFlag > 2 || buffTargetLiveFlag < 0) {
			throw new TemplateConfigException(this.getSheetName(), this.getId(),
					12, "[buff对象存活状态（0活着，1可复活）]buffTargetLiveFlag的值不合法，应为0至2之间");
		}
	...
	public void setCalcTypeId(int calcTypeId) {
		if (calcTypeId > 1 || calcTypeId < 0) {
			throw new TemplateConfigException(this.getSheetName(), this.getId(),
					13, "[是否仙符效果（0否，1是）]calcTypeId的值不合法，应为0至1之间");
		}
	...
	public void setNearby(int nearby) {
		if (nearby > 1 || nearby < 0) {
			throw new TemplateConfigException(this.getSheetName(), this.getId(),
					15, "[是否近身，0否，1是]nearby的值不合法，应为0至1之间");
		}
		this.nearby = nearby;
	}
	...
	public void setTargetSelect(int targetSelect) {
		if (targetSelect > 1 || targetSelect < 0) {
			throw new TemplateConfigException(this.getSheetName(), this.getId(),
					16, "[是否可以选择目标，0否，1是]targetSelect的值不合法，应为0至1之间");
		}
	...
	public void setTargetSelf(int targetSelf) {
		if (targetSelf > 1 || targetSelf < 0) {
			throw new TemplateConfigException(this.getSheetName(), this.getId(),
					17, "[是否使用自身目标，1是，0否]targetSelf的值不合法，应为0至1之间");
		}
	...
	public void setTargetRangeTypeId(int targetRangeTypeId) {
		if (targetRangeTypeId < 0) {
			throw new TemplateConfigException(this.getSheetName(), this.getId(),
					19, "[范围类型]targetRangeTypeId的值不得小于0");
		}
	...
	public void setIsNegativeFlag(int isNegativeFlag) {
		if (isNegativeFlag > 1 || isNegativeFlag < 0) {
			throw new TemplateConfigException(this.getSheetName(), this.getId(),
					21, "[效果数值是否为负数]isNegativeFlag的值不合法，应为0至1之间");
		}
	...
	public void setEffectValueTypeId(int effectValueTypeId) {
		if (effectValueTypeId < 0) {
			throw new TemplateConfigException(this.getSheetName(), this.getId(),
					22, "[效果数值类型]effectValueTypeId的值不得小于0");
		}
	...
	public void setExtraCoef5(int extraCoef5) {
		if (extraCoef5 < 0) {
			throw new TemplateConfigException(this.getSheetName(), this.getId(),
					32, "[加buff概率（扩大1000倍）]extraCoef5的值不得小于0");
		}
	...
	public void setAttackTypeId(int attackTypeId) {
		if (attackTypeId > 2 || attackTypeId < 0) {
			throw new TemplateConfigException(this.getSheetName(), this.getId(),
					33, "[读取物理攻击力还是法术攻击力(0默认,1物理,2法术)]attackTypeId的值不合法，应为0至2之间");
		}
		this.attackTypeId = attackTypeId;
	}
	
	public List<Integer> getSkillLayerEffectList() {
		return this.skillLayerEffectList;
	}

	public void setSkillLayerEffectList(List<Integer> skillLayerEffectList) {
		if (skillLayerEffectList == null) {
			throw new TemplateConfigException(this.getSheetName(), this.getId(),
					34, "[层数效果数值列表]skillLayerEffectList不可以为空");
		}	
		this.skillLayerEffectList = skillLayerEffectList;
	}
	

	@Override
	public String toString() {
		return "SkillEffectTemplateVO[effectTypeId=" + effectTypeId + ",buffMutex=" + buffMutex + ",effectLevel=" + effectLevel + ",effectWeight=" + effectWeight + ",effectGroupId=" + effectGroupId + ",buffOverlapNum=" + buffOverlapNum + ",buffTypeId=" + buffTypeId + ",buffRoundNum=" + buffRoundNum + ",buffContinued=" + buffContinued + ",buffGoodBad=" + buffGoodBad + ",buffTargetLiveFlag=" + buffTargetLiveFlag + ",calcTypeId=" + calcTypeId + ",cdRound=" + cdRound + ",nearby=" + nearby + ",targetSelect=" + targetSelect + ",targetSelf=" + targetSelf + ",targetTypeId=" + targetTypeId + ",targetRangeTypeId=" + targetRangeTypeId + ",targetNum=" + targetNum + ",isNegativeFlag=" + isNegativeFlag + ",effectValueTypeId=" + effectValueTypeId + ",valueTypeId=" + valueTypeId + ",valueCoef=" + valueCoef + ",valueBase=" + valueBase + ",valueAdd=" + valueAdd + ",mindCoef=" + mindCoef + ",extraCoef1=" + extraCoef1 + ",extraCoef2=" + extraCoef2 + ",extraCoef3=" + extraCoef3 + ",extraCoef4=" + extraCoef4 + ",extraCoef5=" + extraCoef5 + ",attackTypeId=" + attackTypeId + ",skillLayerEffectList=" + skillLayerEffectList + ",]";

...
public class PetIslandNoticeMessage extends AbstractActivityMessage {
	
	private ActivityTemplate template;
	
	public PetIslandNoticeMessage(ActivityTemplate template){
	...
	public void execute() {
		...
		if(activityService.setActivityState(activityId, ActivityState.NOT_OPEN)) {
			// 提醒阶段的处理
			Globals.getPetIslandService().handleActivityNoticeMsg(activityService.getActivity(activityId));
		}
	}

...
//				info.setLimitPurchase(tmpl.isLimitPurchase() ? ResultType.SUCC.getIndex() : ResultType.FAIL.getIndex());
	...
	public void handleBuyNormalItem(Human human, int mallItemId, int count, boolean flag){
		...
		boolean isBind = true;
		//如果货币有对应的绑定状态，则以货币的为准
		if (currency.getItemBindType() != null) {
			isBind = currency.getItemBindType() == BindType.BIND;
		} else {
			//以道具模板的为准
			isBind = Globals.getTemplateCacheService().get(mallItem.getSellItem().getId(), ItemTemplate.class).isBind();
		}
		...
		ItemParam ip = new ItemParam(mallItem.getSellItem().getId(), buyCount, isBind);
		...
		if (null != costDetail) {
			human.getInventory().addItem(mallItem.getSellItem().getId(), buyCount, ItemGenLogReason.MALL_BUY_ITEM, ItemGenLogReason.MALL_BUY_ITEM.getReasonText(), true, true, 
				currency, costDetail.getTotalCost(), costDetail.getActualCost(), isBind);
		} else {
			human.getInventory().addItem(mallItem.getSellItem().getId(), buyCount, ItemGenLogReason.MALL_BUY_ITEM, ItemGenLogReason.MALL_BUY_ITEM.getReasonText(), true, true, 
					currency, num, num, isBind);
		}
	 ...
	 * @param mallItemId
	 */
	public void handleExchangeItem(Human human, int mallItemId, int count){
		MallNormalItemTemplate mallItem = Globals.getTemplateCacheService().get(mallItemId, MallNormalItemTemplate.class);
		if(mallItem == null){
			// 购买物品不存在
			Loggers.mallLogger.error("MallService.handleBuyNormalItem mallItemId = " + mallItemId + " does not exist!!");
		...
		//是否是兑换物品
		boolean isExchangeItem = mallItem.getCatalogId() == MallDef.CatalogType.EXCHANGE.getIndex();
		if (!isExchangeItem) {
			return;
		}
		
		//兑换物品道具Id和数量
		int costItemId = mallItem.getExchangeDisItem().getId();
		int costItemNum = mallItem.getExchangeDisNum() * count;
		//兑换物品是否充足
		if (!human.getInventory().hasItemByTmplId(costItemId, costItemNum)){ 
			human.sendErrorMessage(LangConstants.EXCHANGE_ITEM_IS_NOT_ENOUGH);
			return;
		}
		
		int buyItemId = mallItem.getSellItem().getId();
		int buyCount = count * mallItem.getSellItemNum();
		int buyNotBindMax = human.getInventory().getItemCountByTmplId(costItemId, false) / mallItem.getExchangeDisNum();
		...
		if (buyNotBindMax >= buyCount) {
			buyNotBindNum = buyCount;
			buyBindNum = 0;
		} else {
			buyNotBindNum = buyNotBindMax;
			buyBindNum = buyCount - buyNotBindMax;
		}
		
		List<ItemParam> ipList = new ArrayList<ItemParam>();
		if (buyNotBindNum > 0) {
			ipList.add(new ItemParam(buyItemId, buyNotBindNum, false));
		}
		if (buyBindNum > 0) {
			ipList.add(new ItemParam(buyItemId, buyBindNum, true));
		}
		//检查背包空间
		if (!human.getInventory().checkSpace(ipList, false)) {
			human.sendBoxMessage(LangConstants.MS_BAG_IS_NOT_ENOUGH);
		...
		//扣除兑换物品，非绑定优先
		Collection<Item> lst = human.getInventory().removeItem(costItemId, costItemNum, LogReasons.ItemLogReason.EXCHANGE_MALL_ITEM_COST, 
				LogUtils.genReasonText(LogReasons.ItemLogReason.EXCHANGE_MALL_ITEM_COST, costItemId), false);
		...
		//给非绑定道具
		if (buyNotBindNum > 0) {
			human.getInventory().addItem(buyItemId, buyNotBindNum, ItemGenLogReason.MALL_BUY_ITEM, 
					ItemGenLogReason.MALL_BUY_ITEM.getReasonText(), false, true);
		}
		//给绑定道具
		if (buyBindNum > 0) {
			human.getInventory().addItem(buyItemId, buyBindNum, ItemGenLogReason.MALL_BUY_ITEM, 
					ItemGenLogReason.MALL_BUY_ITEM.getReasonText(), true, true);
		}
		
		this.sendCloseBuyItemPanel(human);
		// 记日志
		String param = LogUtils.genReasonText(MallLogReason.EXCHANGE_ITEM, mallItem.getExchangeDisItem().getId(), mallItem.getExchangeDisNum(),
				count, mallItemId, mallItem.getSellItem().getId(), buyCount);
		this.sendMallLog(human, MallLogReason.EXCHANGE_ITEM, param);
	...
	public void handleBuyTimeLimitItem(Human human, String queueUUID, int mallItemId, int count, boolean flag){
		...
		if (currency.getItemBindType() != null) {
			...
			//以道具模板的为准
			isBind = Globals.getTemplateCacheService().get(item.getSellItem().getId(), ItemTemplate.class).isBind();
		}
		
		// 背包空间不足
		int buyCount = item.getSellItemNum() * count;
		
		ItemParam ip = new ItemParam(item.getSellItem().getId(), buyCount, isBind);
		List<ItemParam> ipList = new ArrayList<ItemParam>();
		ipList.add(ip);
		if(!human.getInventory().checkSpace(ipList, false)){
			this.sendCloseBuyItemPanel(human);

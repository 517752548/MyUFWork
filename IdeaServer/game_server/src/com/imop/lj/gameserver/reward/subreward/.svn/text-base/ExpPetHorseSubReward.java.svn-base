 ...
 * 
 */
public class ExpPetHorseSubReward extends AbstractExpSubReward {
	
	public ExpPetHorseSubReward(Reward reward) {
		super(reward, RewardType.REWARD_PET_HORSE_EXP);
	}
	...
	public static RewardParam createRewardParam(int exp){
		...
		param.setRewardType(RewardType.REWARD_PET_HORSE_EXP);
		param.setParam2(exp);
		return param;
	}
	...
	public boolean giveReward(Human human, boolean needNotify) {
		Globals.getPetService().addExpForPetHorse(human, exp, reward.getReasonType().getPetExpLogReason(), reward.getParams(), needNotify);
		return true;
	}

	@Override
	public ISubReward createEmptySubReward(Reward reward) {
		return new ExpPetHorseSubReward(reward);
	}

...
public class ShoulderBag extends CommonBag {
	...
	private int addToOverlapSlot(ItemTemplate template, int count, ItemLogReason reason, String reasonDetail, String genKey, List<Item> updateList,
		...
		for (Item item : items) {
			...
			if (Globals.getItemService().canOverlapOn(item, template.getId())
					&& item.isBind() == isBind) {//新增考虑绑定状态，相同状态才能叠加
	...
	public Collection<Item> removeItem(int templateId, int count, ItemLogReason reason, String detailReason, boolean bindFirst) {
		...
		Collection<Item> itemColl = directRemove(matchItems, count, reason, detailReason, bindFirst);
	 ...
	 * @return 删除的所有item
	 */
	private Collection<Item> directRemove(List<Item> items, int count, ItemLogReason reason, String detailReason, boolean bindFirst) {
		//默认先扣除绑定的，按叠加个数从小到大排列，先从叠加小的上面删
		if (bindFirst) {
			Collections.sort(items, overlapCompBind);
		} else {
			Collections.sort(items, overlapCompBind);
		}

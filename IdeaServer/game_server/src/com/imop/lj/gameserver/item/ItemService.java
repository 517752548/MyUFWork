...
//	}
	 ...
	 * @return
	 */
	public Item newActivatedInstance(Human owner, ItemTemplate template, BagType bagType, int bagIndex, boolean isBind) {
		Item item = null;
//		if (template.isEquipment()) {
//			item = equipGenerator.generateActivedEquip(owner, (EquipItemTemplate) template, bagType, bagIndex);
//		} else {
			item = Item.newActivatedInstance(owner, template, bagType, bagIndex, isBind);
//		}
		return item;
	}
	 ...
	 * @return
	 */
	public Item newActivatedInstance(Human owner, ItemTemplate template, BagType bagType, int bagIndex, int overlap, boolean isBind) {
		Item item = null;
//		if (template.isEquipment()) {
//			item = equipGenerator.generateActivedEquip(owner, (EquipItemTemplate) template, bagType, bagIndex);
//		} else {
			item = Item.newActivatedInstance(owner, template, bagType, bagIndex, overlap, isBind);
//		}
		return item;
	}
	...
	public Item newActivatedInstanceForGM(Human owner, ItemTemplate template, BagType bagType, int bagIndex, 
			boolean isBind, int[] attrA, int[] attrB, int...params) {
		Item item = null;
//		if (template.isEquipment()) {
//			item = equipGenerator.generateActivedEquipForGM(owner, (EquipItemTemplate) template, bagType, bagIndex, attrA, attrB, params);
//		} else {
			item = Item.newActivatedInstanceForGM(owner, template, bagType, bagIndex, isBind, attrA, attrB, params);
//		}
		return item;
	}
	 ...
	 * @return
	 */
	public Item newDeactiveInstance(Human owner, ItemTemplate template, BagType bagType, int bagIndex, boolean isBind) {
//		if (template.isEquipment()) {
//			Item item = equipGenerator.generateDeactivedEquip(owner, (EquipItemTemplate) template, bagType, bagIndex);
//			return item;
//		}
			return Item.newDeactivedInstance(owner, template, bagType, bagIndex, isBind);
	...
	public void openStoreBag(Human human) {
		...
		Collection<Item> removeItem = human.getInventory().removeItem(templateId, needItemNum, ItemLogReason.OPEN_BAG,
				ItemLogReason.OPEN_BAG.getReasonText(), true);
		// 开格子
	 ...
	 * @param itemId
	 * @param count
	 * @param isBind 是否绑定，如果道具本身是绑定的则会变为绑定
	 * @param attrA
	 * @param attrB
	 * @param fixedParams
	 */
	public Item addItemByParams(boolean isGM, ItemGenLogReason genReason, String genDetailReason, 
			ItemLogReason reason, Human human, int itemId, int count, boolean isBind, int[] attrA, int[] attrB, Object...fixedParams) {
		// 物品模板
...
//			}
			...
			if (null != addBag && null != emptyItem) {
				if (isGM) {
					newItem = Item.newActivatedInstanceForGM(human, template, addBag.getBagType(), emptyItem.getIndex(),
							isBind, attrA, attrB, fixedParams);
				} else {
				...
				//没有空的格子了，直接返回
				//记录丢弃日志
				String p = attrA + ";" + attrB + ";" + fixedParams;
	...
	/**
	 * 添加到属性的道具，绑定状态由外层自主决定
	 * @param genReason
	 * @param genDetailReason
	 * @param reason
	 * @param human
	 * @param itemId
	 * @param itemFeature
	 * @param isBind 这里不考虑道具模板本身的绑定状态，外层自己决定是否绑定的
	 * @return
	 */
	public Item addItemWithFeature(ItemGenLogReason genReason, String genDetailReason, 
			ItemLogReason reason, Human human, int itemId, ItemFeature itemFeature, boolean isBind) {
		// 物品模板
		ItemTemplate template = Globals.getTemplateCacheService().get(itemId, ItemTemplate.class);
		...
		if (null != addBag && null != emptyItem) {
			newItem = Item.newActivatedInstanceWithFeature(human, template, addBag.getBagType(), emptyItem.getIndex(), itemFeature, isBind);
			newItem.setModified();
			bag.putItem(newItem);

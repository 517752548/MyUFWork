...
public class PetHelper {
	...
	private static void initPetBase(Pet pet, int generalTmplId, long dbId) {
		...
		pet.setSkillBarNum(Globals.getGameConstants().getPetDefaultSkillBarNum());
	 ...
	 * @return
	 */
	public static FightUnit createFightUnit(Pet pet, int battleIndex, String id, boolean isAttacker, PetHorse fightPetHorse) {
		...
		if(fightUnit.isLeader()){
			fightUnit.setNormalAttackTypeId(PetAttackType.STRENGTH.getIndex());
		}
		...
		//Map<技能Id,技能等级>
		Map<Integer, Integer> phSkillMap = Maps.newHashMap();
		if(fightPetHorse != null){
			for (PetSkillInfo info : fightPetHorse.getSkillMap().values()) {
				if(!info.isTalent()){
					continue;
				}
				phSkillMap.put(info.getSkillId(), info.getLevel());
			}
		}
		List<Integer> petHorseAddSkillLst = Lists.newArrayList();
		Map<Integer, Integer> petHorseAddMap = Maps.newHashMap();
...
//			fightUnit.setMindLevel(pet.getOwner().getMainSkillLevel());
			...
			for (PetSkillInfo psi : pl.getSkillMap().values()) {
				FightUnitSkillInfo info = new FightUnitSkillInfo(psi.getSkillId(), psi.getLevel(), psi.getLayer());
				//镶嵌的仙符效果
				...
				petHorseAddSkillLst = Globals.getTemplateCacheService().getBattleTemplateCache().getPetHorseAddSkillId(psi.getSkillId());
				for (Integer petHorseAddSkillId : petHorseAddSkillLst) {
					if(phSkillMap.containsKey(petHorseAddSkillId)){
						petHorseAddMap.put(petHorseAddSkillId, psi.getSkillId());
						fightUnit.setPetHorseAddMap(petHorseAddMap);
						fightUnit.setPetHorseSkillLevel(phSkillMap.get(petHorseAddSkillId));
					}
				}
			}
			
			//主将有捕捉技能
			FightUnitSkillInfo cInfo = new FightUnitSkillInfo(BattleDef.CATCH_PET_SKILL_ID, BattleDef.DEFAULT_SKILL_LEVEL, BattleDef.DEFAULT_SUB_SKILL_LAYER);
			fightUnit.addSkillOnInit(cInfo);
			
			//主将有逃跑技能
			FightUnitSkillInfo eInfo = new FightUnitSkillInfo(BattleDef.ESCAPE_SKILL_ID, BattleDef.DEFAULT_SKILL_LEVEL, BattleDef.DEFAULT_SUB_SKILL_LAYER);
			fightUnit.addSkillOnInit(eInfo);
			
			//主将有召唤技能
			FightUnitSkillInfo sInfo = new FightUnitSkillInfo(BattleDef.SUMMON_PET_SKILL_ID, BattleDef.DEFAULT_SKILL_LEVEL, BattleDef.DEFAULT_SUB_SKILL_LAYER);
			fightUnit.addSkillOnInit(sInfo);
			
			//主将和宠物都有防御技能
			FightUnitSkillInfo defenceInfoLeader = new FightUnitSkillInfo(BattleDef.DEFENCE_SKILL_ID, BattleDef.DEFAULT_SKILL_LEVEL, BattleDef.DEFAULT_SUB_SKILL_LAYER);
			fightUnit.addSkillOnInit(defenceInfoLeader);
			
			//主将和宠物都有嗑药技能
			FightUnitSkillInfo useDrugsInfoLeader = new FightUnitSkillInfo(BattleDef.USEDRUGS_SKILL_ID, BattleDef.DEFAULT_SKILL_LEVEL, BattleDef.DEFAULT_SUB_SKILL_LAYER);
		...
		case FRIEND:
			for (SkillItem esi : petTpl.getValidSkillList()) {
				FightUnitSkillInfo info = new FightUnitSkillInfo(esi.getSkillId(), PetDef.DEFAULT_SKILL_LEVEL, PetDef.DEFAULT_SKILL_LAYER,
		...
		List<IEffect> normal = battleTemplateCache.getSkillEffectsNoMind(BattleDef.NORMAL_ATTACK_SKILL_ID, 
				BattleDef.DEFAULT_SKILL_LEVEL, BattleDef.DEFAULT_SUB_SKILL_LAYER);
	 ...
	 * @param id
	 * @param isAttacker
	 * @param fightPetHorses
	 * @param horseBattle
	 * @param additionSkillIds
	 * @return
	 */
	public static FightUnit createFightUnitByPetSnap(UserSnap userSnap, PetBattleSnap petSnap, int battleIndex, String id, boolean isAttacker, PetBattleSnap fightPetHorse) {
...
//			fightUnit.setMindLevel(userSnap.getMindLevel());
			...
			int leaderSkillLayer = userSnap.getAutoSkillLayer();
			FightUnitSkillInfo info = new FightUnitSkillInfo(leaderSkillId, leaderSkillLevel, leaderSkillLayer);
			//镶嵌的仙符效果
			...
			List<Integer> petHorseAddSkillLst = Globals.getTemplateCacheService().getBattleTemplateCache().getPetHorseAddSkillId(leaderSkillId);
			for (Integer petHorseAddSkillId : petHorseAddSkillLst) {
				if(phSkillMap.containsKey(petHorseAddSkillId)){
					Map<Integer, Integer> petHorseAddMap = Maps.newHashMap();
					petHorseAddMap.put(petHorseAddSkillId, leaderSkillId);
					fightUnit.setPetHorseAddMap(petHorseAddMap);
					fightUnit.setPetHorseSkillLevel(phSkillMap.get(petHorseAddSkillId));
				}
			}
		...
		case PET:
			...
			List<Integer> petHorseAddSkillLst1 = Globals.getTemplateCacheService().getBattleTemplateCache().getPetHorseAddSkillId(petSkillId);
			for (Integer petHorseAddSkillId : petHorseAddSkillLst1) {
				if(phSkillMap.containsKey(petHorseAddSkillId)){
					Map<Integer, Integer> petHorseAddMap = Maps.newHashMap();
					petHorseAddMap.put(petHorseAddSkillId, petSkillId);
					fightUnit.setPetHorseAddMap(petHorseAddMap);
					fightUnit.setPetHorseSkillLevel(phSkillMap.get(petHorseAddSkillId));
				}
			}
		...
		case FRIEND:
			for (SkillItem esi : petTpl.getValidSkillList()) {
				FightUnitSkillInfo fInfo = new FightUnitSkillInfo(esi.getSkillId(), PetDef.DEFAULT_SKILL_LEVEL, PetDef.DEFAULT_SKILL_LAYER,
						esi.getWeight(), esi.getCdRound());
...
//	}
	...
	public static FightUnit createFightUnitForArena(Pet pet, int battleIndex, String id, boolean isAttacker) {
		FightUnit fu = createFightUnit(pet, battleIndex, id, isAttacker, null);
		//设置技能的冷却回合数
	...
	public static FightUnit createFightUnitByPetSnapForArena(UserSnap userSnap, PetBattleSnap petSnap, int battleIndex, String id, boolean isAttacker) {
		FightUnit fu = createFightUnitByPetSnap(userSnap, petSnap, battleIndex, id, isAttacker, null);
		//设置技能的冷却回合数

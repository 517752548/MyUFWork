...
public class GCPetFriendInfo extends GCMessage{
	...
	// 层数
	int _skillList_layer = readInteger();
	//end
	_skillList[skillListIndex].setLayer (_skillList_layer);

	// 熟练度
	long _skillList_proficiency = readLong();
	//end
	_skillList[skillListIndex].setProficiency (_skillList_proficiency);
	}
	//end
	...
	int skillList_layer = skillList[skillListIndex].getLayer();

	// 层数
	writeInteger(skillList_layer);

	long skillList_proficiency = skillList[skillListIndex].getProficiency();

	// 熟练度
	writeLong(skillList_proficiency);
	}
	//end

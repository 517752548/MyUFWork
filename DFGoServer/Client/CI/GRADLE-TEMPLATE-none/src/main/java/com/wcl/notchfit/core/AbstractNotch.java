...
public abstract class AbstractNotch implements INotch{
    ...
    protected int[] getNotchSize_P(Activity activity){
        ...
        if(boundingRects.size() != 0){
            ...
            notchSize[0] = rect.height(); //TODO 这里要0放置高度  1放置宽度，因为这套库O版的刘海屏高度就是P版的宽度 P版是横向为基准 这也许跟游戏是横向还是纵向有关系，总之对于横向的游戏 这里要反过来
            notchSize[1] = rect.width();
            LogUtils.i(notchProperty.getManufacturer() + " P notch size: " + "width> " + notchSize[0] + " height>" + notchSize[1]);
        }
        return notchSize;
    }
}

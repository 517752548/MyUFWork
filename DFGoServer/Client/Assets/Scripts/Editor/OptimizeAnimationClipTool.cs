...
public class OptimizeAnimationClipTool
{
	...
	[MenuItem("LetUs/Release Preprocess/优化Animation")]
	...
	static List<AnimationOpt> FindAnims()
	{
		...
		{
			//没有选择就是全部遍历
			List<string> animFiles = BuildScript.GetFilesPathList("Res", ".anim", new string[] { ".meta" });
			for (int i = 0; i < animFiles.Count; i++)
			{
				AnimationOpt animopt = _GetNewAOpt(animFiles[i]);
				if (animopt != null)
					assets.Add(animopt);
			}
		}

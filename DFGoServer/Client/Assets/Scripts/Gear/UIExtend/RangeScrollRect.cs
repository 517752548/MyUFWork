...
public class RangeScrollRect : UIBehaviour, IInitializePotentialDragHandler, IBeginDragHandler, IEndDragHandler,
    IDragHandler, IScrollHandler, ICanvasElement
{
    public RectTransform template;
    [SerializeField] private RectTransform m_Content;
    public Transform[] childs;
    public float[] RangePosition;
    ...
    public UnityEvent OnBegionDragEvent;
    public UnityEvent OnEndDragEvent;
    public UnityEvent<Vector2> OnValueChangedEvent;
    public UnityEvent OnEndMoveEvent;
    private UnityEvent<int> _OnRangeChangedEvent;

    public class RangeScrollRectEvent : UnityEvent<int>
    {
    }

    public RangeScrollRectEvent OnRangeChangedEvent = new RangeScrollRectEvent();
    public RangeScrollRectEvent OnRangeWillChangeEvent = new RangeScrollRectEvent();

    [SerializeField] private bool m_Inertia = true;

    public ScrollRect.MovementType m_MovementType = ScrollRect.MovementType.Clamped;
    ...
    private Vector2 m_rangePosition = Vector2.zero;
    ...
    {
        inertia = false;
        currentRangeIndex = defaultRange;
        childs = new Transform[Range];
        RangePosition = new float[Range];
        float sizeW = template.rect.size.x;
        float sizeH = template.rect.size.y;
        template.anchorMax = Vector2.one * 0.5f;
        template.anchorMin = Vector2.one * 0.5f;
        template.sizeDelta = new Vector2(sizeW, sizeH);
        float halfContentSizeDelta = 0;
        if (horizontal)
        {
            content.sizeDelta = new Vector2(sizeW * Range, content.sizeDelta.y);
            halfContentSizeDelta = 0.5f * content.sizeDelta.x;
        }
        else
        {
            content.sizeDelta = new Vector2(content.sizeDelta.x, sizeH * Range);
            halfContentSizeDelta = 0.5f * content.sizeDelta.y;
        }
        for (int i = 0; i < Range; i++)
        {
            GameObject obj = Instantiate(template.gameObject, content, false);
            obj.name = "node" + (i + 1).ToString();
            childs[i] = obj.transform;
            if (horizontal)
            {
                obj.transform.localPosition = new Vector3(sizeW * i + 0.5f * sizeW - halfContentSizeDelta, obj.transform.localPosition.y, 0);
                RangePosition[i] = -sizeW * i + halfContentSizeDelta - 0.5f * sizeW;
            }
            else
            {
                obj.transform.localPosition = new Vector3(obj.transform.localPosition.x, sizeH * i, 0);
                RangePosition[i] = -sizeH * i;
            }
        }
        ...
        if (horizontal)
        {
            content.localPosition = new Vector3(RangePosition[defaultRange - 1], content.localPosition.y, 0);
        }
        else
        {
            content.localPosition = new Vector3(content.localPosition.x, RangePosition[defaultRange - 1], 0);
        }

        OnRangeChangedEvent?.Invoke(defaultRange);
        inertia = true;
    }
    ...
    protected virtual void LateUpdate()
    {
        ...
        if (RangePosition == null || RangePosition.Length == 0)
            return;
        EnsureLayoutHasRebuilt();
        UpdateBounds();
        ...
        {
            //手动滑动
            if (!m_Dragging && !PositionInRangepostion() && !MovingToIndex && m_Inertia)
            {
                int targetpos = SubRangeIndex();
                m_rangePosition = new Vector2(RangePosition[targetpos], m_Content.localPosition.y);
                Vector3 newVelocity = (m_rangePosition - m_Content.anchoredPosition) / deltaTime;
                m_Velocity = Vector3.Lerp(Vector3.zero, newVelocity, deltaTime * 5);
            }

            //点击移动
            if (!m_Dragging && MovingToIndex)
            {
                int targetpos = targetMoveIndex;
                m_rangePosition = new Vector2(RangePosition[targetpos], m_Content.localPosition.y);
                float X_Distace = m_rangePosition.x - m_Content.anchoredPosition.x;
                Vector3 newVelocity = (m_rangePosition - m_Content.anchoredPosition) / deltaTime * 5;
                if (Mathf.Abs(X_Distace) <= 1)
                    newVelocity = Vector3.zero;
                m_Velocity = Vector3.Lerp(Vector3.zero, newVelocity, deltaTime * 2);
                SubRangeIndex();
                for (int i = 0; i < 2; i++)
                {
                    if (Mathf.Abs(m_Velocity[i]) < 1)
                        m_Velocity[i] = 0;
                }
            ...
            m_Content.anchoredPosition != m_PrevPosition)
        {
            ...
            OnValueChangedEvent?.Invoke(normalizedPosition);
            UpdatePrevData();
        }
    ...
    private bool MovingToIndex = false;
    private int targetMoveIndex = 0;
    ...
    public void MoveToIndex(int index)
    {
        ...
        {
            MovingToIndex = true;
            targetMoveIndex = index;
            currentRangeIndex = index;
            OnRangeChangedEvent?.Invoke(targetMoveIndex);
        }
    }

    private bool PositionInRangepostion()
    {
        for (int i = 0; i < RangePosition.Length; i++)
        {
            if (Mathf.Abs(Mathf.Abs(content.localPosition.x) - Mathf.Abs(RangePosition[i])) < 1f)
            {
                return true;
            }
        }
    ...
    internal static Vector2 InternalCalculateOffset(ref Bounds viewBounds, ref Bounds contentBounds, bool horizontal,
        bool vertical, ScrollRect.MovementType movementType, ref Vector2 delta)
    {
    ...
    private void PreSubRangeIndex()
    {
        float distance = 99999;
        int index = currentRangeIndex;
        for (int i = 0; i < RangePosition.Length; i++)
        {
            float absdistance = Mathf.Abs(Mathf.Abs(content.localPosition.x) - Mathf.Abs(RangePosition[i]));
        ...
        if (index != currentRangeIndex)
        {
            OnRangeWillChangeEvent?.Invoke(index);
        }
    }
    ...
    private int SubRangeIndex()
    {
        ...
        for (int i = 0; i < RangePosition.Length; i++)
        {
            float absdistance = Mathf.Abs(content.localPosition.x - RangePosition[i]);
            if (absdistance < distance)
            {
                distance = absdistance;
                index = i;
        ...
        if (index != currentRangeIndex)
        {
            ...
            if (!MovingToIndex)
            {
                OnRangeChangedEvent?.Invoke(index);
            }
        }
        return index;
    }

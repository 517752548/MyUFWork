//this source code was auto-generated by tolua#, do not modify it
using System;
using LuaInterface;

public class UnityEngine_AI_NavMeshWrap
{
	public static void Register(LuaState L)
	{
		L.BeginStaticLibs("NavMesh");
		L.RegFunction("Raycast", new LuaCSFunction(Raycast));
		L.RegFunction("CalculatePath", new LuaCSFunction(CalculatePath));
		L.RegFunction("FindClosestEdge", new LuaCSFunction(FindClosestEdge));
		L.RegFunction("SamplePosition", new LuaCSFunction(SamplePosition));
		L.RegFunction("SetAreaCost", new LuaCSFunction(SetAreaCost));
		L.RegFunction("GetAreaCost", new LuaCSFunction(GetAreaCost));
		L.RegFunction("GetAreaFromName", new LuaCSFunction(GetAreaFromName));
		L.RegFunction("CalculateTriangulation", new LuaCSFunction(CalculateTriangulation));
		L.RegFunction("AddNavMeshData", new LuaCSFunction(AddNavMeshData));
		L.RegFunction("RemoveNavMeshData", new LuaCSFunction(RemoveNavMeshData));
		L.RegFunction("AddLink", new LuaCSFunction(AddLink));
		L.RegFunction("RemoveLink", new LuaCSFunction(RemoveLink));
		L.RegFunction("CreateSettings", new LuaCSFunction(CreateSettings));
		L.RegFunction("RemoveSettings", new LuaCSFunction(RemoveSettings));
		L.RegFunction("GetSettingsByID", new LuaCSFunction(GetSettingsByID));
		L.RegFunction("GetSettingsCount", new LuaCSFunction(GetSettingsCount));
		L.RegFunction("GetSettingsByIndex", new LuaCSFunction(GetSettingsByIndex));
		L.RegFunction("GetSettingsNameFromID", new LuaCSFunction(GetSettingsNameFromID));
		L.RegFunction("RemoveAllNavMeshData", new LuaCSFunction(RemoveAllNavMeshData));
		L.RegConstant("AllAreas", -1);
		L.RegVar("onPreUpdate", new LuaCSFunction(get_onPreUpdate), new LuaCSFunction(set_onPreUpdate));
		L.RegVar("avoidancePredictionTime", new LuaCSFunction(get_avoidancePredictionTime), new LuaCSFunction(set_avoidancePredictionTime));
		L.RegVar("pathfindingIterationsPerFrame", new LuaCSFunction(get_pathfindingIterationsPerFrame), new LuaCSFunction(set_pathfindingIterationsPerFrame));
		L.RegFunction("OnNavMeshPreUpdate", new LuaCSFunction(UnityEngine_AI_NavMesh_OnNavMeshPreUpdate));
		L.EndStaticLibs();
	}

	[MonoPInvokeCallbackAttribute(typeof(LuaCSFunction))]
	static int Raycast(IntPtr L)
	{
		try
		{
			int count = LuaDLL.lua_gettop(L);

			if (count == 4 && TypeChecker.CheckTypes<int>(L, 4))
			{
				UnityEngine.Vector3 arg0 = ToLua.ToVector3(L, 1);
				UnityEngine.Vector3 arg1 = ToLua.ToVector3(L, 2);
				UnityEngine.AI.NavMeshHit arg2;
				int arg3 = (int)LuaDLL.lua_tointeger(L, 4);
				bool o = UnityEngine.AI.NavMesh.Raycast(arg0, arg1, out arg2, arg3);
				LuaDLL.lua_pushboolean(L, o);
				ToLua.PushValue(L, arg2);
				return 2;
			}
			else if (count == 4 && TypeChecker.CheckTypes<UnityEngine.AI.NavMeshQueryFilter>(L, 4))
			{
				UnityEngine.Vector3 arg0 = ToLua.ToVector3(L, 1);
				UnityEngine.Vector3 arg1 = ToLua.ToVector3(L, 2);
				UnityEngine.AI.NavMeshHit arg2;
				UnityEngine.AI.NavMeshQueryFilter arg3 = StackTraits<UnityEngine.AI.NavMeshQueryFilter>.To(L, 4);
				bool o = UnityEngine.AI.NavMesh.Raycast(arg0, arg1, out arg2, arg3);
				LuaDLL.lua_pushboolean(L, o);
				ToLua.PushValue(L, arg2);
				return 2;
			}
			else
			{
				return LuaDLL.luaL_throw(L, "invalid arguments to method: UnityEngine.AI.NavMesh.Raycast");
			}
		}
		catch (Exception e)
		{
			return LuaDLL.toluaL_exception(L, e);
		}
	}

	[MonoPInvokeCallbackAttribute(typeof(LuaCSFunction))]
	static int CalculatePath(IntPtr L)
	{
		try
		{
			int count = LuaDLL.lua_gettop(L);

			if (count == 4 && TypeChecker.CheckTypes<int, UnityEngine.AI.NavMeshPath>(L, 3))
			{
				UnityEngine.Vector3 arg0 = ToLua.ToVector3(L, 1);
				UnityEngine.Vector3 arg1 = ToLua.ToVector3(L, 2);
				int arg2 = (int)LuaDLL.lua_tointeger(L, 3);
				UnityEngine.AI.NavMeshPath arg3 = (UnityEngine.AI.NavMeshPath)ToLua.ToObject(L, 4);
				bool o = UnityEngine.AI.NavMesh.CalculatePath(arg0, arg1, arg2, arg3);
				LuaDLL.lua_pushboolean(L, o);
				return 1;
			}
			else if (count == 4 && TypeChecker.CheckTypes<UnityEngine.AI.NavMeshQueryFilter, UnityEngine.AI.NavMeshPath>(L, 3))
			{
				UnityEngine.Vector3 arg0 = ToLua.ToVector3(L, 1);
				UnityEngine.Vector3 arg1 = ToLua.ToVector3(L, 2);
				UnityEngine.AI.NavMeshQueryFilter arg2 = StackTraits<UnityEngine.AI.NavMeshQueryFilter>.To(L, 3);
				UnityEngine.AI.NavMeshPath arg3 = (UnityEngine.AI.NavMeshPath)ToLua.ToObject(L, 4);
				bool o = UnityEngine.AI.NavMesh.CalculatePath(arg0, arg1, arg2, arg3);
				LuaDLL.lua_pushboolean(L, o);
				return 1;
			}
			else
			{
				return LuaDLL.luaL_throw(L, "invalid arguments to method: UnityEngine.AI.NavMesh.CalculatePath");
			}
		}
		catch (Exception e)
		{
			return LuaDLL.toluaL_exception(L, e);
		}
	}

	[MonoPInvokeCallbackAttribute(typeof(LuaCSFunction))]
	static int FindClosestEdge(IntPtr L)
	{
		try
		{
			int count = LuaDLL.lua_gettop(L);

			if (count == 3 && TypeChecker.CheckTypes<int>(L, 3))
			{
				UnityEngine.Vector3 arg0 = ToLua.ToVector3(L, 1);
				UnityEngine.AI.NavMeshHit arg1;
				int arg2 = (int)LuaDLL.lua_tointeger(L, 3);
				bool o = UnityEngine.AI.NavMesh.FindClosestEdge(arg0, out arg1, arg2);
				LuaDLL.lua_pushboolean(L, o);
				ToLua.PushValue(L, arg1);
				return 2;
			}
			else if (count == 3 && TypeChecker.CheckTypes<UnityEngine.AI.NavMeshQueryFilter>(L, 3))
			{
				UnityEngine.Vector3 arg0 = ToLua.ToVector3(L, 1);
				UnityEngine.AI.NavMeshHit arg1;
				UnityEngine.AI.NavMeshQueryFilter arg2 = StackTraits<UnityEngine.AI.NavMeshQueryFilter>.To(L, 3);
				bool o = UnityEngine.AI.NavMesh.FindClosestEdge(arg0, out arg1, arg2);
				LuaDLL.lua_pushboolean(L, o);
				ToLua.PushValue(L, arg1);
				return 2;
			}
			else
			{
				return LuaDLL.luaL_throw(L, "invalid arguments to method: UnityEngine.AI.NavMesh.FindClosestEdge");
			}
		}
		catch (Exception e)
		{
			return LuaDLL.toluaL_exception(L, e);
		}
	}

	[MonoPInvokeCallbackAttribute(typeof(LuaCSFunction))]
	static int SamplePosition(IntPtr L)
	{
		try
		{
			int count = LuaDLL.lua_gettop(L);

			if (count == 4 && TypeChecker.CheckTypes<int>(L, 4))
			{
				UnityEngine.Vector3 arg0 = ToLua.ToVector3(L, 1);
				UnityEngine.AI.NavMeshHit arg1;
				float arg2 = (float)LuaDLL.luaL_checknumber(L, 3);
				int arg3 = (int)LuaDLL.lua_tointeger(L, 4);
				bool o = UnityEngine.AI.NavMesh.SamplePosition(arg0, out arg1, arg2, arg3);
				LuaDLL.lua_pushboolean(L, o);
				ToLua.PushValue(L, arg1);
				return 2;
			}
			else if (count == 4 && TypeChecker.CheckTypes<UnityEngine.AI.NavMeshQueryFilter>(L, 4))
			{
				UnityEngine.Vector3 arg0 = ToLua.ToVector3(L, 1);
				UnityEngine.AI.NavMeshHit arg1;
				float arg2 = (float)LuaDLL.luaL_checknumber(L, 3);
				UnityEngine.AI.NavMeshQueryFilter arg3 = StackTraits<UnityEngine.AI.NavMeshQueryFilter>.To(L, 4);
				bool o = UnityEngine.AI.NavMesh.SamplePosition(arg0, out arg1, arg2, arg3);
				LuaDLL.lua_pushboolean(L, o);
				ToLua.PushValue(L, arg1);
				return 2;
			}
			else
			{
				return LuaDLL.luaL_throw(L, "invalid arguments to method: UnityEngine.AI.NavMesh.SamplePosition");
			}
		}
		catch (Exception e)
		{
			return LuaDLL.toluaL_exception(L, e);
		}
	}

	[MonoPInvokeCallbackAttribute(typeof(LuaCSFunction))]
	static int SetAreaCost(IntPtr L)
	{
		try
		{
			ToLua.CheckArgsCount(L, 2);
			int arg0 = (int)LuaDLL.luaL_checkinteger(L, 1);
			float arg1 = (float)LuaDLL.luaL_checknumber(L, 2);
			UnityEngine.AI.NavMesh.SetAreaCost(arg0, arg1);
			return 0;
		}
		catch (Exception e)
		{
			return LuaDLL.toluaL_exception(L, e);
		}
	}

	[MonoPInvokeCallbackAttribute(typeof(LuaCSFunction))]
	static int GetAreaCost(IntPtr L)
	{
		try
		{
			ToLua.CheckArgsCount(L, 1);
			int arg0 = (int)LuaDLL.luaL_checkinteger(L, 1);
			float o = UnityEngine.AI.NavMesh.GetAreaCost(arg0);
			LuaDLL.lua_pushnumber(L, o);
			return 1;
		}
		catch (Exception e)
		{
			return LuaDLL.toluaL_exception(L, e);
		}
	}

	[MonoPInvokeCallbackAttribute(typeof(LuaCSFunction))]
	static int GetAreaFromName(IntPtr L)
	{
		try
		{
			ToLua.CheckArgsCount(L, 1);
			string arg0 = ToLua.CheckString(L, 1);
			int o = UnityEngine.AI.NavMesh.GetAreaFromName(arg0);
			LuaDLL.lua_pushinteger(L, o);
			return 1;
		}
		catch (Exception e)
		{
			return LuaDLL.toluaL_exception(L, e);
		}
	}

	[MonoPInvokeCallbackAttribute(typeof(LuaCSFunction))]
	static int CalculateTriangulation(IntPtr L)
	{
		try
		{
			ToLua.CheckArgsCount(L, 0);
			UnityEngine.AI.NavMeshTriangulation o = UnityEngine.AI.NavMesh.CalculateTriangulation();
			ToLua.PushValue(L, o);
			return 1;
		}
		catch (Exception e)
		{
			return LuaDLL.toluaL_exception(L, e);
		}
	}

	[MonoPInvokeCallbackAttribute(typeof(LuaCSFunction))]
	static int AddNavMeshData(IntPtr L)
	{
		try
		{
			int count = LuaDLL.lua_gettop(L);

			if (count == 1)
			{
				UnityEngine.AI.NavMeshData arg0 = (UnityEngine.AI.NavMeshData)ToLua.CheckObject<UnityEngine.AI.NavMeshData>(L, 1);
				UnityEngine.AI.NavMeshDataInstance o = UnityEngine.AI.NavMesh.AddNavMeshData(arg0);
				ToLua.PushValue(L, o);
				return 1;
			}
			else if (count == 3)
			{
				UnityEngine.AI.NavMeshData arg0 = (UnityEngine.AI.NavMeshData)ToLua.CheckObject<UnityEngine.AI.NavMeshData>(L, 1);
				UnityEngine.Vector3 arg1 = ToLua.ToVector3(L, 2);
				UnityEngine.Quaternion arg2 = ToLua.ToQuaternion(L, 3);
				UnityEngine.AI.NavMeshDataInstance o = UnityEngine.AI.NavMesh.AddNavMeshData(arg0, arg1, arg2);
				ToLua.PushValue(L, o);
				return 1;
			}
			else
			{
				return LuaDLL.luaL_throw(L, "invalid arguments to method: UnityEngine.AI.NavMesh.AddNavMeshData");
			}
		}
		catch (Exception e)
		{
			return LuaDLL.toluaL_exception(L, e);
		}
	}

	[MonoPInvokeCallbackAttribute(typeof(LuaCSFunction))]
	static int RemoveNavMeshData(IntPtr L)
	{
		try
		{
			ToLua.CheckArgsCount(L, 1);
			UnityEngine.AI.NavMeshDataInstance arg0 = StackTraits<UnityEngine.AI.NavMeshDataInstance>.Check(L, 1);
			UnityEngine.AI.NavMesh.RemoveNavMeshData(arg0);
			return 0;
		}
		catch (Exception e)
		{
			return LuaDLL.toluaL_exception(L, e);
		}
	}

	[MonoPInvokeCallbackAttribute(typeof(LuaCSFunction))]
	static int AddLink(IntPtr L)
	{
		try
		{
			int count = LuaDLL.lua_gettop(L);

			if (count == 1)
			{
				UnityEngine.AI.NavMeshLinkData arg0 = StackTraits<UnityEngine.AI.NavMeshLinkData>.Check(L, 1);
				UnityEngine.AI.NavMeshLinkInstance o = UnityEngine.AI.NavMesh.AddLink(arg0);
				ToLua.PushValue(L, o);
				return 1;
			}
			else if (count == 3)
			{
				UnityEngine.AI.NavMeshLinkData arg0 = StackTraits<UnityEngine.AI.NavMeshLinkData>.Check(L, 1);
				UnityEngine.Vector3 arg1 = ToLua.ToVector3(L, 2);
				UnityEngine.Quaternion arg2 = ToLua.ToQuaternion(L, 3);
				UnityEngine.AI.NavMeshLinkInstance o = UnityEngine.AI.NavMesh.AddLink(arg0, arg1, arg2);
				ToLua.PushValue(L, o);
				return 1;
			}
			else
			{
				return LuaDLL.luaL_throw(L, "invalid arguments to method: UnityEngine.AI.NavMesh.AddLink");
			}
		}
		catch (Exception e)
		{
			return LuaDLL.toluaL_exception(L, e);
		}
	}

	[MonoPInvokeCallbackAttribute(typeof(LuaCSFunction))]
	static int RemoveLink(IntPtr L)
	{
		try
		{
			ToLua.CheckArgsCount(L, 1);
			UnityEngine.AI.NavMeshLinkInstance arg0 = StackTraits<UnityEngine.AI.NavMeshLinkInstance>.Check(L, 1);
			UnityEngine.AI.NavMesh.RemoveLink(arg0);
			return 0;
		}
		catch (Exception e)
		{
			return LuaDLL.toluaL_exception(L, e);
		}
	}

	[MonoPInvokeCallbackAttribute(typeof(LuaCSFunction))]
	static int CreateSettings(IntPtr L)
	{
		try
		{
			ToLua.CheckArgsCount(L, 0);
			UnityEngine.AI.NavMeshBuildSettings o = UnityEngine.AI.NavMesh.CreateSettings();
			ToLua.PushValue(L, o);
			return 1;
		}
		catch (Exception e)
		{
			return LuaDLL.toluaL_exception(L, e);
		}
	}

	[MonoPInvokeCallbackAttribute(typeof(LuaCSFunction))]
	static int RemoveSettings(IntPtr L)
	{
		try
		{
			ToLua.CheckArgsCount(L, 1);
			int arg0 = (int)LuaDLL.luaL_checkinteger(L, 1);
			UnityEngine.AI.NavMesh.RemoveSettings(arg0);
			return 0;
		}
		catch (Exception e)
		{
			return LuaDLL.toluaL_exception(L, e);
		}
	}

	[MonoPInvokeCallbackAttribute(typeof(LuaCSFunction))]
	static int GetSettingsByID(IntPtr L)
	{
		try
		{
			ToLua.CheckArgsCount(L, 1);
			int arg0 = (int)LuaDLL.luaL_checkinteger(L, 1);
			UnityEngine.AI.NavMeshBuildSettings o = UnityEngine.AI.NavMesh.GetSettingsByID(arg0);
			ToLua.PushValue(L, o);
			return 1;
		}
		catch (Exception e)
		{
			return LuaDLL.toluaL_exception(L, e);
		}
	}

	[MonoPInvokeCallbackAttribute(typeof(LuaCSFunction))]
	static int GetSettingsCount(IntPtr L)
	{
		try
		{
			ToLua.CheckArgsCount(L, 0);
			int o = UnityEngine.AI.NavMesh.GetSettingsCount();
			LuaDLL.lua_pushinteger(L, o);
			return 1;
		}
		catch (Exception e)
		{
			return LuaDLL.toluaL_exception(L, e);
		}
	}

	[MonoPInvokeCallbackAttribute(typeof(LuaCSFunction))]
	static int GetSettingsByIndex(IntPtr L)
	{
		try
		{
			ToLua.CheckArgsCount(L, 1);
			int arg0 = (int)LuaDLL.luaL_checkinteger(L, 1);
			UnityEngine.AI.NavMeshBuildSettings o = UnityEngine.AI.NavMesh.GetSettingsByIndex(arg0);
			ToLua.PushValue(L, o);
			return 1;
		}
		catch (Exception e)
		{
			return LuaDLL.toluaL_exception(L, e);
		}
	}

	[MonoPInvokeCallbackAttribute(typeof(LuaCSFunction))]
	static int GetSettingsNameFromID(IntPtr L)
	{
		try
		{
			ToLua.CheckArgsCount(L, 1);
			int arg0 = (int)LuaDLL.luaL_checkinteger(L, 1);
			string o = UnityEngine.AI.NavMesh.GetSettingsNameFromID(arg0);
			LuaDLL.lua_pushstring(L, o);
			return 1;
		}
		catch (Exception e)
		{
			return LuaDLL.toluaL_exception(L, e);
		}
	}

	[MonoPInvokeCallbackAttribute(typeof(LuaCSFunction))]
	static int RemoveAllNavMeshData(IntPtr L)
	{
		try
		{
			ToLua.CheckArgsCount(L, 0);
			UnityEngine.AI.NavMesh.RemoveAllNavMeshData();
			return 0;
		}
		catch (Exception e)
		{
			return LuaDLL.toluaL_exception(L, e);
		}
	}

	[MonoPInvokeCallbackAttribute(typeof(LuaCSFunction))]
	static int get_onPreUpdate(IntPtr L)
	{
		try
		{
			ToLua.Push(L, UnityEngine.AI.NavMesh.onPreUpdate);
			return 1;
		}
		catch (Exception e)
		{
			return LuaDLL.toluaL_exception(L, e);
		}
	}

	[MonoPInvokeCallbackAttribute(typeof(LuaCSFunction))]
	static int get_avoidancePredictionTime(IntPtr L)
	{
		try
		{
			LuaDLL.lua_pushnumber(L, UnityEngine.AI.NavMesh.avoidancePredictionTime);
			return 1;
		}
		catch (Exception e)
		{
			return LuaDLL.toluaL_exception(L, e);
		}
	}

	[MonoPInvokeCallbackAttribute(typeof(LuaCSFunction))]
	static int get_pathfindingIterationsPerFrame(IntPtr L)
	{
		try
		{
			LuaDLL.lua_pushinteger(L, UnityEngine.AI.NavMesh.pathfindingIterationsPerFrame);
			return 1;
		}
		catch (Exception e)
		{
			return LuaDLL.toluaL_exception(L, e);
		}
	}

	[MonoPInvokeCallbackAttribute(typeof(LuaCSFunction))]
	static int set_onPreUpdate(IntPtr L)
	{
		try
		{
			UnityEngine.AI.NavMesh.OnNavMeshPreUpdate arg0 = (UnityEngine.AI.NavMesh.OnNavMeshPreUpdate)ToLua.CheckDelegate<UnityEngine.AI.NavMesh.OnNavMeshPreUpdate>(L, 2);
			UnityEngine.AI.NavMesh.onPreUpdate = arg0;

			if (!object.ReferenceEquals(UnityEngine.AI.NavMesh.onPreUpdate, arg0))
			{
				if (UnityEngine.AI.NavMesh.onPreUpdate != null) UnityEngine.AI.NavMesh.onPreUpdate.SubRef();
				UnityEngine.AI.NavMesh.onPreUpdate = arg0;
			}

			return 0;
		}
		catch (Exception e)
		{
			return LuaDLL.toluaL_exception(L, e);
		}
	}

	[MonoPInvokeCallbackAttribute(typeof(LuaCSFunction))]
	static int set_avoidancePredictionTime(IntPtr L)
	{
		try
		{
			float arg0 = (float)LuaDLL.luaL_checknumber(L, 2);
			UnityEngine.AI.NavMesh.avoidancePredictionTime = arg0;
			UnityEngine.AI.NavMesh.avoidancePredictionTime = arg0;
			return 0;
		}
		catch (Exception e)
		{
			return LuaDLL.toluaL_exception(L, e);
		}
	}

	[MonoPInvokeCallbackAttribute(typeof(LuaCSFunction))]
	static int set_pathfindingIterationsPerFrame(IntPtr L)
	{
		try
		{
			int arg0 = (int)LuaDLL.luaL_checkinteger(L, 2);
			UnityEngine.AI.NavMesh.pathfindingIterationsPerFrame = arg0;
			UnityEngine.AI.NavMesh.pathfindingIterationsPerFrame = arg0;
			return 0;
		}
		catch (Exception e)
		{
			return LuaDLL.toluaL_exception(L, e);
		}
	}

	[MonoPInvokeCallbackAttribute(typeof(LuaCSFunction))]
	static int UnityEngine_AI_NavMesh_OnNavMeshPreUpdate(IntPtr L)
	{
		try
		{
			int count = LuaDLL.lua_gettop(L);
			LuaFunction func = ToLua.CheckLuaFunction(L, 1);

			if (count == 1)
			{
				Delegate arg1 = DelegateTraits<UnityEngine.AI.NavMesh.OnNavMeshPreUpdate>.Create(func);
				ToLua.Push(L, arg1);
				func.Dispose();
			}
			else
			{
				LuaTable self = ToLua.CheckLuaTable(L, 2);
				Delegate arg1 = DelegateTraits<UnityEngine.AI.NavMesh.OnNavMeshPreUpdate>.Create(func, self);
				ToLua.Push(L, arg1);
				func.Dispose();
				self.Dispose();
			}
			return 1;
		}
		catch(Exception e)
		{
			return LuaDLL.toluaL_exception(L, e);
		}
	}
}


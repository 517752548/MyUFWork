...
namespace LuaFramework
{
	...
	public class ObjectPoolManager : Manager
	{
		...
		Transform PoolRootObject
		{
			get
			{
				if (m_PoolRootObject == null)
				{
					...
					objectPool.transform.localPosition = new Vector3(1000, 1000, 1000);
		...
		public GameObjectPool CreatePool(string poolName, int initSize = 0, int maxSize = 0, GameObject prefab = null)
		{
			var pool = new GameObjectPool(poolName, PoolRootObject, prefab, initSize, maxSize);
		...
		{
			foreach (GameObjectPool gop in m_GameObjectPools.Values)
			{
				gop.ClearAll();
			}
		}

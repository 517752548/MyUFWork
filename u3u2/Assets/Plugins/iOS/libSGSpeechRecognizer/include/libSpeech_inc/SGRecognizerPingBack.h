...
@property (nonatomic, retain) NSString *imeiNo;
...
//不建议调用此函数
+ (void)onStopRecordWithClickType:(int)click;
+ (void)setPre_interval_start_time:(long long)preIntervalStartTime;
...
//语音识别的开始时间戳
+ (void)onStartRecord:(NSString *)startTime;
//检测到有效声音了，会取一个当前时间戳。
+ (void)onVoiced;
//设置chosen字段
